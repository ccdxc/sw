...
class TunnelObject(objects.FrameworkObject):
    def __init__(self):
        super().__init__()
    ...
    def Init(self, tenant, spec, remote_ep):
        ...
        self.remote_tep = remote_ep.GetIpAddress()
        self.Show()
        return
    ...
    def Show(self):
        ...
        cfglogger.info("  - LocalTep    = %s" % self.tenant.local_tep.get())
        cfglogger.info("  - RemoteTep   = %s" % self.remote_tep.get())
    ...
    def PrepareHALRequestSpec(self, req_spec):
        ...
        if self.IsVxlan():
            ...
            req_spec.if_tunnel_info.vxlan_info.local_tep.v4_addr = self.tenant.local_tep.getnum()
            # Remote TEP
            req_spec.if_tunnel_info.vxlan_info.remote_tep.ip_af = haldefs.common.IP_AF_INET
            req_spec.if_tunnel_info.vxlan_info.remote_tep.v4_addr = self.remote_tep.getnum()
        else:
            assert(0)
        return
