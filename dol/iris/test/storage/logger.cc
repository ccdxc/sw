...
const auto LOG_FILENAME = "./offload.log";
     ...
     const char *requested_level)
{
    ...
    if (std::getenv("OFFLOAD_LOG_LEVEL") != NULL) {
        level_str.assign(std::getenv("OFFLOAD_LOG_LEVEL"));
    ...
    _logger = std::make_shared<spdlog::logger>("offload", begin(sinks), end(sinks));
    assert(_logger != NULL);
    ...
    _syslogger = spdlog::syslog_logger("offload-syslog", "syslog", LOG_PID);
...
void
offl_api_trace (const char *trace)
{
