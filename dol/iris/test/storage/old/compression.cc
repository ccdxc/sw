...
static void populate_sgls(uint32_t data_len, uint16_t num_entries,
                          void *sgl_va_in, uint64_t sgl_pa, uint64_t src_pa) {
  bool sgl_in_host_mem = sgl_pa & 0x8000000000000000ull;
  uint32_t chunk_size = data_len / num_entries;
  uint32_t last_chunk_size = chunk_size + (data_len - (chunk_size * num_entries));
  assert(chunk_size > 0);
...
static int run_cp_test(comp_test_t *params) {
  ...
  if (params->num_src_sgls == 1) {
    ...
    if (params->src_sgl_is_hbm) {
      populate_sgls(params->datain_size, params->num_src_sgls, nullptr,
                    hbm_sgl_buf_pa, bufin_pa);
      d.src = hbm_sgl_buf_pa;
    } else {
      populate_sgls(params->datain_size, params->num_src_sgls, sgl_buf,
                    sgl_buf_pa, bufin_pa);
      d.src = sgl_buf_pa;
    }
  ...
  if (params->cmd_bits.comp_decomp_en) {
    if (params->num_dst_sgls == 1) {
      d.dst = bufout_pa;
    } else {
      if (params->dst_sgl_is_hbm) {
        populate_sgls(params->dataout_size, params->num_dst_sgls, nullptr,
                      hbm_sgl_buf_pa + 2048, bufout_pa);
        d.dst = hbm_sgl_buf_pa + 2048;
      } else {
        populate_sgls(params->dataout_size, params->num_dst_sgls, sgl_buf + 2048,
  ...
  d.datain_len = params->datain_size & 0xFFFF;
  d.extended_len = (params->datain_size >> 16) & 0xFFFF;
  d.threshold_len = params->comp_threshold;
