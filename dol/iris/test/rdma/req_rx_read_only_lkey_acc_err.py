...
def TestCasePreTrigger(tc):
    ...
    tc.pvtdata.acc_ctrl = kt_entry.data.acc_ctrl
    kt_entry.data.acc_ctrl = 0 #no access rights
...
def TestCaseStepVerify(tc, step):
    ...
    if step.step_id == 0:
        ...
        # verify qp_err_dis_rrqlkey_acc_no_wr_perm is set
        if not VerifyFieldModify(tc, tc.pvtdata.sq_pre_qstate, tc.pvtdata.sq_post_qstate, 'qp_err_dis_rrqlkey_acc_no_wr_perm', 1):
            return False
...
def TestCaseTeardown(tc):
    ...
    rs = tc.config.rdmasession
    kt_entry = RdmaKeyTableEntryObject(rs.lqp.pd.ep.intf.lif, tc.pvtdata.l_key)
    kt_entry.data.acc_ctrl = tc.pvtdata.acc_ctrl
    kt_entry.WriteWithDelay()
