...
def TestCaseStepVerify(tc, step):
    ...
    if step.step_id == 0:
        # verify that tx_psn is incremented by 9
        if not VerifyFieldModify(tc, tc.pvtdata.sq_pre_qstate, tc.pvtdata.sq_post_qstate, 'tx_psn', 9):
            return False
        ...
        # verify that token_id is incremented by 9
        if not VerifyFieldModify(tc, tc.pvtdata.sq_pre_qstate, tc.pvtdata.sq_post_qstate, 'token_id', 9):
            return False

        # verify that nxt_to_go_token_id is incremented by 9
        if not VerifyFieldModify(tc, tc.pvtdata.sq_pre_qstate, tc.pvtdata.sq_post_qstate, 'nxt_to_go_token_id', 9):
            return False
