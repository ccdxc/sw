...
def TestCaseStepVerify(tc, step):
    ...
    if step.step_id == 0:
        # verify that tx_psn is incremented by 5
        if not VerifyFieldModify(tc, tc.pvtdata.sq_pre_qstate, tc.pvtdata.sq_post_qstate, 'tx_psn', 5):
            return False
        ...
        # verify that ssn is incremented by 4
        if not VerifyFieldModify(tc, tc.pvtdata.sq_pre_qstate, tc.pvtdata.sq_post_qstate, 'ssn', 4):
            return False
        ...
        # validate cqcb pindex and color
        if not ValidateReqRxCQChecks(tc, 'EXP_CQ_DESC_3'):
            return False
