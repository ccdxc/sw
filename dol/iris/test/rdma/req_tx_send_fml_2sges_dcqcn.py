...
def TestCaseSetup(tc):
    ...
    rs.lqp.dcqcn_data.cur_timestamp = 0xCA06C0 # 13240k ticks
...
def TestCaseVerify(tc):
    ...
    # verify that cur_avail_tokens in dcqcn state is 78. Since at end of 9 iteration 13240k / 833 + 9*120=15894 + 1080 = 16974 bits will be accumulated 
    # with core-clock running at 833 MHz.
    # 8*2112=16896  bits will be consumed by packet. So 16974-16896=78 tokens remain.
    if not VerifyFieldAbsolute(tc, tc.pvtdata.dcqcn_post_qstate, 'cur_avail_tokens', 78):
        return False
    ...
    # verify that packets dropped = 9 to accumulate the required tokens to send out Middle and Last packets eventually.
    ...
    if not VerifyFieldAbsolute(tc, tc.pvtdata.dcqcn_post_qstate, 'num_sched_drop', 9):
        return False
