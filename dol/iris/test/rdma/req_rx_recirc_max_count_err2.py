...
def TestCaseStepVerify(tc, step):
    ...
    if step.step_id == 0:
        ...
        if not VerifyFieldAbsolute(tc, tc.pvtdata.sq_post_qstate, 'sqcb5_recirc_reason', 1):
            return False 

         # recirc bth opcode should be 13 (read-resp-first) 
        if not VerifyFieldAbsolute(tc, tc.pvtdata.sq_post_qstate, 'sqcb5_recirc_bth_opcode', 13):
            return False
