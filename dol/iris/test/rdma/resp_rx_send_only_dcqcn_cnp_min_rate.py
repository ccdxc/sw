...
def TestCaseSetup(tc):
    ...
    tc.pvtdata.dcqcn_profile.data.rp_rate_reduce_monitor_period = 50    # 50us - min_time_between_cnps
    ...
    # Set min_qp_rate and min_qp_target_rate to 4000Mbps and 9000 Mbps respectively
    tc.pvtdata.dcqcn_profile.data.rp_min_rate = 4000
    tc.pvtdata.dcqcn_profile.data.rp_min_target_rate = 9000
...
def TestCaseStepVerify(tc, step):
    ...
    if step.step_id == 0:
        ...
        if not VerifyFieldAbsolute(tc, tc.pvtdata.dcqcn_post_qstate, 'target_rate', 9000):
            return False

            # verify that rate-enforced is set to min_rate
        if not VerifyFieldAbsolute(tc, tc.pvtdata.dcqcn_post_qstate, 'rate_enforced', 4000):
            return False
