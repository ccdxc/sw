...
def TestCaseSetup(tc):
    ...
    rs.lqp.dcqcn_data.rate_enforced = 1000
    ...
    # Set min_qp_rate and min_qp_target_rate to 200Mbps and 400 Mbps respectively
    tc.pvtdata.dcqcn_profile.data.rp_min_rate = 200
    tc.pvtdata.dcqcn_profile.data.rp_min_target_rate = 400
...
def TestCaseStepVerify(tc, step):
    ...
    if step.step_id == 0:
        ...
        if not VerifyFieldAbsolute(tc, tc.pvtdata.dcqcn_post_qstate, 'target_rate', 1000):
            return False

            # verify that rate-enforced is cut by half.
        if not VerifyFieldAbsolute(tc, tc.pvtdata.dcqcn_post_qstate, 'rate_enforced', 500):
            return False
        ...
        if not VerifyFieldAbsolute(tc, tc.pvtdata.dcqcn_post_qstate, 'cur_byte_counter', 0):
            ...
            # verify that target-rate is set to rate-enforced.
        if not VerifyFieldAbsolute(tc, tc.pvtdata.dcqcn_post_qstate, 'target_rate', 500):
            return False

            # verify that rate-enforced is cut by half.
        if not VerifyFieldAbsolute(tc, tc.pvtdata.dcqcn_post_qstate, 'rate_enforced', 250):
            return False
        ...
        if not VerifyFieldAbsolute(tc, tc.pvtdata.dcqcn_post_qstate, 'cur_byte_counter', 0):
            ...
            # verify that target-rate is set to min_target_rate.
        if not VerifyFieldAbsolute(tc, tc.pvtdata.dcqcn_post_qstate, 'target_rate', 400):
            return False

            # verify that rate-enforced is set to min_rate.
        if not VerifyFieldAbsolute(tc, tc.pvtdata.dcqcn_post_qstate, 'rate_enforced', 200):
            return False
