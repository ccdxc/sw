...
def TestCaseSetup(tc):
    ...
    dcqcn_profile.data.rp_dce_tcp_g = 256
...
def TestCaseStepTrigger(tc, step):
    ...
    if step.step_id == 2:
        logger.info("RDMA TestCaseStepTrigger() - Fast Forwarding the system time by by 112 ticks for FAST_TIMER wheel")
        timer = tc.infra_data.ConfigStore.objects.db['FAST_TIMER']
        timer.Step(112)
    return
...
def TestCaseStepVerify(tc, step):
    ...
    if step.step_id == 0:
        ...
        if not VerifyFieldAbsolute(tc, tc.pvtdata.dcqcn_post_qstate, 'max_rate_reached', 0):
            return False
    ...
    if step.step_id == 1:
        ...
        # verify that alpha value is set to 65279 based on dcqcn algorithm calculations.
        if not VerifyFieldAbsolute(tc, tc.pvtdata.dcqcn_post_qstate, 'alpha_value', 65279):
            return False

        if not VerifyFieldModify(tc, tc.pvtdata.dcqcn_pre_qstate, tc.pvtdata.dcqcn_post_qstate, 'num_alpha_exp_cnt', 1):
            return False

        # timer_process is triggered prior to rate_process, max_rate_reached is not set yet
        # so timer_process will still restart timer this time
        if not VerifyFieldAbsolute(tc, tc.pvtdata.dcqcn_post_qstate, 'max_rate_reached', 1):
            return False
        
    if step.step_id == 2:
        # skip timer restart
        # verify that alpha value is set to 65024 based on dcqcn algorithm calculations.
        if not VerifyFieldAbsolute(tc, tc.pvtdata.dcqcn_post_qstate, 'alpha_value', 65024):
            return False

        if not VerifyFieldModify(tc, tc.pvtdata.dcqcn_pre_qstate, tc.pvtdata.dcqcn_post_qstate, 'num_alpha_exp_cnt', 2):
            return False
