...
def TestCaseSetup(tc):
    ...
    rs.lqp.dcqcn_data.cur_timestamp = 0x1 # 1 tick
...
def TestCaseVerify(tc):
    ...
    # verify that cur_avail_tokens in dcqcn state is 2772. Since at end of 2 iteration 2*9834=19668 bits will be accumulated 
    ...
    # verify that last_sched_timestamp in dcqcn state is set to cur_timestamp of iteration 2 which is 2*8192000 + 1 ticks
    if not VerifyFieldAbsolute(tc, tc.pvtdata.dcqcn_post_qstate, 'last_sched_timestamp', 0xFA0001):
        return False

    # verify that packets dropped = 2 to accumulate the required tokens to send out Middle and Last packets eventually.
    # It requires 2 iterations to accumulate enough tokens to send 2112B packet at rate 1 Mbps,
