        ...
        try {
            BOOST_FOREACH (pt::ptree::value_type &obj,
                ...
                if (kind == "device") {
                    ...
                    cfg_params.service_ipv4_addr = 0; // TODO
...
public:
    ...
    void add_local_ep(pds_local_mapping_spec_t *local_spec) {
        ...
        if (ip_addr.af == IP_AF_IPV4) {
            ...
            service_ip.addr.v4_addr = cfg_params.service_ipv4_addr | ip_offset;
            ip_offset++;
    ...
    sdk_ret_t create_session_info(uint32_t vpc, uint32_t nexthop_idx) {
            ...
            for (i = 0; i < MAX_EP_PAIRS_PER_VPC ; i++) {
                for (auto lp = cfg_params.sport_lo; lp <= cfg_params.sport_hi; lp++) {
                    for (auto rp = cfg_params.dport_lo; rp <= cfg_params.dport_hi; rp++) {
                        ...
                        // create V4 session
                        ret = create_session_info(vpc,  ep_pairs[i].v4_remote.nh_idx);
                        if (ret != SDK_RET_OK) {
                            return ret;
                        }
                        session_index++;
                        ...
                        if (dual_stack) {
                            ...
                            // create V6 session
                            ret = create_session_info(vpc, ep_pairs[i].v6_remote.nh_idx);
                            if (ret != SDK_RET_OK) {
                                return ret;
                            }
                            session_index++;
