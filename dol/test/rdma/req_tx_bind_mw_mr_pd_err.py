...
def TestCaseStepVerify(tc, step):
    ...
    if step.step_id == 0:
        ...
        # read the key table entry for rkey and verify state
        kt_entry = RdmaKeyTableEntryObject(rs.lqp.pd.ep.intf.lif, (tc.pvtdata.r_key & 0xFFFFFF))

        if ((kt_entry.data.state != 1) or (kt_entry.data.pt_base != tc.pvtdata.pre_pt_base)
            or (kt_entry.data.base_va != tc.pvtdata.pre_base_va)):
            logger.info("RDMA TestCaseVerify(): Bind MW type pd check validation fails  for hw_lif %d qp %s rkey %d " %
    ...
    logger.info("RDMA TestCaseVerify(): Bind MW PD mismatch between MW and MR for hw_lif %d qp %s rkey %d lkey %d" %
                (rs.lqp.pd.ep.intf.lif.hw_lif_id, rs.lqp.GID(), tc.pvtdata.r_key, tc.pvtdata.l_key))
