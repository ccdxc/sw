...
def TestCasePreTrigger(tc):
    ...
    rs = tc.config.rdmasession
    kt_entry = RdmaKeyTableEntryObject(rs.lqp.pd.ep.intf.lif, (tc.pvtdata.r_key & 0xFFFFFF))
    # Set MW RKEY state to invalid and request MW bind
...
def TestCaseStepVerify(tc, step):
    ...
    if step.step_id == 0:
        ...
        if ((kt_entry.data.state == 2) or (kt_entry.data.pt_base != tc.pvtdata.pre_pt_base)
            ...
            logger.info("RDMA TestCaseVerify(): Bind MW invalid MW state fails for hw_lif %d qp %s rkey %d " %
    ...
    logger.info("RDMA TestCaseVerify(): Bind MW invalid MW state for hw_lif %d qp %s rkey %d" %
                (rs.lqp.pd.ep.intf.lif.hw_lif_id, rs.lqp.GID(), tc.pvtdata.r_key))
    return True
