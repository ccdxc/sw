...
def TestCaseStepVerify(tc, step):
    ...
    if step.step_id == 0:
        ...
        if ((kt_entry.data.state != 1) or (kt_entry.data.pt_base != tc.pvtdata.pre_pt_base)
            or (kt_entry.data.base_va != tc.pvtdata.pre_base_va)):
            logger.info("RDMA TestCaseVerify(): Bind MW type mismatch fails for hw_lif %d qp %s rkey %d " %
                    (rs.lqp.pd.ep.intf.lif.hw_lif_id, rs.lqp.GID(), tc.pvtdata.r_key))
            logger.info("RDMA TestCaseVerify(): Bind MW  Rkey is not in Free state: state %d" %
                    kt_entry.data.state)
            return False
    ...
    logger.info("RDMA TestCaseVerify(): Bind MW type mismatch for hw_lif %d qp %s rkey %d" %
            (rs.lqp.pd.ep.intf.lif.hw_lif_id, rs.lqp.GID(), tc.pvtdata.r_key))
