...
def TestCasePreTrigger(tc):
    ...
    rs = tc.config.rdmasession
    slab_id = tc.buffers.Get('READ_BUF').slab_id
...
def TestCaseStepVerify(tc, step):
    ...
    if step.step_id == 0:
        ...
        # verify that rexmit_psn and tx_psn are equal
        if not VerifyFieldAbsolute(tc, tc.pvtdata.sq_post_qstate, 'rexmit_psn',  tc.pvtdata.sq_post_qstate.tx_psn):
            return False
