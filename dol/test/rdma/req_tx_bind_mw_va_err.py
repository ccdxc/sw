...
def TestCaseSetup(tc):
    ...
    if (GlobalOptions.dryrun):
        ...
        tc.pvtdata.mw_va = tc.pvtdata.slab.address - 1
...
def TestCaseStepVerify(tc, step):
    ...
    if step.step_id == 0:
        ...
        if ((kt_entry.data.state != 2) or (kt_entry.data.pt_base != tc.pvtdata.pre_pt_base)
            ...
            logger.info("RDMA TestCaseVerify(): Bind MW invalid va fails for hw_lif %d qp %s rkey %d " %
    ...
    logger.info("RDMA TestCaseVerify(): Bind MW invalid va for hw_lif %d qp %s rkey %d" %
                (rs.lqp.pd.ep.intf.lif.hw_lif_id, rs.lqp.GID(), tc.pvtdata.r_key))
    return True
