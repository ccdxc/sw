...
def TestCasePreTrigger(tc):
    ...
    # UnSet MR's local write access ctrl and request MW bind to the region
...
def TestCaseStepVerify(tc, step):
    ...
    logger.info("RDMA TestCaseVerify(): Bind MW acc ctrl err state for hw_lif %d qp %s rkey %d" %
                (rs.lqp.pd.ep.intf.lif.hw_lif_id, rs.lqp.GID(), tc.pvtdata.r_key))
    return True
