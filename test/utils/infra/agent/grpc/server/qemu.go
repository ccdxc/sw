...
const (
	testInfraClientImage    = infra.RemoteQemuDirectory + "/" + "testInfraClient.tar.gz"
	...
	qemuImage               = "build-37"
	qemuImageOva            = infra.RemoteQemuDirectory + "/" + qemuImage + ".ova"
	qemuImageQcow           = infra.RemoteQemuDirectory + "/" + qemuImage + ".qcow2"
...
func (s *Qemu) setupQemu() error {
	...
	cwd, _ := os.Getwd()
	mkdir := []string{"mkdir", "-p", infra.RemoteQemuDirectory}
	if _, stdout, err := Common.Run(mkdir, 0, false, false, nil); err != nil {
		return errors.Wrap(err, stdout)
	}

	os.Chdir(infra.RemoteQemuDirectory)
	defer os.Chdir(cwd)
