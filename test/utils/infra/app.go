...
func (ns *NS) AttachInterface(intfName string) error {
	...
	_, stdout, err := Common.Run(cmdArgs, 0, false, false, nil)
	if err != nil {
...
func (ns *NS) Delete() {
	...
	Common.Run(cmdArgs, 0, false, false, nil)
}
...
func (ns *NS) RunCommand(cmd []string, timeout int, background bool) (string, error) {
	...
	_, out, err := Common.Run(nsCmd, timeout, background, false, nil)
	return out, err
}
...
func NewContainer(name string,
	...
	Common.Run(nsCmd, 0, false, false, nil)
