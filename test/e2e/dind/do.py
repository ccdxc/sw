...
class NaplesNode(Node):
    ...
    def startNode(self):
        ...
        if self.testMode == "TELEMETRY":
            ...
            runCommand("""docker exec {}  bash -c "ip link set eth0 down && sleep 1 && if [[ \`ip link show oob_mnic0\` ]] ; then ip link set oob_mnic0 name oob_minc0old; fi && sleep 1 && ip link set eth0 name oob_mnic0 && ip link set oob_mnic0 up" """.format(self.name))
        else:
