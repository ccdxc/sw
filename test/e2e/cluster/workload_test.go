...
func (wtg *workloadTestGroup) testWorkloadCrudops() {
	...
	Eventually(func() bool {
		w1, err := wtg.wkldIf.Create(ts.tu.NewLoggedInContext(context.Background()), &wkld1)
	...
	Eventually(func() bool {
		w2, err := wtg.wkldIf.Create(ts.tu.NewLoggedInContext(context.Background()), &wkld2)
	...
	Eventually(func() bool {
		...
		w1, err := wtg.wkldIf.Get(ts.tu.NewLoggedInContext(context.Background()), &obj)
		if err != nil || w1.Name != wkld1.Name {
	...
	Eventually(func() bool {
		...
		w2, err := wtg.wkldIf.Get(ts.tu.NewLoggedInContext(context.Background()), &obj)
		if err != nil || w2.Name != wkld2.Name {
	...
	Eventually(func() bool {
		...
		wklds, err := wtg.wkldIf.List(ts.tu.NewLoggedInContext(context.Background()), &api.ListWatchOptions{ObjectMeta: ometa})
	...
	Eventually(func() bool {
		w2, err := wtg.wkldIf.Update(ts.tu.NewLoggedInContext(context.Background()), &wkld2)
		if err != nil || w2.Name != wkld2.Name {
			return false
		}
	...
	Eventually(func() bool {
		obj := api.ObjectMeta{Name: wkld1Name, Tenant: "default"}
		w1, err := wtg.wkldIf.Delete(ts.tu.NewLoggedInContext(context.Background()), &obj)
		if err != nil || w1.Name != wkld1.Name {
			return false
		}
	...
	Eventually(func() bool {
		obj := api.ObjectMeta{Name: wkld2Name, Tenant: "default"}
		w2, err := wtg.wkldIf.Delete(ts.tu.NewLoggedInContext(context.Background()), &obj)
		if err != nil || w2.Name != wkld2.Name {
			return false
		}
...
func (wtg *workloadTestGroup) testWorkloadHostname() {
	...
	Eventually(func() bool {
		_, err := wtg.wkldIf.Create(ts.tu.NewLoggedInContext(context.Background()), &wkld3)
	...
	Eventually(func() bool {
		w3, err := wtg.wkldIf.Create(ts.tu.NewLoggedInContext(context.Background()), &wkld3)
	...
	Eventually(func() bool {
		_, err := wtg.wkldIf.Update(ts.tu.NewLoggedInContext(context.Background()), &wkld3)
	...
	Eventually(func() bool {
		w3, err := wtg.wkldIf.Update(ts.tu.NewLoggedInContext(context.Background()), &wkld3)
		if err != nil || w3.Name != wkld3.Name || w3.Spec.HostName != wkld3.Spec.HostName {
...
func (wtg *workloadTestGroup) testWorkloadMacaddr() {
	...
	Eventually(func() bool {
		_, err := wtg.wkldIf.Create(ts.tu.NewLoggedInContext(context.Background()), &wkld4)
	...
	Eventually(func() bool {
		w4, err := wtg.wkldIf.Create(ts.tu.NewLoggedInContext(context.Background()), &wkld4)
...
func (wtg *workloadTestGroup) testdownTest() {
	...
	wtg.wkldIf.Delete(ts.tu.NewLoggedInContext(context.Background()), &obj1)

	obj2 := api.ObjectMeta{Name: wkld2Name, Tenant: "default"}
	wtg.wkldIf.Delete(ts.tu.NewLoggedInContext(context.Background()), &obj2)

	obj3 := api.ObjectMeta{Name: wkld3Name, Tenant: "default"}
	wtg.wkldIf.Delete(ts.tu.NewLoggedInContext(context.Background()), &obj3)

	obj4 := api.ObjectMeta{Name: wkld4Name, Tenant: "default"}
	wtg.wkldIf.Delete(ts.tu.NewLoggedInContext(context.Background()), &obj4)
}
