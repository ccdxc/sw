...
func NewNpmClient(agent netagent.CtrlerIntf, srvURL string) (*NpmClient, error) {
	...
	if err != nil {
		logrus.Errorf("Error connecting to grpc server. Err: %v", err)
		return nil, err
	}
	...
	if err != nil {
		logrus.Fatalf("Error registering the controller interface. Err: %v", err)
		return nil, err
	}
...
func (client *NpmClient) runNetworkWatcher(ctx context.Context) {
	...
	if err != nil {
		logrus.Fatalf("Error watching network")
	}
	...
	for {
		...
		evt, err := stream.Recv()
		if err != nil {
			logrus.Errorf("Error receving from watch channel. Exiting network watch. Err: %v", err)
			return
		}

		logrus.Infof("Ctrlerif: Got Network watch event: {%+v}", evt)
		...
		case api.EventType_CreateEvent:
			...
			if err != nil {
				logrus.Errorf("Error creating the network {%+v}. Err: %v", evt, err)
			}
		case api.EventType_UpdateEvent:
			// update the network
			err = client.agent.UpdateNetwork(&evt.Network)
			if err != nil {
				logrus.Errorf("Error updating the network {%+v}. Err: %v", evt, err)
			}
		case api.EventType_DeleteEvent:
			// delete the network
			err = client.agent.DeleteNetwork(&evt.Network)
			if err != nil {
				logrus.Errorf("Error deleting the network {%+v}. Err: %v", evt, err)
			}
		}
	}
}
...
func (client *NpmClient) runEndpointWatcher(ctx context.Context) {
	...
	if err != nil {
		logrus.Fatalf("Error watching endpoint")
	}
	...
	for {
		...
		evt, err := stream.Recv()
		if err != nil {
			logrus.Errorf("Error receving from watch channel. Exiting endpoint watch. Err: %v", err)
			return
		}

		logrus.Infof("Ctrlerif: Got Endpoint watch event: {%+v}", evt)
		...
		case api.EventType_CreateEvent:
			...
			if !ok {
				...
				if err != nil {
					logrus.Errorf("Error creating the endpoint {%+v}. Err: %v", evt, err)
				}
			}
		case api.EventType_UpdateEvent:
			// create the endpoint
			...
			if err != nil {
				logrus.Errorf("Error updating the endpoint {%+v}. Err: %v", evt, err)
			}
		case api.EventType_DeleteEvent:
			// if we got a watch event for an endpoint thats pending response, ignore it
			...
			if !ok {
				...
				if err != nil {
					logrus.Errorf("Error deleting the endpoint {%+v}. Err: %v", evt, err)
				}
			}
		}
...
func (client *NpmClient) runSecurityGroupWatcher(ctx context.Context) {
	...
	if err != nil {
		logrus.Fatalf("Error watching security group")
	}
	...
	for {
		...
		evt, err := stream.Recv()
		if err != nil {
			logrus.Errorf("Error receving from watch channel. Exiting security group watch. Err: %v", err)
			return
		}

		logrus.Infof("Ctrlerif: Got Security group watch event: {%+v}", evt)
		...
		case api.EventType_CreateEvent:
			...
			if err != nil {
				logrus.Errorf("Error creating the sg {%+v}. Err: %v", evt, err)
			}
		case api.EventType_UpdateEvent:
			// update the sg
			err = client.agent.UpdateSecurityGroup(&evt.SecurityGroup)
			if err != nil {
				logrus.Errorf("Error updating the sg {%+v}. Err: %v", evt, err)
			}
		case api.EventType_DeleteEvent:
			// delete the sg
			err = client.agent.DeleteSecurityGroup(&evt.SecurityGroup)
			if err != nil {
				logrus.Errorf("Error deleting the sg {%+v}. Err: %v", evt, err)
			}
		}
	}
}
...
func (client *NpmClient) EndpointCreateReq(epinfo *netproto.Endpoint) (*netproto.Endpoint, error) {
	...
	if err != nil {
		logrus.Errorf("Error resp from netctrler for ep create {%+v}. Err: %v", epinfo, err)
		return nil, err
	}
...
func (client *NpmClient) EndpointDeleteReq(epinfo *netproto.Endpoint) (*netproto.Endpoint, error) {
	...
	if err != nil {
		logrus.Errorf("Error resp from netctrler for ep delete {%+v}. Err: %v", epinfo, err)
		return nil, err
	}
