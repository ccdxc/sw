...
{
    return ptr ? osal_realloc(ptr, size ? size : 1) : osal_alloc(size ? size : 1);
}
...
yaml_strdup(const yaml_char_t *str)
{
    ...
    return (yaml_char_t *)osal_strdup((char *)str);
}
...
{
    OSAL_ASSERT(parser);     /* Non-NULL parser object expected. */
        ...
        size_t *size_read)
{
    ...
    if (parser->input.string.cur == parser->input.string.end) {
        *size_read = 0;
        return 1;
    }

    if (size > (size_t)(parser->input.string.end
                - parser->input.string.cur)) {
        size = parser->input.string.end - parser->input.string.cur;
    }

    memcpy(buffer, parser->input.string.cur, size);
    parser->input.string.cur += size;
    *size_read = size;
    return 1;
}
...
{
    OSAL_ASSERT(parser); /* Non-NULL parser object expected. */
    OSAL_ASSERT(!parser->read_handler);  /* You can set the source only once. */
    OSAL_ASSERT(input);  /* Non-NULL input string expected. */

    parser->read_handler = yaml_string_read_handler;
    parser->read_handler_data = parser;

    parser->input.string.start = input;
    parser->input.string.cur = input;
...
yaml_parser_set_input_file(yaml_parser_t *parser, FILE *file)
{
    OSAL_ASSERT(parser); /* Non-NULL parser object expected. */
    OSAL_ASSERT(!parser->read_handler);  /* You can set the source only once. */
    OSAL_ASSERT(file);   /* Non-NULL file object expected. */
        ...
        yaml_read_handler_t *handler, void *data)
{
    OSAL_ASSERT(parser); /* Non-NULL parser object expected. */
    OSAL_ASSERT(!parser->read_handler);  /* You can set the source only once. */
    OSAL_ASSERT(handler);    /* Non-NULL read handler expected. */
...
{
    OSAL_ASSERT(parser); /* Non-NULL parser object expected. */
    OSAL_ASSERT(!parser->encoding); /* Encoding is already set or detected. */
...
{
    OSAL_ASSERT(emitter);    /* Non-NULL emitter object expected. */
...
{
    OSAL_ASSERT(emitter);    /* Non-NULL emitter object expected. */
    OSAL_ASSERT(!emitter->write_handler);    /* You can set the output only once. */
    OSAL_ASSERT(output);     /* Non-NULL output string expected. */
        ...
        yaml_write_handler_t *handler, void *data)
{
    OSAL_ASSERT(emitter);    /* Non-NULL emitter object expected. */
    OSAL_ASSERT(!emitter->write_handler);    /* You can set the output only once. */
    OSAL_ASSERT(handler);    /* Non-NULL handler object expected. */
...
{
    OSAL_ASSERT(emitter);    /* Non-NULL emitter object expected. */
    OSAL_ASSERT(!emitter->encoding);     /* You can set encoding only once. */
...
{
    OSAL_ASSERT(token);  /* Non-NULL token object expected. */
        ...
        yaml_encoding_t encoding)
{
    ...
    OSAL_ASSERT(event);  /* Non-NULL event object is expected. */
        ...
        int implicit)
{
    ...
    OSAL_ASSERT(event);          /* Non-NULL event object is expected. */
    OSAL_ASSERT((tag_directives_start && tag_directives_end) ||
    ...
    if (tag_directives_start != tag_directives_end) {
        ...
        for (tag_directive = tag_directives_start;
            ...
            OSAL_ASSERT(tag_directive->handle);
            OSAL_ASSERT(tag_directive->prefix);
...
yaml_document_end_event_initialize(yaml_event_t *event, int implicit)
{
    ...
    OSAL_ASSERT(event);      /* Non-NULL emitter object is expected. */
...
yaml_alias_event_initialize(yaml_event_t *event, yaml_char_t *anchor)
{
    ...
    OSAL_ASSERT(event);      /* Non-NULL event object is expected. */
    OSAL_ASSERT(anchor);     /* Non-NULL anchor is expected. */
        ...
        yaml_scalar_style_t style)
{
    ...
    OSAL_ASSERT(event);      /* Non-NULL event object is expected. */
    OSAL_ASSERT(value);      /* Non-NULL anchor is expected. */
...
yaml_event_delete(yaml_event_t *event)
{
    ...
    OSAL_ASSERT(event);  /* Non-NULL event object expected. */
        ...
        int start_implicit, int end_implicit)
{
    ...
    OSAL_ASSERT(document);       /* Non-NULL document object is expected. */
    OSAL_ASSERT((tag_directives_start && tag_directives_end) ||
            (tag_directives_start == tag_directives_end));
                            /* Valid tag directives are expected. */
    ...
    while (!STACK_EMPTY(&context, document->nodes)) {
        ...
        switch (node.type) {
            ...
            case YAML_MAPPING_NODE:
                ...
                break;
            default:
                OSAL_ASSERT(0);  /* Should not happen. */
        }
    }
        ...
        yaml_scalar_style_t style)
{
    ...
    OSAL_ASSERT(document);   /* Non-NULL document object is expected. */
    OSAL_ASSERT(value);      /* Non-NULL value is expected. */
        ...
        int sequence, int item)
{
    ...
    OSAL_ASSERT(document);       /* Non-NULL document is required. */
    OSAL_ASSERT(sequence > 0
            && document->nodes.start + sequence <= document->nodes.top);
                            /* Valid sequence id is required. */
    OSAL_ASSERT(document->nodes.start[sequence-1].type == YAML_SEQUENCE_NODE);
                            /* A sequence node is required. */
    OSAL_ASSERT(item > 0 && document->nodes.start + item <= document->nodes.top);
        ...
        int mapping, int key, int value)
{
    ...
    OSAL_ASSERT(document);       /* Non-NULL document is required. */
    OSAL_ASSERT(mapping > 0
            && document->nodes.start + mapping <= document->nodes.top);
                            /* Valid mapping id is required. */
    OSAL_ASSERT(document->nodes.start[mapping-1].type == YAML_MAPPING_NODE);
                            /* A mapping node is required. */
    OSAL_ASSERT(key > 0 && document->nodes.start + key <= document->nodes.top);
                            /* Valid key id is required. */
    OSAL_ASSERT(value > 0 && document->nodes.start + value <= document->nodes.top);
