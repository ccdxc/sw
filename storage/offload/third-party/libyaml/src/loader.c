...
yaml_parser_load(yaml_parser_t *parser, yaml_document_t *document)
{
    ...
    OSAL_ASSERT(parser);     /* Non-NULL parser object is expected. */
    ...
    if (!parser->stream_start_produced) {
        ...
        OSAL_ASSERT(event.type == YAML_STREAM_START_EVENT);
...
yaml_parser_load_document(yaml_parser_t *parser, yaml_event_t *first_event)
{
    ...
    OSAL_ASSERT(first_event->type == YAML_DOCUMENT_START_EVENT);
    ...
    if (!yaml_parser_parse(parser, &event)) return 0;
    OSAL_ASSERT(event.type == YAML_DOCUMENT_END_EVENT);
...
yaml_parser_load_node(yaml_parser_t *parser, yaml_event_t *first_event)
{
    switch (first_event->type) {
        ...
        default:
            OSAL_ASSERT(0);  /* Could not happen. */
            return 0;
    }
