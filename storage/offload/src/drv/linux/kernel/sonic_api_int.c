...
{
	OSAL_LOG_NOTICE("%30s: %s", "name", q->name);
	OSAL_LOG_NOTICE("%30s: %d", "descs_inuse",
...
report_ring_resource(uint32_t ring_id)
{
	...
	OSAL_LOG_NOTICE("%30s: %s", "name", sonic_accel_ring_name_get(ring_id));
	OSAL_LOG_NOTICE("%30s: %d", "descs_inuse",
...
report_status_queues(const struct per_core_resource *pcr)
{
	...
	OSAL_LOG_NOTICE("%30s: ", "=== cpdc_seq_status_qs");
	OSAL_LOG_NOTICE("%30s: %u", "num_queues",
			pcr->cpdc_statusq_track.num_queues);
	OSAL_LOG_NOTICE("%30s: %d", "next_free_id",
	...
	OSAL_LOG_NOTICE("%30s: %u", "# of non-zero entries", count);

	OSAL_LOG_NOTICE("%30s: ", "=== crypto_seq_status_qs");
	OSAL_LOG_NOTICE("%30s: %u", "num_queues",
...
sonic_report_pcr_counters(const struct per_core_resource *pcr)
{
	...
	OSAL_LOG_NOTICE("%.*s", 30, "==================================================");
	OSAL_LOG_NOTICE("%30s: ", "=== pcr");
	OSAL_LOG_NOTICE("%30s: %d", "core_id", pcr->core_id);
	OSAL_LOG_NOTICE("%30s: %d", "index", pcr->idx);

	lif = pcr->lif;
	OSAL_LOG_NOTICE("%30s: ", "=== lif");
	OSAL_LOG_NOTICE("%30s: %s", "name", pcr->lif->name);
	OSAL_LOG_NOTICE("%30s: %u", "index", pcr->lif->index);
	OSAL_LOG_NOTICE("%30s: %u", "id", pcr->lif->lif_id);
	OSAL_LOG_NOTICE("%30s: %u", "seq_q_index", pcr->lif->seq_q_index);
	OSAL_LOG_NOTICE("%30s: " PRIu64, "last_eid", pcr->lif->last_eid);
	OSAL_LOG_NOTICE("%30s: %u", "flags", pcr->lif->flags);

	q = &pcr->cp_seq_q;
	OSAL_LOG_NOTICE("%30s: ", "=== cp_seq_q");
	report_queue_resource(q);

	q = &pcr->dc_seq_q;
	OSAL_LOG_NOTICE("%30s: ", "=== dc_seq_q");
	report_queue_resource(q);

	q = &pcr->crypto_enc_seq_q;
	OSAL_LOG_NOTICE("%30s: ", "=== enc_seq_q");
	report_queue_resource(q);

	q = &pcr->crypto_dec_seq_q;
	OSAL_LOG_NOTICE("%30s: ", "=== dec_seq_q");
	report_queue_resource(q);

	report_status_queues(pcr);

	OSAL_LOG_NOTICE("%30s: ", "=== ring");
