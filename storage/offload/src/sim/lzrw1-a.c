...
int lzrw1a_compress_compress  (UBYTE *,UBYTE *,ULONG,UBYTE *,ULONG *);
int lzrw1a_compress_decompress(UBYTE *,UBYTE *,ULONG,UBYTE *,ULONG *);
...
//        break;
    case COMPRESS_ACTION_COMPRESS:
       rc = lzrw1a_compress_compress(wrk_mem,src_adr,src_len,dst_adr,p_dst_len);
       break;
    case COMPRESS_ACTION_DECOMPRESS:
       rc = lzrw1a_compress_decompress(wrk_mem,src_adr,src_len,dst_adr,p_dst_len);
       break;
   }
 return rc;
}
...
LOCAL int lzrw1a_compress_compress
...
{register UBYTE *p_src=p_src_first,*p_dst=p_dst_first;
 ...
 if (*p_dst_len && *p_dst_len < src_len) p_dst_post=p_dst_first+*p_dst_len;
...
LOCAL int lzrw1a_compress_decompress
	(UBYTE *p_wrk_mem,UBYTE *p_src_first,ULONG src_len,UBYTE *p_dst_first,ULONG* p_dst_len)
/* Input  : Specify input block using p_src_first and src_len.          */
...
{register UBYTE *p_src=p_src_first+FLAG_BYTES, *p_dst=p_dst_first;
 UBYTE *p_src_post=p_src_first+src_len,*p_dst_post=p_dst_first+9*src_len;
 UBYTE *p_src_max16=p_src_first+src_len-(16*2);
 register ULONG control=1;
 if (*p_dst_len && *p_dst_len < 9*src_len) p_dst_post=p_dst_first+*p_dst_len;
 if (*p_src_first==FLAG_COPY)
   {fast_copy(p_src_first+FLAG_BYTES,p_dst_first,src_len-FLAG_BYTES);
    *p_dst_len=src_len-FLAG_BYTES; return TRUE;}
    ...
    while (unroll--)
      {if (control&1)
         {register UWORD lenmt; register UBYTE *p;
          ...
          if (p_dst+3 > p_dst_post) return FALSE;
          *p_dst++=*p++; *p_dst++=*p++; *p_dst++=*p++;
          lenmt&=0xF;
          if (p_dst+lenmt > p_dst_post) return FALSE;
          while (lenmt--) *p_dst++=*p++;}
       else
         {if (p_dst+1 > p_dst_post) return FALSE;
          *p_dst++=*p_src++;}
       control>>=1;
      }
   }
