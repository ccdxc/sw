...
static sdk::sdk_ret_t
pds_nat_cfg_get(void *info) {
    pds_nat_port_block_info_t *nat_pb_info = (pds_nat_port_block_info_t *)info;
    pds_nat_port_block_export_t plugin_nat_pb;
    nat_addr_type_t nat_addr_type;
    uint16_t vpc_hw_id;
    sdk::sdk_ret_t vpc_ret;

    nat_addr_type = map_pds_address_type(nat_pb_info->spec.address_type);

    vpc_ret = pds_vpc_cfg_hw_id_get(&nat_pb_info->spec.vpc, &vpc_hw_id);
    if (sdk::SDK_RET_OK != vpc_ret) {
        return vpc_ret;
    }

    nat_port_block_get_stats((const uint8_t *)nat_pb_info->spec.key.id,
                             vpc_hw_id, nat_pb_info->spec.ip_proto,
                             nat_addr_type, &plugin_nat_pb);

    nat_pb_info->stats.in_use_count = plugin_nat_pb.in_use_cnt;
    nat_pb_info->stats.session_count = plugin_nat_pb.session_cnt;
