...
if [ "$server_type" = "tls" ]; then
    $SUDO $GDB $NIC_DIR/../bazel-bin/nic/proxy-e2etest/nic_proxy-e2etest_tls-server $portlist
else
    $SUDO $GDB $NIC_DIR/../bazel-bin/nic/proxy-e2etest/nic_proxy-e2etest_tcp-server $portlist
fi
