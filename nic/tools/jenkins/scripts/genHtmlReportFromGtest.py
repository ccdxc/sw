...
def process(outFname, files):
    ...
    for f in xrange(0, len(files)):
        ...
        # Bharat: Print only the file name not the entire path
        # report += generateElements([(files[f], len(ctr[f]), failures, totalTime, timeStamp)], True)
        report += generateElements([(os.path.basename(files[f]), len(ctr[f]), failures, totalTime, timeStamp)], True)
    ...
    # Bharat: Preventing the display of individual test case generation
    ...
    htmlDocument = htmlDocumentText.format(tableHeader ="", rows = "", report = reportHeader + report)
