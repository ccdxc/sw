...
resp_rx_cqpt_process:
    ...
    sll             PAGE_ADDR_P, k.args.page_seg_offset, CAPRI_LOG_SIZEOF_U64_BITS
    tblrdp          PAGE_ADDR_P, PAGE_ADDR_P, 0, CAPRI_SIZEOF_U64_BITS
    ...
    DMA_PHV2MEM_SETUP(DMA_CMD_BASE, c1, PHV_CQWQE_START, PHV_CQWQE_END, CQWQE_P)
    add             DMA_CMD_INDEX, DMA_CMD_INDEX, 1

    CAPRI_SET_TABLE_I_VALID(TBL_ID, 0)
    seq             c2, k.args.arm, 1 
    bcf             [!c2], cqpt_exit
    EQCB_ADDR_GET(EQCB_ADDR, k.args.eq_id) // BD Slot
    ...
    CAPRI_NEXT_TABLE_I_READ(KEY_P, CAPRI_TABLE_LOCK_DIS, CAPRI_TABLE_SIZE_512_BITS, RAW_TABLE_PC, EQCB_ADDR)
