...
#define K_PREF_BASE_ADDR CAPRI_KEY_FIELD(IN_P, pref_cb_or_base_addr)
...
resp_tx_rqprefetch_process:
    ...
    // first compare RQ PI with RQ proxy CI
    // if equal, all WQE's have already been prefetched
    seq    c1, K_RQ_PINDEX, RQ_PROXY_C_INDEX
    bcf    [c1], prefetch_done
    nop // BD Slot

    bbeq   d.pref_init_done, 1, skip_init
    add    r1, r0, K_PREF_BASE_ADDR // BD Slot

    tblwr  d.prefetch_base_addr, r1
    tblwr  d.pref_buff_index, K_PREF_BUFF_INDEX
    ...
    tblwr       d.pref_init_done, 1

skip_init:
    // if prefetch proxy CI = (prefetch PI + 1) % prefetch_buffer_size, 
    // there is no space in the prefetch buffer
    add    r1, r0, PREF_P_INDEX
    mincr  r1, d.log_num_pref_wqes, 1
    seq    c2, PREF_PROXY_C_INDEX, r1
    ...
    slt    c1, K_RQ_PINDEX, RQ_PROXY_C_INDEX
    sll    RQ_NUM_WQES, 1, d.log_num_pref_wqes
    ...
    // if pref_proxy_cindex <= pref_pindex, handle wrap around
    sle    c1, PREF_PROXY_C_INDEX, PREF_P_INDEX
    bcf    [!c1], no_wrap_around
    sll    PREF_NUM_WQES, 1, d.log_num_pref_wqes // BD Slot
    sub    PREF_NUM_WQES, PREF_NUM_WQES, PREF_P_INDEX
    seq    c2, PREF_PROXY_C_INDEX, 0
    sub.c2 PREF_NUM_WQES, PREF_NUM_WQES, 1

no_wrap_around:
    // pref_num_wqes = prefetch proxy CI - prefetch PI - 1
    sub.!c1 PREF_NUM_WQES, PREF_PROXY_C_INDEX, PREF_P_INDEX
    sub.!c1 PREF_NUM_WQES, PREF_NUM_WQES, 1

    slt    c1, RQ_NUM_WQES, PREF_NUM_WQES
    cmov   TRANSFER_NUM_WQES, c1, RQ_NUM_WQES, PREF_NUM_WQES
    ...
    add         r1, r0, PREF_P_INDEX
    CAPRI_SET_FIELD2(INFO_OUT1_P, pref_pindex_pre, r1)

    tblmincr    RQ_PROXY_C_INDEX, K_LOG_NUM_WQES, TRANSFER_NUM_WQES
    tblmincr    PREF_P_INDEX, d.log_num_pref_wqes, TRANSFER_NUM_WQES

    phvwrpair   CAPRI_PHV_FIELD(INFO_OUT1_P, pref_pindex_post), d.pref_pindex, \
                CAPRI_PHV_FIELD(INFO_OUT1_P, cmd_eop), K_CMD_EOP
    CAPRI_SET_FIELD2(INFO_OUT1_P, transfer_num_wqes, TRANSFER_NUM_WQES)
    CAPRI_NEXT_TABLE2_READ_PC_E(CAPRI_TABLE_LOCK_DIS, CAPRI_TABLE_SIZE_0_BITS, resp_tx_rqpt_mpu_only_process, r3)

prefetch_done:
    //seq         c2, K_CHECK_IN, 1
    //tblwr.c2    d.pref_init_done, 0
