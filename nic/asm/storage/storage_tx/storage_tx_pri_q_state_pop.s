...
storage_tx_pri_q_state_pop_start:
   ...
   // priority queue
   PRI_QUEUE_CAN_POP(d.p_ndx_hi, d.c_ndx_hi, d.hi_running, d.hi_running, 
                     check_med)
...
check_med:
   ...
   // priority queue
   PRI_QUEUE_CAN_POP(d.p_ndx_med, d.c_ndx_med, d.med_running, d.med_running,
                     check_lo)
   SERVICE_PRI_QUEUE(d.w_ndx_med, NVME_BE_PRIORITY_MED)

check_lo:
   // If medium priority queue can be serviced, go with it, else exit
   PRI_QUEUE_CAN_POP(d.p_ndx_lo, d.c_ndx_lo, d.lo_running, d.lo_running,
                     exit)
