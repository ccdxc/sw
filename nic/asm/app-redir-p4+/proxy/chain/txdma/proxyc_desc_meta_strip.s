...
proxyc_s2_desc_meta_strip:
    ...
    addi        aol_p_r, r0, APP_REDIR_BIT_OFFS_D_VEC(A0)   // delay slot
    
    sne         c1, r_total_strip, r0
    bal.c1      r_return, _aol_meta_strip
    addi.c1     aol_p_r, r0, APP_REDIR_BIT_OFFS_D_VEC(A1)   // delay slot
    
    sne         c1, r_total_strip, r0
    bal.c1      r_return, _aol_meta_strip
    addi.c1     aol_p_r, r0, APP_REDIR_BIT_OFFS_D_VEC(A2)   // delay slot
...
_aol_meta_strip:
    ...
    tblrdp      r_offs, aol_p_r, APP_REDIR_BIT_OFFS_STRUCT(descr_aol_single_t, O),  \
                                 APP_REDIR_BIT_SIZE_STRUCT(descr_aol_single_t, O)
    tblrdp      r_len,  aol_p_r, APP_REDIR_BIT_OFFS_STRUCT(descr_aol_single_t, L),  \
                                 APP_REDIR_BIT_SIZE_STRUCT(descr_aol_single_t, L)
    /*
     * Calculate r_cpu_header_addr first
     */
    seq         c1, r_cpu_header_addr, r0
    slt         c2, r_len.wx, CPU_TO_P4PLUS_HEADER_SIZE
    setcf       c1, [c1 & !c2] 
    tblrdp.c1   r_cpu_header_addr, aol_p_r, APP_REDIR_BIT_OFFS_STRUCT(descr_aol_single_t, A), \
                                            APP_REDIR_BIT_SIZE_STRUCT(descr_aol_single_t, A)
    add.c1      r_cpu_header_addr, r_cpu_header_addr.dx, r_offs.wx

    /*
     * Now adjust O and L to remove strip amount
     */    
    slt         c1, r_len.wx, r_total_strip
    cmov        r_curr_strip, c1, r_len.wx, r_total_strip
    add         r_offs, r_offs.wx, r_curr_strip
    tblwrp      aol_p_r, APP_REDIR_BIT_OFFS_STRUCT(descr_aol_single_t, O),          \
                         APP_REDIR_BIT_SIZE_STRUCT(descr_aol_single_t, O), r_offs.wx
    sub         r_len, r_len.wx, r_curr_strip
    tblwrp      aol_p_r, APP_REDIR_BIT_OFFS_STRUCT(descr_aol_single_t, L),          \
                         APP_REDIR_BIT_SIZE_STRUCT(descr_aol_single_t, L), r_len.wx
    jr          r_return
