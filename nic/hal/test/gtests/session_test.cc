...
TEST_F(session_test, test1) 
{
    ...
    L2SegmentSpec               l2seg_spec;
    L2SegmentResponse           l2seg_rsp;
    InterfaceSpec               session_spec;
    InterfaceResponse           session_rsp;
    ...
    // Create session
    session_spec.mutable_meta()->set_tenant_id(1);
    session_spec.set_type(intf::IF_TYPE_ENIC);
    session_spec.mutable_lif_key_or_handle()->set_lif_id(1);
    session_spec.mutable_key_or_handle()->set_interface_id(1);
    session_spec.mutable_if_enic_info()->set_enic_type(intf::IF_ENIC_TYPE_USEG);
    session_spec.mutable_if_enic_info()->mutable_useg_info()->set_l2segment_id(1);
    session_spec.mutable_if_enic_info()->mutable_useg_info()->set_useg_vlan_id(20);

    ret = hal::interface_create(session_spec, &session_rsp);
    ASSERT_TRUE(ret == HAL_RET_OK);
}
#endif
