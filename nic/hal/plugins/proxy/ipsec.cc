...
ipsec_exec3(fte::ctx_t&ctx)
{
    ...
    HAL_TRACE_DEBUG("IPSec flow forwarding role: {} direction: {} key {} is_local_src {} is_local_dest {} src_lif {}", 
        ctx.role(), ctx.direction(), ctx.get_key(), is_local_src, is_local_dest, ctx.cpu_rxhdr()->src_lif);
    if ((ctx.get_key().proto == types::IPPROTO_ESP) && (ctx.cpu_rxhdr()->src_lif == 1003)) {
        HAL_TRACE_DEBUG("ESP flow from CPU Lif - do nothing");
        return  fte::PIPELINE_CONTINUE;
    }
    ...
    if ((ctx.direction() == hal::FLOW_DIR_FROM_UPLINK) && 
        (ctx.role() ==  hal::FLOW_ROLE_INITIATOR) &&
        (ctx.cpu_rxhdr()->src_lif == 1003)) {
        HAL_TRACE_DEBUG("Post Decrypt flow from CPU Lif - do nothing");
        return  fte::PIPELINE_CONTINUE;
    }
    ...
    if (acl_ctx != NULL) {
     ...
     if (rule) {
        ...
        HAL_TRACE_DEBUG("Reverse: IPSec flow forwarding role: {} direction: {} key {} is_local_src {} is_local_dest{} ", 
            ctx.role(), ctx.direction(), ctx.get_key(), is_local_src, is_local_dest);
        ...
        case types::IPPROTO_ICMPV6:
         ...
         case types::IPPROTO_TCP:
         case types::IPPROTO_UDP:
             acl_key.port_src = ctx.get_key().dport;
             acl_key.port_dst = ctx.get_key().sport;
             break;
         default:
             HAL_ASSERT(true);
             ret = HAL_RET_FTE_RULE_NO_MATCH;
