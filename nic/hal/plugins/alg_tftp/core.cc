...
static void
incr_parse_error(l4_alg_status_t *sess)
{
    HAL_ATOMIC_INC_UINT32(&((tftp_info_t *)sess->info)->parse_errors, 1);
}

static void
incr_unknown_opcode(l4_alg_status_t *sess)
{
...
hal_ret_t process_tftp(fte::ctx_t& ctx) {
    ...
    l4_alg_status_t      *exp_flow = (l4_alg_status_t *)ctx.feature_session_state();
...
static void tftp_completion_hdlr (fte::ctx_t& ctx, bool status) {
    ...
    if (!status) {
        ...
        if (l4_sess->isCtrl == TRUE) { /* Control session */
            ...
            HAL_TRACE_DEBUG("Info {:p}", exp_flow->info);
            l4_sess->info = (tftp_info_t *)g_tftp_state->alg_info_slab()->alloc();
            HAL_ASSERT(l4_sess->info != NULL);
            HAL_TRACE_DEBUG("Set Expected flow with key:{} entry {:p}", exp_flow->entry.key, (void *)exp_flow);
        } else { /* Data session */
            /*
...
fte::pipeline_action_t alg_tftp_exec(fte::ctx_t &ctx) {
    ...
    HAL_TRACE_DEBUG("Sfw info alg_proto: {}", sfw_info->alg_proto);
    ...
    } else if (l4_sess != NULL && l4_sess->alg == nwsec::APP_SVC_TFTP) {
        tftp_info = (tftp_info_t *)l4_sess->info;
        HAL_ASSERT(tftp_info);
        
        tftp_info->callback(ctx);
    }
