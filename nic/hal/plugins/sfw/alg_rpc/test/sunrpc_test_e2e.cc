...
void *start_server (void *ptr)
{
    ...
    cmd = prefix_cmd + " rpcbind -h 64.0.0.1 && PYTHONPATH=/sw/nic/hal/plugins/sfw/alg_rpc/test " + \
...
void *start_client (void *ptr)
{
    rpc_test      *test = (rpc_test *)ptr;
    std::string     cmd;
    cmd = "PYTHONPATH=/sw/nic/hal/plugins/sfw/alg_rpc/test " + test->prefix_cmd(test->c_e2e_eph) + \
