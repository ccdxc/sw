...
appid_scan(fte::ctx_t& ctx)
{
    ...
    ret = scanner_run(ctx.appid_info(), ctx.pkt(), ctx.pkt_len(), &ctx.app_redir());
    if (ret != HAL_RET_OK) {
        goto error;
    }
...
{
    // TODO: for now just assume we need more data
    ctx.set_appid_state(APPID_STATE_IN_PROGRESS);

    return appid_scan(ctx);
}
...
//    assert(ctx.appid_state() != APPID_STATE_INIT);
    ...
    return ret == HAL_RET_OK ? fte::PIPELINE_CONTINUE : fte::PIPELINE_END;
}
