...
class learn_mix_fsm_test : public hal_base_test {
   protected:
    learn_mix_fsm_test() {
    }

    virtual ~learn_mix_fsm_test() {}
...
hal_ret_t dpkt_learn_packet_send(hal_handle_t ep_handle,
    ...
    fte_ctx_init(ctx, dummy_ten,
            dummy_ep, dst_ep, &sip, &dip, &cpu_rxhdr, &buffer[0], buffer.size(), true, NULL, NULL, feature_state);
...
TEST_F(learn_mix_fsm_test, dpkt_learn_arp_learn) {
    ...
    arp_packet_send(ep_handles[0], ARP::Flags::REQUEST, "1.1.1.1",
                    GET_MAC_ADDR(0), "1.1.1.2");
    arp_trans_key_t arp_key;
    ip = {0};
    inet_pton(AF_INET, "1.1.1.1", &(ip.addr.v4_addr));
    ip.addr.v4_addr = ntohl(ip.addr.v4_addr);
    dummy_ep = find_ep_by_handle(ep_handles[0]);
    arp_trans_t::init_arp_trans_key(GET_MAC_ADDR(0), dummy_ep, ARP_TRANS_IPV4, &ip, &arp_key);
    arp_trans_t *arp_entry = reinterpret_cast<arp_trans_t *>(
        arp_trans_t::arplearn_key_ht()->lookup(&arp_key));
    ASSERT_TRUE(arp_entry != NULL);
    ...
    arp_trans_t::init_arp_trans_key(GET_MAC_ADDR(1), dummy_ep, ARP_TRANS_IPV4, &ip, &arp_key);
    arp_entry = reinterpret_cast<arp_trans_t *>(
        arp_trans_t::arplearn_key_ht()->lookup(&arp_key));
    ASSERT_TRUE(arp_entry != NULL);
    inet_pton(AF_INET, "1.1.1.2", &(ip.addr.v4_addr));
    ip.addr.v4_addr = ntohl(ip.addr.v4_addr);
...
TEST_F(learn_mix_fsm_test, arp_entry_timeout) {
    ip_addr_t ip = {0};
    inet_pton(AF_INET, "1.1.1.1", &(ip.addr.v4_addr));
    ip.addr.v4_addr = ntohl(ip.addr.v4_addr);
    arp_packet_send(ep_handles[0], ARP::Flags::REQUEST, "1.1.1.1",
    ...
    ASSERT_TRUE(entry == NULL);
    dummy_ep = find_ep_by_handle(ep_handles[0]);
    /* ARP Entry timeout should not removed as Flow miss entry still present. */;
    ASSERT_TRUE(ip_in_ep(&ip, dummy_ep, NULL));
}
...
TEST_F(learn_mix_fsm_test, dpkt_learn_arp_learn_diffrent_ip_other_ep) {
    ...
    ip.addr.v4_addr = ntohl(ip.addr.v4_addr);
    ep_t *dummy_ep = find_ep_by_handle(ep_handles[0]);
    /* Flow miss entry should still be present. */;
    ASSERT_TRUE(ip_in_ep(&ip, dummy_ep, NULL));

    arp_packet_send(ep_handles[1], ARP::Flags::REQUEST, "1.1.1.1",
    ...
    dummy_ep = find_ep_by_handle(ep_handles[0]);
    /* Flow miss entry should be removed as IP moved. */;
    ASSERT_FALSE(ip_in_ep(&ip, dummy_ep, NULL));
...
TEST_F(learn_mix_fsm_test, dpkt_learn_arp_learn_diffrent_ip_same_ep) {
    ...
    arp_packet_send(ep_handles[0], ARP::Flags::REQUEST, "1.1.1.4",
                    GET_MAC_ADDR(0), "1.1.1.5");
    arp_trans_key_t arp_key;
    ip = {0};
    ...
    ASSERT_TRUE(ip_in_ep(&ip, dummy_ep, NULL));
    inet_pton(AF_INET, "1.1.1.5", &(ip.addr.v4_addr));
    ip.addr.v4_addr = ntohl(ip.addr.v4_addr);
    ASSERT_FALSE(ip_in_ep(&ip, dummy_ep, NULL));
    arp_packet_send(ep_handles[1], ARP::Flags::REPLY, "1.1.1.5",
                    GET_MAC_ADDR(1), "1.1.1.4", GET_MAC_ADDR(0));
    dummy_ep = find_ep_by_handle(ep_handles[1]);
    arp_trans_t::init_arp_trans_key(GET_MAC_ADDR(1), dummy_ep, ARP_TRANS_IPV4, &ip, &arp_key);
    arp_entry = reinterpret_cast<arp_trans_t *>(
        arp_trans_t::arplearn_key_ht()->lookup(&arp_key));
