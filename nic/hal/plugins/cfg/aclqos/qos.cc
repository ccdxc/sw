...
//------------------------------------------------------------------------------
hal_ret_t
qosclear_stats (QosClearStatsRequest& req, QosClearStatsResponse *rsp)
{
    hal_ret_t               ret = HAL_RET_OK;
    pd::pd_qos_clear_stats_args_t args = { 0 };
    pd::pd_func_args_t      pd_func_args = { 0 };

    hal_api_trace(" API Begin: qos clear stats ");

    // Call PD to clear the qos stats
    // Get the tm port from uplink ifindex
    uint32_t eth_ifindex = UPLINK_IFINDEX_TO_ETH_IFINDEX(req.port_num());
    args.port_num = ETH_IFINDEX_TO_PARENT_PORT(eth_ifindex);
