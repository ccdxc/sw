...
uint32_t
nvme_ns_compute_hash_func (void *key, uint32_t ht_size)
{
    return sdk::lib::hash_algo::fnv_hash(key, sizeof(nvme_ns_id_t)) % ht_size;
}

bool
nvme_ns_compare_key_func (void *key1, void *key2)
{
    SDK_ASSERT((key1 != NULL) && (key2 != NULL));
    if (*(nvme_ns_id_t *)key1 == *(nvme_ns_id_t *)key2) {
        return true;
    }
    return false;
}
...
uint32_t
nvme_ns_compute_handle_hash_func (void *key, uint32_t ht_size)
{
    return sdk::lib::hash_algo::fnv_hash(key, sizeof(hal_handle_t)) % ht_size;
}

bool
nvme_ns_compare_handle_key_func (void *key1, void *key2)
{
    SDK_ASSERT((key1 != NULL) && (key2 != NULL));
