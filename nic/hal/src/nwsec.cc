...
{   
    ...
    if (spec.has_policy_rules()) {
        fw_sz = spec.policy_rules().fw_rules_size();
        ...
        for (uint32_t i = 0; i < fw_sz; i++) {
           ...
           svcs_sz = spec.policy_rules().fw_rules(i).svc_size();
           ...
           for (uint32_t svcs_cnt = 0; svcs_cnt < svcs_sz; svcs_cnt++) {
                ...
                nwsec_plcy_svc->ipproto = 
                                spec.policy_rules().fw_rules(i).svc(svcs_cnt).ip_protocol();
                if (spec.policy_rules().fw_rules(i).svc(svcs_cnt).l4_info_case()
                                                     == Service::kDstPort){
                    nwsec_plcy_svc->dst_port =
                            spec.policy_rules().fw_rules(i).svc(svcs_cnt).dst_port();

                } else {
                    nwsec_plcy_svc->icmp_msg_type =
                            spec.policy_rules().fw_rules(i).svc(svcs_cnt).icmp_msg_type();
                }
