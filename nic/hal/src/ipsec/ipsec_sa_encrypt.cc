...
ipsec_sa_encrypt_get_key_func (void *entry)
{
    ...
    return (void *)&(((ipsec_sa_encrypt_t *)entry)->cb_id);
}

uint32_t
ipsec_sa_encrypt_compute_hash_func (void *key, uint32_t ht_size)
{
    return sdk::lib::hash_algo::fnv_hash(key, sizeof(ipsec_sa_encrypt_id_t)) % ht_size;
}
...
ipsec_sa_encrypt_compare_key_func (void *key1, void *key2)
{
    ...
    if (*(ipsec_sa_encrypt_id_t *)key1 == *(ipsec_sa_encrypt_id_t *)key2) {
        return true;
    }
    return false;
}
...
{
    HAL_ASSERT(entry != NULL);
    return (void *)&(((ipsec_sa_encrypt_t *)entry)->hal_handle);
}
...
//------------------------------------------------------------------------------
static hal_ret_t
validate_ipsec_sa_encrypt_create (IpsecCbSpec& spec, IpsecCbResponse *rsp)
{
...
add_ipsec_sa_encrypt_to_db (ipsec_sa_encrypt_t *ipseccb)
{
    g_hal_state->ipsec_sa_encrypt_id_ht()->insert(ipseccb, &ipseccb->ht_ctxt);
    return HAL_RET_OK;
}
...
//------------------------------------------------------------------------------
hal_ret_t
ipsec_sa_encrypt_create (IpsecCbSpec& spec, IpsecCbResponse *rsp)
{
    hal_ret_t              ret = HAL_RET_OK;
    ipsec_sa_encrypt_t                *ipseccb;
    pd::pd_ipsec_sa_encrypt_create_args_t    pd_ipsec_sa_encrypt_args;
    ep_t *sep, *dep;
    ...
    ipseccb = ipsec_sa_encrypt_alloc_init();
    if (ipseccb == NULL) {
        rsp->set_api_status(types::API_STATUS_OUT_OF_MEM);
        return HAL_RET_OOM;
    }

    ipseccb->cb_id = spec.key_or_handle().ipsec_sa_encrypt_id();
    ...
    pd::pd_ipsec_sa_encrypt_create_args_init(&pd_ipsec_sa_encrypt_args);
    pd_ipsec_sa_encrypt_args.ipseccb = ipseccb;
    ret = pd::hal_pd_call(pd::PD_FUNC_ID_IPSECCB_CREATE, (void *)&pd_ipsec_sa_encrypt_args);
    if (ret != HAL_RET_OK) {
    ...
    ret = add_ipsec_sa_encrypt_to_db(ipseccb);
    HAL_ASSERT(ret == HAL_RET_OK);

    // prepare the response
    rsp->set_api_status(types::API_STATUS_OK);
    rsp->mutable_ipsec_sa_encrypt_status()->set_ipsec_sa_encrypt_handle(ipseccb->hal_handle);
...
//------------------------------------------------------------------------------
hal_ret_t
ipsec_sa_encrypt_update (IpsecCbSpec& spec, IpsecCbResponse *rsp)
{
    hal_ret_t              ret = HAL_RET_OK;
    ipsec_sa_encrypt_t*               ipseccb;
    pd::pd_ipsec_sa_encrypt_update_args_t    pd_ipsec_sa_encrypt_args;
    ep_t *sep, *dep;
    mac_addr_t *smac = NULL, *dmac = NULL;
    vrf_t   *vrf;
    vrf_id_t tid;
    ...
    ipseccb = find_ipsec_sa_encrypt_by_id(kh.ipsec_sa_encrypt_id());
    if (ipseccb == NULL) {
        rsp->set_api_status(types::API_STATUS_NOT_FOUND);
        return HAL_RET_IPSEC_CB_NOT_FOUND;
    }

    pd::pd_ipsec_sa_encrypt_update_args_init(&pd_ipsec_sa_encrypt_args);
    pd_ipsec_sa_encrypt_args.ipseccb = ipseccb;
    ...
    ret = pd::hal_pd_call(pd::PD_FUNC_ID_IPSECCB_UPDATE, (void *)&pd_ipsec_sa_encrypt_args);
    if(ret != HAL_RET_OK) {
...
//------------------------------------------------------------------------------
hal_ret_t
ipsec_sa_encrypt_get (IpsecCbGetRequest& req, IpsecCbGetResponseMsg *resp)
{
    hal_ret_t              ret = HAL_RET_OK;
    ipsec_sa_encrypt_t                ripseccb;
    ipsec_sa_encrypt_t*               ipseccb;
    pd::pd_ipsec_sa_encrypt_get_args_t    pd_ipsec_sa_encrypt_args;
    ...
    ipsec_sa_encrypt_init(&ripseccb);
    ripseccb.cb_id = ipseccb->cb_id;
    pd::pd_ipsec_sa_encrypt_get_args_init(&pd_ipsec_sa_encrypt_args);
    pd_ipsec_sa_encrypt_args.ipseccb = &ripseccb;

    ret = pd::hal_pd_call(pd::PD_FUNC_ID_IPSECCB_GET, (void *)&pd_ipsec_sa_encrypt_args);
    if(ret != HAL_RET_OK) {
    ...
    rsp->mutable_spec()->mutable_key_or_handle()->set_ipsec_sa_encrypt_id(ripseccb.cb_id);
    ...
    rsp->mutable_status()->set_ipsec_sa_encrypt_handle(ipseccb->hal_handle);
...
//------------------------------------------------------------------------------
hal_ret_t
ipsec_sa_encrypt_delete (ipseccb::IpsecCbDeleteRequest& req, ipseccb::IpsecCbDeleteResponseMsg *rsp)
{
    hal_ret_t              ret = HAL_RET_OK;
    ipsec_sa_encrypt_t*               ipseccb;
    pd::pd_ipsec_sa_encrypt_delete_args_t    pd_ipsec_sa_encrypt_args;
    ...
    pd::pd_ipsec_sa_encrypt_delete_args_init(&pd_ipsec_sa_encrypt_args);
    pd_ipsec_sa_encrypt_args.ipseccb = ipseccb;

    ret = pd::hal_pd_call(pd::PD_FUNC_ID_IPSECCB_DELETE, (void *)&pd_ipsec_sa_encrypt_args);
    if(ret != HAL_RET_OK) {
