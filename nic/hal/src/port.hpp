...
// max. number of ports supported  (TODO: we can take this from cfg file)
...
port_alloc (void)
{
    return (port_t *)g_hal_state->port_slab()->alloc();
}
...
port_free (port_t *pi_p)
{
    ...
    g_hal_state->port_slab()->free(pi_p);
    return HAL_RET_OK;
}
...
find_port_by_id (port_num_t port_num)
{
    ...
    entry = (hal_handle_id_ht_entry_t *)
            g_hal_state->port_id_ht()->lookup(&port_num);
...
#endif    // __NIC_HAL_SRC_PORT_HPP__
