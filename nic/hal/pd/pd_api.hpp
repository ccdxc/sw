...
typedef struct pd_if_inp_prop_pgm_args_s {
    if_t *intf;
} __PACK__ pd_if_inp_prop_pgm_args_t;
...
typedef struct pd_qos_clear_stats_args_s {
    uint32_t       port_num;
    uint8_t        qos_group_bitmap;
} __PACK__ pd_qos_clear_stats_args_t;
...
typedef struct pd_barco_asym_poll_pend_req_args_s {
    uint32_t batch_size;
    uint32_t *id_count;
    uint32_t *ids;
} __PACK__ pd_barco_asym_poll_pend_req_args_t;
...
PD_FUNCP_TYPEDEF(pd_if_inp_prop_pgm);
