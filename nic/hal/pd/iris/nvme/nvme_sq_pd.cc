...
bool
nvme_sq_pd_compare_hw_key_func (void *key1, void *key2)
{
    SDK_ASSERT((key1 != NULL) && (key2 != NULL));
    if (*(nvme_sq_hw_id_t *)key1 == *(nvme_sq_hw_id_t *)key2) {
        return true;
    }
    return false;
}
...
uint32_t
nvme_sq_pd_compute_hw_hash_func (void *key, uint32_t ht_size)
{
    return sdk::lib::hash_algo::fnv_hash(key, sizeof(nvme_sq_hw_id_t)) % ht_size;
}
