...
uint32_t
proxyrcb_pd_compute_hw_hash_func (void *key, uint32_t ht_size)
{
    return hal::utils::hash_algo::fnv_hash(key, sizeof(proxyrcb_hw_id_t)) % ht_size;
}

bool
proxyrcb_pd_compare_hw_key_func (void *key1, void *key2)
{
    HAL_ASSERT((key1 != NULL) && (key2 != NULL));
    if (*(proxyrcb_hw_id_t *)key1 == *(proxyrcb_hw_id_t *)key2) {
        return true;
    }
    return false;
}
                                       ...
                                       bool qstate_header_overwrite)
{
    ...
    if (proxyrcb->my_txq_base) {
         ...
         */
        pd_wring_meta = wring_pd_get_meta(types::WRING_TYPE_APP_REDIR_PROXYR);
        ret = wring_pd_get_base_addr(types::WRING_TYPE_APP_REDIR_PROXYR,
                                     proxyrcb->cb_id, &my_txq_base);
        if((ret == HAL_RET_OK) && pd_wring_meta) {
            HAL_TRACE_DEBUG("PROXYRCB {} my_txq_base: {:#x}",
                            proxyrcb->cb_id, my_txq_base);
            ...
            HAL_TRACE_DEBUG("PROXYRCB my_txq_ring_size_shift: {} "
                            "my_txq_entry_size_shift: {}",
                            data.u.start_d.my_txq_ring_size_shift,
                            data.u.start_d.my_txq_entry_size_shift);
        } else {
            HAL_TRACE_ERR("Failed to receive WRING_TYPE_APP_REDIR_PROXYR");
    ...
    if (proxyrcb->chain_rxq_base) {
        ...
        ret = wring_pd_get_base_addr(types::WRING_TYPE_ARQRX,
                                     proxyrcb->chain_rxq_ring_index_select,
                                     &arq_base);
        if((ret == HAL_RET_OK) && pd_wring_meta) {
            ...
            HAL_TRACE_ERR("Failed to receive ARQRX ring {} info for PROXYRCB",
                          proxyrcb->chain_rxq_ring_index_select);
            ret = HAL_RET_WRING_NOT_FOUND;
        }
    }
...
hal_ret_t
pd_proxyrcb_create (pd_proxyrcb_args_t *args)
{
    hal_ret_t               ret;
 ...
 */
static hal_ret_t
pd_proxyrcb_deactivate (pd_proxyrcb_args_t *args)
{
    hal_ret_t           ret;
    proxyrcb_t          curr_proxyrcb;
    pd_proxyrcb_t       curr_proxyrcb_pd;
    pd_proxyrcb_args_t  curr_args;
    proxyrcb_t*         proxyrcb = args->proxyrcb;

    pd_proxyrcb_args_init(&curr_args);
...
hal_ret_t
pd_proxyrcb_update (pd_proxyrcb_args_t *args)
{
    hal_ret_t       ret;
...
hal_ret_t
pd_proxyrcb_get (pd_proxyrcb_args_t *args)
{
    hal_ret_t               ret;
