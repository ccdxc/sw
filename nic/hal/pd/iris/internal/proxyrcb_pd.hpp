...
{
    g_hal_state_pd->proxyrcb_hwid_ht()->remove(&proxyrcb_pd->hw_ht_ctxt);
    return HAL_RET_OK;
}
...
extern uint32_t proxyrcb_pd_compute_hw_hash_func(void *key, uint32_t ht_size);
extern bool proxyrcb_pd_compare_hw_key_func(void *key1, void *key2);
