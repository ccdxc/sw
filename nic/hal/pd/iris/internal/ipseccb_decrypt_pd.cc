...
p4pd_add_or_del_ipsec_decrypt_rx_stage0_entry(pd_ipseccb_decrypt_t* ipseccb_pd, bool del)
{
    ...
    if(!del) {
        ...
        ipsec_cb_ring_addr = asicpd_get_mem_addr(ASIC_HBM_REG_IPSECCB) + ((ipseccb_pd->ipseccb->cb_id + HAL_MAX_IPSEC_SUPP_SA) * DEFAULT_WRING_SLOT_SIZE * IPSEC_PER_CB_RING_SIZE);
        ...
        ipsec_barco_ring_addr = asicpd_get_mem_addr(ASIC_HBM_REG_IPSECCB_BARCO) + ((ipseccb_pd->ipseccb->cb_id + HAL_MAX_IPSEC_SUPP_SA) * IPSEC_PER_CB_BARCO_SLOT_ELEM_SIZE * IPSEC_PER_CB_BARCO_RING_SIZE);
