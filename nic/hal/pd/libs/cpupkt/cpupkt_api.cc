...
is_phyaddr_cpu_rx_dpr_descr (cpupkt_hw_id_t descr_addr, int tg_cpu_id, pd_descr_aol_t **virt_addr)
{
     ...
     * directly.
     */
    HAL_TRACE_DEBUG("CPU-RX-DPR obj-base-addr {:#x} size {} KB",
...
is_phyaddr_cpu_tx_dr_descr (cpupkt_hw_id_t descr_addr, int tg_cpu_id, pd_descr_aol_t **virt_addr)
{
     ...
     */
    HAL_TRACE_DEBUG("CPU-TX-DR({}) obj-base-addr {:#x} size {} KB", tg_cpu_id,
...
is_virtaddr_cpu_rx_dpr_page (cpupkt_hw_id_t page_addr, int tg_cpu_id, cpupkt_hw_id_t *phy_page_addr)
{
     ...
     * directly.
     */
    HAL_TRACE_DEBUG("CPU-RX-DPR virt-obj-base-addr {:#x} size {} KB",
...
cpupkt_update_slot_addr (cpupkt_qinst_info_t* qinst_info)
{
    ...
    HAL_TRACE_DEBUG("updated pc_index queue: type: {} id: {}, index: {} virt-idx-addr: {:#x} addr: {:#x}: valid_bit_value: {:#x}",
...
pd_cpupkt_free_rx_descr (cpupkt_hw_id_t descr_addr)
{
    ...
    if(ret != HAL_RET_OK) {
        HAL_TRACE_ERR("Failed to get to slot addr for cpu-rx-dpr ci: {}",
                      cpu_rx_dpr_cindex);
        return ret;
    }
	...
	HAL_TRACE_DEBUG("cpu-id: {} update cpudr: slot: {:#x} virt-slot: {:#x} ci: {} descr: {:#x}, value: {#x}",
	...
	if (asic_reg_write(ci_sem_addr, &cpu_rx_dpr_sem_cindex, 1,
			   ASIC_WRITE_MODE_WRITE_THRU) != sdk::SDK_RET_OK) {
            HAL_TRACE_ERR("Failed to program CPU-RX-DPR CI semaphore to {}",
			  cpu_rx_dpr_sem_cindex);
	    return HAL_RET_HW_FAIL;
	}
    }

    HAL_TRACE_DEBUG("Updated CPU-RX-DPR CI semaphore to {}", cpu_rx_dpr_sem_cindex);
                       ...
                       types::WRingType type, uint32_t queue_id)
{
	...
	if (!qinst_info->virt_base_addr) {
            HAL_TRACE_ERR("Failed to mmap the CPU RX Ring(T:{}, Q:{})", type, queue_id);
	    return HAL_RET_NO_RESOURCE;
	}
	else {
            HAL_TRACE_DEBUG("mmap the CPU RX Ring(T:{}, Q:{}) phy {:#x} @ virt {:#x}",
...
pd_cpupkt_poll_receive (pd_func_args_t *pd_func_args)
{
	...
	value = ntohll(value);
        ...
        HAL_TRACE_DEBUG("Rcvd valid data: queue: {}, qid: {} pc_index: {}, "
	...
	if (is_comp_queue) {
	    ...
	    descr_p = &descr;
	    HAL_TRACE_DEBUG("Received packet descriptor {:#x} from {} memory pool",
			    descr_addr, cpu_rx_descr ? "CPU-Rx" : (cpu_tx_descr ? "CPU-Tx" : "RNMDPR"));
	} else {
	    HAL_TRACE_DEBUG("Received packet descriptor {:#x} from {} memory pool, virtual-addr {:#x}",
			    descr_addr, cpu_rx_descr ? "CPU-Rx" : (cpu_tx_descr ? "CPU-Tx" : "RNMDPR"),
	 ...
	 */
	    ...
	    if (cpu_rx_descr) {
	        ...
	        HAL_TRACE_ERR("Packet descriptor from Invalid pool, unable to free {:#x}",
			      (uint64_t) descr_p);
		abort();
	    }
        } else {
...
pd_cpupkt_free_pkt_resources (pd_func_args_t *pd_func_args)
{
	...
	HAL_TRACE_DEBUG("Unknown packet page-addr: {:#x}, page-virt-addr {:#x}", phy_page_addr,
			(uint64_t) pkt);
    }
		      ...
		      cpupkt_hw_id_t phy_page_addr)
{
	...
	*descr_addr = (cpupkt_hw_id_t)(phy_page_addr - CAPRI_CPU_RX_DPR_PAGE_OFFSET);
	HAL_TRACE_DEBUG("programming zero-copy descr: descr_addr: {:#x}, virt-addr {:#x}, offset {:#x}, len {}", *descr_addr,
	...
	descr_p = &descr;
	HAL_TRACE_DEBUG("programming descr: descr_addr: {:#x}, offset {:#x}, len {}", *descr_addr, offset, len);
    }
                           ...
                           uint32_t queue_id, cpupkt_hw_id_t descr_addr, bool no_copy)
{
	...
	HAL_TRACE_DEBUG("Programming zero-copy send queue: addr: {:#x} virt-addr: {:#x} value: {:#x}",
...
pd_cpupkt_send (pd_func_args_t *pd_func_args)
{
    ...
    HAL_TRACE_DEBUG("Doing {} Tx of packet on LIF/Qtype/ID/ring {}/{}/{}/{}",
	...
	HAL_TRACE_DEBUG("Allocated page at address {#x}", page_addr);
	write_addr = page_addr;
    }

    HAL_TRACE_DEBUG("CPU page-addr {:#x}, phy-page-addr {:#x} write-addr {:#x}",
    ...
    if(ret != HAL_RET_OK) {
        ...
        HAL_TRACE_ERR("Failed to program tx descr");
        goto cleanup;
    }
