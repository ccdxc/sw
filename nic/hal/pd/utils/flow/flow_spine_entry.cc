...
FlowSpineEntry::form_action_data(flow_hash_actiondata *action_data)
{
    ...
    if (next_) {
        // TODO : Fix it once hw table changes Populate the next spine
        // next_spe_fhct_idx = next_->get_fhct_index();
    }
}
...
FlowSpineEntry::program_table()
{
    ...
    if (is_in_ft_) {
        ...
        if (anchor_entry_) {
            // Will not enter here as anchor entry exists only in spine entry
            // which is in FT. 
            ...
            // May not come here. anchor entry has to be NULL except for FT
            memcpy(loc, hwkey, hw_key_len);
            ::operator delete(hwkey);
        }
        loc += sw_key_len; // not hwkey_len right?
        memcpy(loc, (&action_data) + 1, /* Skipping actionid */
