...
do {                                                                                        \
    if (pad_size) {                                                                         \
        if (sdk::asic::asic_mem_write(curr_ptr, zero_array, pad_size)) {                    \
            HAL_TRACE_ERR(CAPRI_BARCO_API_NAME                                              \
                    " Failed to write pad for ECC param " #param " into ilist memory @ {:x}", (uint64_t) curr_ptr); \
            ret = HAL_RET_INVALID_ARG;                                                      \
            goto cleanup;                                                                   \
        }                                                                                   \
        curr_ptr += pad_size;                                                               \
    ...
    if (sdk::asic::asic_mem_write(curr_ptr, (uint8_t*)param, key_size)) {                   \
        HAL_TRACE_ERR(CAPRI_BARCO_API_NAME                                                  \
                " Failed to write ECC param " #param " into ilist memory @ {:x}", (uint64_t) curr_ptr); \
        ret = HAL_RET_INVALID_ARG;                                                          \
        goto cleanup;                                                                       \
    }                                                                                       \
    curr_ptr += key_size;                                                                   \
...
#define CAPRI_BARCO_API_NAME "ECC Point Mul: "
    ...
    pd_crypto_asym_alloc_key_args_t args;
    args.key_idx = &ecc_pm_key_idx;
    // ret = pd_crypto_asym_alloc_key(&ecc_pm_key_idx);
    pd_func_args.pd_crypto_asym_alloc_key = &args;
    ret = pd::hal_pd_call(pd::PD_FUNC_ID_CRYPTO_ASYM_ALLOC_KEY, &pd_func_args);
    if (ret != HAL_RET_OK) {
    ...
    HAL_TRACE_DEBUG(CAPRI_BARCO_API_NAME "Allocated Key Descr @ {:x}", ecc_pm_key_idx);
    ...
    pd_crypto_asym_write_key_args_t w_args;
    w_args.key_idx = ecc_pm_key_idx;
    w_args.key = &asym_key;
    pd_func_args.pd_crypto_asym_write_key= &w_args;
    // ret = pd_crypto_asym_write_key(ecc_pm_key_idx, &asym_key);
    ret = pd::hal_pd_call(pd::PD_FUNC_ID_CRYPTO_ASYM_WRITE_KEY, &pd_func_args);
    if (ret != HAL_RET_OK) {
        HAL_TRACE_ERR(CAPRI_BARCO_API_NAME "Failed to write key: {}", ecc_pm_key_idx);
        goto cleanup;
    }
    HAL_TRACE_DEBUG(CAPRI_BARCO_API_NAME "Setup key @ {:x}", ecc_pm_key_idx);


    ret = capri_barco_res_alloc(CRYPTO_BARCO_RES_ASYM_DMA_DESCR,
            NULL, &ilist_dma_descr_addr1);
    if (ret != HAL_RET_OK) {
        HAL_TRACE_ERR(CAPRI_BARCO_API_NAME " Failed to allocate memory for ilist DMA Descr1");
        goto cleanup;
    }
    HAL_TRACE_DEBUG(CAPRI_BARCO_API_NAME " Allocated memory for ilist DMA Descr1 @ {:x}", ilist_dma_descr_addr1);
    if (operand_size > 64) {
        ret = capri_barco_res_alloc(CRYPTO_BARCO_RES_ASYM_DMA_DESCR,
                NULL, &ilist_dma_descr_addr2);
        if (ret != HAL_RET_OK) {
            HAL_TRACE_ERR(CAPRI_BARCO_API_NAME " Failed to allocate memory for ilist DMA Descr2");
            goto cleanup;
        }
        HAL_TRACE_DEBUG(CAPRI_BARCO_API_NAME " Allocated memory for ilist DMA Descr2 @ {:x}", ilist_dma_descr_addr2);
    }
    ...
    if (ret != HAL_RET_OK) {
        HAL_TRACE_ERR(CAPRI_BARCO_API_NAME " Failed to allocate memory for ilist1 content");
        goto cleanup;
    }
    HAL_TRACE_DEBUG(CAPRI_BARCO_API_NAME " Allocated memory for input mem1 @ {:x}", ilist_mem_addr1);

    if (operand_size > 64) {
        ret = capri_barco_res_alloc(CRYPTO_BARCO_RES_HBM_MEM_512B,
                NULL, &ilist_mem_addr2);
        if (ret != HAL_RET_OK) {
            HAL_TRACE_ERR(CAPRI_BARCO_API_NAME " Failed to allocate memory for ilist2 content");
            goto cleanup;
        }
        HAL_TRACE_DEBUG(CAPRI_BARCO_API_NAME " Allocated memory for input mem2 @ {:x}", ilist_mem_addr2);
    }
    ...
    if (operand_size > 64) {
        ...
        if (sdk::asic::asic_mem_write(ilist_dma_descr_addr1, (uint8_t*)&ilist_dma_descr,
                    sizeof(ilist_dma_descr))) {
            HAL_TRACE_ERR(CAPRI_BARCO_API_NAME " Failed to write ilist DMA Descr1 @ {:x}",
    ...
    if (operand_size > 64) {
        ...
        if (sdk::asic::asic_mem_write(ilist_dma_descr_addr2, (uint8_t*)&ilist_dma_descr,
                    sizeof(ilist_dma_descr))) {
            HAL_TRACE_ERR(CAPRI_BARCO_API_NAME " Failed to write ilist DMA Descr2 @ {:x}",
    ...
    /* Poll for completion */
    while (capri_barco_ring_poll(types::BARCO_RING_ASYM, req_tag) != TRUE) {
        //HAL_TRACE_DEBUG(CAPRI_BARCO_API_NAME " Waiting for Barco completion...");
    }
    ...
    if (sdk::asic::asic_mem_read(olist_mem_addr + pad_size, (uint8_t*)x3, key_size)) {
        HAL_TRACE_ERR(CAPRI_BARCO_API_NAME " Failed to read x3 output from memory @ {:x}",
                (uint64_t) olist_mem_addr + pad_size);
        ret = HAL_RET_INVALID_ARG;
        goto cleanup;
    }
    if (sdk::asic::asic_mem_read(olist_mem_addr + operand_size + pad_size, (uint8_t*)y3, key_size)) {
        HAL_TRACE_ERR(CAPRI_BARCO_API_NAME " Failed to read y3 output from memory @ {:x}",
...
cleanup:
    ...
    if (ilist_mem_addr2) {
        ...
        if (ret != HAL_RET_OK) {
            HAL_TRACE_ERR(CAPRI_BARCO_API_NAME " Failed to free memory for ilist2 content:{:x}",
                    ilist_mem_addr2);
        }
    }
    ...
    if (ilist_mem_addr1) {
        ...
        if (ret != HAL_RET_OK) {
            HAL_TRACE_ERR(CAPRI_BARCO_API_NAME " Failed to free memory for ilist1 content:{:x}",
                    ilist_mem_addr1);
        }
    }
    ...
    if (ilist_dma_descr_addr2) {
        ...
        if (ret != HAL_RET_OK) {
            HAL_TRACE_ERR(CAPRI_BARCO_API_NAME " Failed to free memory for ilist DMA Descr2: {:x}",
    ...
    if (ilist_dma_descr_addr1) {
        ...
        if (ret != HAL_RET_OK) {
            HAL_TRACE_ERR(CAPRI_BARCO_API_NAME " Failed to free memory for ilist DMA Descr1: {:x}",
    ...
    if (ecc_pm_key_idx != -1) {
        pd_crypto_asym_free_key_args_t f_args;
        f_args.key_idx = ecc_pm_key_idx;
        // ret = pd_crypto_asym_free_key(ecc_pm_key_idx);
        pd_func_args.pd_crypto_asym_free_key = &f_args;
        ret = pd::hal_pd_call(pd::PD_FUNC_ID_CRYPTO_ASYM_FREE_KEY, &pd_func_args);
        if (ret != HAL_RET_OK) {
