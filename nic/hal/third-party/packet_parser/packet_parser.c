			       ...
			       char **error)
{
    ...
    struct dot1q *dot1q = NULL;
    u16 ether_type = ETHERTYPE_IP;
    ...
    if (ntohs(ether->ether_type) == ETHERTYPE_DOT1Q) {
        /* Found DOT1Q header */
        if (p + sizeof(*dot1q) > packet_end) {
            asprintf(error, "dot1q header overflows packet");
            goto error_out;
        }
        dot1q = (struct dot1q *)p;
        p += sizeof(*dot1q);
        packet->l2_header_bytes += sizeof(*dot1q);
        ether_type = ntohs(dot1q->ether_type);
    }

	return parse_layer3_packet_by_proto(packet, ether_type,
					    p, packet_end, error);
		      ...
		      char **error)
{
	...
	u8 *p = header_start;
	const boolt is_outer = (packet->ip_bytes == 0);
	enum packet_parse_result_t result = PACKET_BAD;
			...
			u8 *packet_end, char **error)
{
    ...
    asprintf(error, "Invalid layer4 proto");
	return PACKET_UNKNOWN_L4;
}
