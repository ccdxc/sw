...
static inline boolt mpls_entry_stack(const struct mpls *mpls)
{
...
{
	return (struct mpls_stack *)(calloc(1, sizeof(struct mpls_stack)));
}
...
{
	if (stack->length >= (int)ARRAY_SIZE(stack->entries))
		return STATUS_ERR;
