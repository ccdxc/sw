...
static inline boolt is_equal_ipv4(struct in_addr a, struct in_addr b)
{
	return a.s_addr == b.s_addr;
}

/* For comparing ports, for consistency with is_equal_ipv4. */
static inline boolt is_equal_port(u16 a, u16 b)
{
	return a == b;
}
...
static inline boolt is_valid_u8(s64 x)
{
	return (x >= 0) && (x <= UCHAR_MAX);
}

static inline boolt is_valid_u16(s64 x)
{
	return (x >= 0) && (x <= USHRT_MAX);
}

static inline boolt is_valid_u32(s64 x)
{
	return (x >= 0) && (x <= UINT_MAX);
}

static inline boolt is_valid_u20(s64 x)
{
