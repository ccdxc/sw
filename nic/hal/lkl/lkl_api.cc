...
bool lkl_handle_flow_miss_pkt(void* skb, hal::flow_direction_t dir, uint32_t iqid, uint32_t rqid, const p4_to_p4plus_cpu_pkt_t *rxhdr, uint16_t hw_vlan_id, proxy_flow_info_t* pfi) {
    ...
    if (rxhdr->lkp_type == FLOW_KEY_LOOKUP_TYPE_IPV4)
        return hal::lklshim_process_flow_miss_rx_packet(skb, dir, iqid, rqid, rxhdr->src_lif, hw_vlan_id, pfi);
    return hal::lklshim_process_v6_flow_miss_rx_packet(skb, dir, iqid, rqid, rxhdr->src_lif, hw_vlan_id, pfi);
}
