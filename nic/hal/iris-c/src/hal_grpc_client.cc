...
shared_ptr<HalClient>
HalGRPCClient::GetInstance(enum ForwardingMode fwd_mode)
{
    if (!instance) {
...
HalClient *
HalGRPCClient::Factory(enum ForwardingMode mode)
{
    if (!instance) {
...
void
HalGRPCClient::Destroy(HalClient *hal_client)
{
    delete hal_client;
...
HalGRPCClient::HalGRPCClient(enum ForwardingMode mode) : HalClient(mode)
{
    std::string svc_url;
...
// HAL_DELETE_API(endpoint, Endpoint);
//-----------------------------------------------------------------------------
// Endpoint Update to HAL
//-----------------------------------------------------------------------------
Status
