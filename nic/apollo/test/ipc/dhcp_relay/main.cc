...
TEST_F(ipc_dhcp_relay_test, ipc_dhcp_relay_cfg_create) {
    ...
    ret_val = pds_dhcp_policy_read(&key, &info);
    EXPECT_EQ(ret_val, sdk::SDK_RET_OK);
    EXPECT_EQ(info.spec.key, int2pdsobjkey(1234));
    EXPECT_EQ(info.spec.relay_spec.vpc, int2pdsobjkey(200));
    EXPECT_EQ(info.spec.relay_spec.server_ip.af, IP_AF_IPV4);
    EXPECT_EQ(info.spec.relay_spec.agent_ip.af, IP_AF_IPV4);
    EXPECT_EQ(info.spec.relay_spec.server_ip.addr.v4_addr,
              spec.relay_spec.server_ip.addr.v4_addr);
    EXPECT_EQ(info.spec.relay_spec.agent_ip.addr.v4_addr,
              spec.relay_spec.agent_ip.addr.v4_addr);

    // now delete the entry, it should succeed
    key = int2pdsobjkey(1234);
    ret_val = pds_dhcp_policy_delete(&key);
    EXPECT_EQ(ret_val, sdk::SDK_RET_OK);

    // now try to read the entry, it should fail
    key = int2pdsobjkey(1234);
    ret_val = pds_dhcp_policy_read(&key, &info);
    EXPECT_EQ(ret_val, sdk::SDK_RET_ENTRY_NOT_FOUND);
}
