...
table_constants_init (void)
{
    ...
    asicpd_program_table_constant(P4TBL_ID_METER_STATS, stats_base_addr);

    stats_base_addr = asicpd_get_mem_addr(MEM_REGION_SESSION_STATS_BASE);
    stats_base_addr -= ((uint64_t)1 << 31);
    asicpd_program_table_constant(P4TBL_ID_SESSION, stats_base_addr);

    asicpd_program_table_constant(P4TBL_ID_EGRESS_VNIC_INFO, g_device_mac);
}
