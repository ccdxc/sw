...
static uint16_t    g_udp_vnic_id = VNIC_ID_L2_UDPSPORT;
static uint32_t    g_h2s_udp_vlan = VLAN_ID_L2_UDPSPORT;
 ...
 * L2 ep mac address
 */
static mac_addr_t  ep_smac = {0x00, 0x00, 0xF1, 0xD0, 0xD1, 0xD1};
static mac_addr_t  ep_dmac = {0x00, 0x00, 0x00, 0x40, 0x08, 0x02};
...
static uint16_t    substrate_udp_srcport = 0xdcba;

static uint32_t    vni = 0xaabbcc;
static uint32_t    source_slot_id = 0x112233;
static uint32_t    destination_slot_id = GENEVE_DST_SLOT_ID_UDPSPORT;
...
static sdk_ret_t
setup_l2_flow_v4_udp_udpsrcport(void)
{
    ...
    // Setup VNIC Mappings
    ret = vlan_to_vnic_map(g_h2s_udp_vlan, g_udp_vnic_id, vnic_type);
    if (ret != SDK_RET_OK) {
        return ret;
    }

    // Setup VNIC Mappings
    ret = mpls_label_to_vnic_map(destination_slot_id, g_udp_vnic_id, vnic_type);
    if (ret != SDK_RET_OK) {
        return ret;
    }
    ...
    ret = create_s2h_session_rewrite_insert_ctag(s2h_session_rewrite_id,  g_h2s_udp_vlan);
    if (ret != SDK_RET_OK) {
        return ret;
    }
	...
	sg_id3, sg_id4,
	sg_id5, sg_id6,
	originator_physical_ip, substrate_udp_srcport);
    if (ret != SDK_RET_OK) {
        return ret;
    }
    ...
    ret = create_flow_v6_tcp_udp (g_udp_vnic_id, &g_h2s_sipv6, &g_h2s_dipv6,
            g_h2s_proto, g_h2s_sport, g_h2s_dport,
            PDS_FLOW_SPEC_INDEX_SESSION, g_session_index);
    g_session_index++;
    if (ret != SDK_RET_OK) {
    ...
    ret = create_l2_flow (g_udp_vnic_id, ep_dmac, s2h_session_rewrite_encap_id);
    if (ret != SDK_RET_OK) {
        return ret;
    }

    ret = create_l2_flow (g_udp_vnic_id, ep_smac, h2s_session_rewrite_encap_id);
    if (ret != SDK_RET_OK) {
        return ret;
    }
...
athena_gtest_setup_l2_flows_udp_udpsrcport(void)
{
    ...
    ret = setup_l2_flow_v4_udp_udpsrcport();
...
static uint8_t g_snd_pkt_ipv4_udp_h2s[] = {
    ...
    0x00, 0x40, 0x08, 0x02, 0x81, 0x00, 0x00, 0x0D,
...
static uint8_t g_rcv_pkt_ipv4_udp_s2h[] {
    ...
    0xf1, 0xd0, 0xd1, 0xd1, 0x81, 0x00, 0x00, 0x0d,
    0x08, 0x00, 0x45, 0x00, 0x00, 0x50, 0x00, 0x01,
...
athena_gtest_test_l2_flows_udp_udpsrcport(void)
{
    ...
    ret = send_packet("L2-UDP-IPv4-UDP-SrcPort: h2s pkt", g_snd_pkt_ipv4_udp_h2s,
    ...
    ret = send_packet("L2-UDP-IPv4: s2h pkt", g_snd_pkt_ipv4_udp_s2h,
