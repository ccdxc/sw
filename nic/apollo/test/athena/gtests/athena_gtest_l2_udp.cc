...
{
    pds_l2_flow_spec_t             spec;


    spec.key.vnic_id = vnic_id;
    //   memcpy(spec.key.dmac, dmac, ETH_ADDR_LEN);
    sdk::lib::memrev(spec.key.dmac, dmac, ETH_ADDR_LEN);
    spec.data.index = index;

    return (sdk_ret_t)pds_l2_flow_cache_entry_create(&spec);
}


static uint16_t    g_udp_vnic_id = VNIC_ID_L2_UDP;
static uint32_t    g_h2s_udp_vlan = VLAN_ID_L2_UDP;
...
static uint32_t    destination_slot_id = GENEVE_DST_SLOT_ID_UDP;
...
static uint8_t g_snd_pkt_ipv4_udp_h2s[] = {
    ...
    0x00, 0x40, 0x08, 0x02, 0x81, 0x00, 0x00, 0x06,
...
static uint8_t g_rcv_pkt_ipv4_udp_h2s[] = {
    ...
    0x22, 0x01, 0x00, 0x01, 0x23, 0x4A, 0x00, 0x00,
...
static uint8_t g_rcv_pkt_ipv4_udp_h2s[] = {
    ...
    0x02, 0x01, 0x01, 0x02, 0x03, 0x04, 0xa3, 0xe5,
    0x19, 0xeb, 0x00, 0x60, 0x00, 0x00, 0x12, 0x34,
    0x50, 0x40, 0x67, 0x89, 0xa1, 0x40, 0x45, 0x00,
...
/*
 * Host to Switch UDP flow bad IP Checksum: Packet to be sent
 */
static uint8_t g_snd_pkt_ipv4_udp_bad_ip_csum_h2s[] = {
...
/*
 * Host to Switch UDP flow bad UDP Checksum: Packet to be sent
 */
static uint8_t g_snd_pkt_ipv4_udp_bad_udp_csum_h2s[] = {
    0x00, 0x00, 0xF1, 0xD0, 0xD1, 0xD0, 0x00, 0x00,
    0x00, 0x40, 0x08, 0x01, 0x81, 0x00, 0x00, 0x01,
    0x08, 0x00, 0x45, 0x00, 0x00, 0x50, 0x00, 0x01,
    0x00, 0x00, 0x40, 0x11, 0xB6, 0x9A, 0x02, 0x00,
    0x00, 0x01, 0xC0, 0x00, 0x02, 0x01, 0x03, 0xE8,
    0x27, 0x10, 0x00, 0x3C, 0xF3, 0x45, 0x61, 0x62,
    0x63, 0x64, 0x65, 0x66, 0x67, 0x68, 0x69, 0x6A,
    0x6C, 0x6B, 0x6D, 0x6E, 0x6F, 0x70, 0x71, 0x72,
    0x73, 0x74, 0x75, 0x76, 0x77, 0x7A, 0x78, 0x79,
    0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68,
...
static uint8_t g_rcv_pkt_ipv4_udp_s2h[] {
    ...
    0xf1, 0xd0, 0xd1, 0xd1, 0x81, 0x00, 0x00, 0x06,
    0x08, 0x00, 0x45, 0x00, 0x00, 0x50, 0x00, 0x01,
...
/*
 * Switch to Host Bad substrate IP checksum: Packet to be sent
 */
static uint8_t g_snd_pkt_ipv4_udp_bad_substrate_ip_csum_s2h[] = {
    0x00, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x00, 0x01,
    0x02, 0x03, 0x04, 0x05, 0x81, 0x00, 0x00, 0x02,
    0x08, 0x00, 0x45, 0x00, 0x00, 0x92, 0x00, 0x00,
    0x00, 0x00, 0x40, 0x11, 0x70, 0x53, 0x04, 0x03,
    0x02, 0x01, 0x01, 0x02, 0x03, 0x04, 0x00, 0x00,
    0x17, 0xc1, 0x00, 0x7e, 0x00, 0x00, 0x04, 0x00,
    0x65, 0x58, 0xAA, 0xBB, 0xCC, 0x00, 0x00, 0x00,
    0x21, 0x01, 0x00, 0x11, 0x22, 0x33, 0x00, 0x00,
...
/*
 * Switch to Host Bad user IP checksum: Packet to be sent
 */
static uint8_t g_snd_pkt_ipv4_udp_bad_user_ip_csum_s2h[] = {
    0x00, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x00, 0x01,
    0x02, 0x03, 0x04, 0x05, 0x81, 0x00, 0x00, 0x02,
    0x08, 0x00, 0x45, 0x00, 0x00, 0x92, 0x00, 0x00,
    0x00, 0x00, 0x40, 0x11, 0x70, 0x52, 0x04, 0x03,
    ...
    0x00, 0x40, 0x08, 0x02, 0x00, 0x00, 0xF1, 0xD0,
    0xD1, 0xD1, 0x08, 0x00, 0x45, 0x00, 0x00, 0x50,
    0x00, 0x01, 0x00, 0x00, 0x40, 0x11, 0xb6, 0x9b, 
    0xC0, 0x00, 0x02, 0x01, 0x02, 0x00, 0x00, 0x01, 
    0x27, 0x10, 0x03, 0xE8, 0x00, 0x3c, 0xf3, 0x44, 
    0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68, 
    0x69, 0x6a, 0x6c, 0x6b, 0x6d, 0x6e, 0x6f, 0x70, 
...
/*
 * Switch to Host Bad user UDP checksum: Packet to be sent
 */
static uint8_t g_snd_pkt_ipv4_udp_bad_user_udp_csum_s2h[] = {
    0x00, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x00, 0x01,
    0x02, 0x03, 0x04, 0x05, 0x81, 0x00, 0x00, 0x02,
    0x08, 0x00, 0x45, 0x00, 0x00, 0x92, 0x00, 0x00,
    0x00, 0x00, 0x40, 0x11, 0x70, 0x52, 0x04, 0x03,
    ...
    0x00, 0x01, 0x00, 0x00, 0x40, 0x11, 0xb6, 0x9a, 
    0xC0, 0x00, 0x02, 0x01, 0x02, 0x00, 0x00, 0x01, 
    0x27, 0x10, 0x03, 0xE8, 0x00, 0x3c, 0xf3, 0x45, 
    0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68, 
    0x69, 0x6a, 0x6c, 0x6b, 0x6d, 0x6e, 0x6f, 0x70, 
    0x71, 0x72, 0x73, 0x74, 0x75, 0x76, 0x77, 0x7a, 
    0x78, 0x79, 0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 
...
static uint8_t g_rcv_pkt_ipv6_udp_h2s[] = {
    ...
    0x22, 0x01, 0x00, 0x01, 0x23, 0x4A, 0x00, 0x00,
    0xF1, 0xD0, 0xD1, 0xD1, 0x00, 0x00, 0x00, 0x40,
    0x08, 0x02, 0x86, 0xDD, 0x60, 0x00, 0x00, 0x00, 
    0x00, 0x3C, 0x11, 0x40, 0x02, 0x00, 0x00, 0x00, 
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
    0x00, 0x00, 0x00, 0x01, 0x0C, 0x00, 0x00, 0x00, 
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
    0x00, 0x00, 0x02, 0x01, 0x03, 0xE8, 0x27, 0x10, 
    0x00, 0x3C, 0xA7, 0x45, 0x61, 0x62, 0x63, 0x64, 
    0x65, 0x66, 0x67, 0x68, 0x69, 0x6A, 0x6C, 0x6B, 
    0x6D, 0x6E, 0x6F, 0x70, 0x71, 0x72, 0x73, 0x74, 
...
static uint8_t g_rcv_pkt_ipv6_udp_h2s[] = {
    ...
    0x17, 0xc1, 0x00, 0x92, 0x00, 0x00, 0x12, 0x34,
...
static uint8_t g_snd_pkt_ipv6_udp_s2h[] = {
    ...
    0x22, 0x01, 0x00, 0x01, 0x23, 0x4A, 0x00, 0x00,
    0x00, 0x40, 0x08, 0x02, 0x00, 0x00, 0xF1, 0xD0,
    0xD1, 0xD1, 0x86, 0xdd, 0x60, 0x00, 0x00, 0x00, 
...
athena_gtest_test_l2_flows_udp(void)
{
...
#else
    ret = send_packet("UDP-L2-IPv4: h2s pkt", g_snd_pkt_ipv4_udp_h2s,
            sizeof(g_snd_pkt_ipv4_udp_h2s), g_h_port,
    ...
    ret = send_packet("UDP-L2-IPv4-Bad-IP-CSUM: h2s pkt", g_snd_pkt_ipv4_udp_bad_ip_csum_h2s,
            sizeof(g_snd_pkt_ipv4_udp_bad_ip_csum_h2s), g_h_port,
            NULL, 0, 0);
    if (ret != SDK_RET_OK) {
        return ret;
    }

    ret = send_packet("UDP-L2-IPv4-Bad-UDP-CSUM: h2s pkt", g_snd_pkt_ipv4_udp_bad_udp_csum_h2s,
            sizeof(g_snd_pkt_ipv4_udp_bad_udp_csum_h2s), g_h_port,
            NULL, 0, 0);
    if (ret != SDK_RET_OK) {
        return ret;
    }
    ...
    ret = send_packet("UDP-L2-IPv4-Bad-Substrate-IP-CSUM: s2h pkt",
            g_snd_pkt_ipv4_udp_bad_substrate_ip_csum_s2h,
            sizeof(g_snd_pkt_ipv4_udp_bad_substrate_ip_csum_s2h), g_s_port,
            NULL, 0, 0);
    if (ret != SDK_RET_OK) {
        return ret;
    }

    ret = send_packet("UDP-L2-IPv4-Bad-User-IP-CSUM: s2h pkt",
            g_snd_pkt_ipv4_udp_bad_user_ip_csum_s2h,
            sizeof(g_snd_pkt_ipv4_udp_bad_user_ip_csum_s2h), g_s_port,
            NULL, 0, 0);
    if (ret != SDK_RET_OK) {
        return ret;
    }

    ret = send_packet("UDP-L2-IPv4-Bad-User-UDP-CSUM: s2h pkt",
            g_snd_pkt_ipv4_udp_bad_user_udp_csum_s2h,
            sizeof(g_snd_pkt_ipv4_udp_bad_user_udp_csum_s2h), g_s_port,
            NULL, 0, 0);
    if (ret != SDK_RET_OK) {
        return ret;
    }
...
#ifndef P4_14
    ret = send_packet_wmask("UDP-L2-IPv6: h2s pkt", g_snd_pkt_ipv6_udp_h2s,
            sizeof(g_snd_pkt_ipv6_udp_h2s), g_h_port,
			    g_rcv_pkt_ipv6_udp_h2s, sizeof(g_rcv_pkt_ipv6_udp_h2s), g_s_port, mask_start_pos);
#else
    ret = send_packet("UDP-L2-IPv6: h2s pkt", g_snd_pkt_ipv6_udp_h2s,
            sizeof(g_snd_pkt_ipv6_udp_h2s), g_h_port,
    ...
    ret = send_packet("UDP-L2-IPv6: s2h pkt", g_snd_pkt_ipv6_udp_s2h,
