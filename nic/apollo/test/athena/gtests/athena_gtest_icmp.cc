...
static uint16_t    g_h2s_icmp_type = 0x08;
static uint16_t    g_h2s_icmp_code = 0x0;
...
athena_gtest_test_flows_icmp(void)
{
    ...
    ret = send_packet("ICMP-IPv4: h2s pkt", g_snd_pkt_h2s, sizeof(g_snd_pkt_h2s), g_h_port,
                       g_rcv_pkt_h2s, sizeof(g_rcv_pkt_h2s), g_s_port);
    if (ret != SDK_RET_OK) {
        return ret;
    }

    ret = send_packet("ICMP-IPv4: s2h pkt", g_snd_pkt_s2h, sizeof(g_snd_pkt_s2h), g_s_port,
