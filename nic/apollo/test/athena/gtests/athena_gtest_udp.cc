...
static uint16_t    g_udp_vnic_id = 0x0001;
static uint32_t    g_h2s_udp_vlan = 0x0001;
...
static uint32_t    mpls2_label = 0x6789a;
...
static uint8_t g_rcv_pkt_h2s[] = {
    ...
    0x08, 0x00, 0x45, 0x00, 0x00, 0x86, 0x00, 0x00,
    0x00, 0x00, 0x40, 0x11, 0x70, 0x5e, 0x04, 0x03,
    0x02, 0x01, 0x01, 0x02, 0x03, 0x04, 0x35, 0xb1,
    0x19, 0xeb, 0x00, 0x72, 0x00, 0x00, 0x12, 0x34,
...
athena_gtest_test_flows_udp(void)
{
    ...
    ret = send_packet("UDP-IPv4: h2s pkt", g_snd_pkt_h2s, sizeof(g_snd_pkt_h2s), g_h_port,
                       g_rcv_pkt_h2s, sizeof(g_rcv_pkt_h2s), g_s_port);
    if (ret != SDK_RET_OK) {
        return ret;
    }

    ret = send_packet("UDP-IPv4: s2h pkt", g_snd_pkt_s2h, sizeof(g_snd_pkt_s2h), g_s_port,
