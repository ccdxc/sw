...
TEST_F(dnat_test, dnat_crud) {
    ...
    SDK_ASSERT(pds_dnat_map_entry_create(&spec) == SDK_RET_OK);

    memcpy(key.addr, &v4_addr_1, IP4_ADDR8_LEN);
    key.key_type = IP_AF_IPV4;
    key.vnic_id = 2;
    SDK_ASSERT(pds_dnat_map_entry_read(&key, &info) == SDK_RET_OK);
    ...
    SDK_ASSERT(pds_dnat_map_entry_create(NULL) == SDK_RET_INVALID_ARG);
    ...
    memcpy(spec.data.addr, &v4_addr_2, IP4_ADDR8_LEN);
    SDK_ASSERT(pds_dnat_map_entry_create(&spec) == SDK_RET_INVALID_ARG);
    ...
    key.key_type = IP_AF_IPV4;
    key.vnic_id = 2;
    SDK_ASSERT(pds_dnat_map_entry_delete(&key) == SDK_RET_OK);

    memset(&info, 0, sizeof(info));
    SDK_ASSERT(pds_dnat_map_entry_read(&key, &info) == SDK_RET_ENTRY_NOT_FOUND);
    ...
    memcpy(spec.data.addr, &v6_addr_1, IP6_ADDR8_LEN);
    spec.data.addr_type = IP_AF_IPV6;
    SDK_ASSERT(pds_dnat_map_entry_update(&spec) == SDK_RET_OK);
