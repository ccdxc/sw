...
extern "C" {
// Function prototypes
pds_ret_t pds_l2_flow_cache_create(void);
...
//----------------------------------------------------------------------------
// L2_FLOW CACHE SCALE test class
//----------------------------------------------------------------------------

class l2_flow_cache_scale: public ftl_scale_base {
...
public:
    ...
    pds_ret_t create_helper(uint32_t index, pds_ret_t expret) {
        ...
        auto rs = (pds_ret_t)pds_l2_flow_cache_entry_create(&spec);
    ...
    pds_ret_t delete_helper(uint32_t index, pds_ret_t expret) {
        ...
        fill_key(index, &key);
        auto rs = (pds_ret_t)pds_l2_flow_cache_entry_delete(&key);
        MHTEST_CHECK_RETURN(rs == expret, PDS_RET_MAX);
        if (rs == PDS_RET_OK) {
            delete_();
            table_count--;
    ...
    pds_ret_t update_helper(uint32_t index, pds_ret_t expret) {
        ...
        fill_key(index, &spec.key);
        fill_data(index + 10,  &spec.data);
        auto rs = (pds_ret_t)pds_l2_flow_cache_entry_update(&spec);
        MHTEST_CHECK_RETURN(rs == expret, PDS_RET_MAX);
        if (rs == PDS_RET_OK) {
            update_();
        }
    ...
    pds_ret_t read_helper(uint32_t index, pds_ret_t expret) {
        ...
        fill_key(index, &key);
        auto rs = (pds_ret_t)pds_l2_flow_cache_entry_read(&key, &info);;
        MHTEST_CHECK_RETURN(rs == expret, PDS_RET_MAX);
        if (rs == PDS_RET_OK) {
            read_();
        }
	//ALE TODO
        if ((info.spec.data.index == index + 10))
            return PDS_RET_OK;
        else if (
...
//----------------------------------------------------------------------------
// L2_Flow cache scale test cases implementation
//----------------------------------------------------------------------------

/// \defgroup L2_FLOW CACHE SCALE L2_Flow cache scale tests
/// @{

/// \brief L2_Flow cache scale tests
TEST_F(l2_flow_cache_scale, read16) {
    pds_ret_t rs;
...
TEST_F(l2_flow_cache_scale, cud_16) {
    pds_ret_t rs;
    rs = create_entries(16, PDS_RET_OK);
    ASSERT_TRUE(rs == PDS_RET_OK);
...
TEST_F(l2_flow_cache_scale, cud_1K) {
    pds_ret_t rs;
...
TEST_F(l2_flow_cache_scale, crurd_1K) {
    pds_ret_t rs;
    rs = create_entries(1024, PDS_RET_OK);
    ASSERT_TRUE(rs == PDS_RET_OK);
...
TEST_F(l2_flow_cache_scale, cud_16K) {
    pds_ret_t rs;
...
TEST_F(l2_flow_cache_scale, delete256K) {
    pds_ret_t rs;
...
TEST_F(l2_flow_cache_scale, create512K) {
    pds_ret_t rs;
    rs = create_entries(512*1024, PDS_RET_OK);
    ASSERT_TRUE(rs == PDS_RET_OK);
}

TEST_F(l2_flow_cache_scale, delete512K) {
    pds_ret_t rs;
    rs = create_entries(512*1024, PDS_RET_OK);
    ASSERT_TRUE(rs == PDS_RET_OK);
...
#if 0
TEST_F(l2_flow_cache_scale, create1M) {
    pds_ret_t rs;
    rs = create_entries(1024*1024, PDS_RET_OK);
    ASSERT_TRUE(rs == PDS_RET_OK);
}

TEST_F(l2_flow_cache_scale, delete1M) {
    pds_ret_t rs;
    rs = create_entries(1024*1024, PDS_RET_OK);
    ASSERT_TRUE(rs == PDS_RET_OK);
...
TEST_F(l2_flow_cache_scale, DISABLED_create2M) {
    pds_ret_t rs;
    rs = create_entries(2*1024*1024, PDS_RET_OK);
    ASSERT_TRUE(rs == PDS_RET_OK);
}

TEST_F(l2_flow_cache_scale, DISABLED_delete2M) {
    pds_ret_t rs;
    rs = create_entries(2*1024*1024, PDS_RET_OK);
    ASSERT_TRUE(rs == PDS_RET_OK);
...
TEST_F(l2_flow_cache_scale, DISABLED_create4M) {
    pds_ret_t rs;
    rs = create_entries(4*1024*1024, PDS_RET_OK);
    ASSERT_TRUE(rs == PDS_RET_OK);
}

TEST_F(l2_flow_cache_scale, DISABLED_delete4M) {
    pds_ret_t rs;
    rs = create_entries(4*1024*1024, PDS_RET_OK);
    ASSERT_TRUE(rs == PDS_RET_OK);
