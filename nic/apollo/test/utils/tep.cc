...
tep_util::tep_util(std::string ip_str, oci_encap_type_t type) {
    this->ip_str = ip_str;
    this->type = type;
}
...
tep_util::create() {
    ...
    oci_tep.type = this->type;
    oci_tep.key.ip_addr = ip_pfx.addr.addr.v4_addr;
    return (oci_tep_create(&oci_tep));
}

sdk::sdk_ret_t
tep_util::many_create(uint32_t num_tep, std::string pfxstr,
                      oci_encap_type_t type) {
    sdk::sdk_ret_t rv = sdk::SDK_RET_OK;
    ip_prefix_t ip_pfx;
    SDK_ASSERT(num_tep <= OCI_MAX_TEP);
...
static inline void
debug_dump_tep_spec (oci_tep_spec_t *spec)
{
    printf("Spec data : \n");
...
static inline void
debug_dump_tep_status (oci_tep_status_t *status)
{
    printf("Status data : \n");
...
static inline void
debug_dump_tep_stats (oci_tep_stats_t *stats)
{
    printf("Stats data : \n");
    printf("\n");
    return;
...
static inline void
debug_dump_tep_info (oci_tep_info_t *info)
{
...
sdk::sdk_ret_t
tep_util::validate(oci_tep_info_t *info) {
    if (info == NULL)
...
sdk::sdk_ret_t
tep_util::read(oci_tep_info_t *info) {
    oci_tep_key_t key;
    ip_prefix_t ip_pfx;

    SDK_ASSERT(str2ipv4pfx((char *)this->ip_str.c_str(), &ip_pfx) == 0);
    memset(&key, 0, sizeof(oci_tep_key_t));
    memset(info, 0, sizeof(oci_tep_info_t));
    key.ip_addr = ip_pfx.addr.addr.v4_addr;
    return (oci_tep_read(&key, info));
}

sdk::sdk_ret_t
tep_util::del() {
    oci_tep_key_t oci_tep_key;
    ip_prefix_t ip_pfx;

    SDK_ASSERT(str2ipv4pfx((char *)this->ip_str.c_str(), &ip_pfx) == 0);
    memset(&oci_tep_key, 0, sizeof(oci_tep_key));
    oci_tep_key.ip_addr = ip_pfx.addr.addr.v4_addr;
    return (oci_tep_delete(&oci_tep_key));
}
