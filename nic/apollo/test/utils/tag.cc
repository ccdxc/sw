...
tag_feeder::init(uint32_t base_tag_table_id,
    ...
    extract_ip_pfx(base_tag_pfx_str_v4.c_str(), &this->base_route_pfx_v4);
    extract_ip_pfx(base_tag_pfx_str_v6.c_str(), &this->base_route_pfx_v6);
...
bool
tag_feeder::key_compare(pds_tag_key_t *key) {
    return true;
}

bool
tag_feeder::spec_compare(pds_tag_spec_t *spec) {
    return true;
}
}    // namespace api_test
