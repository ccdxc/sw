...
TEST_F(nh_test, nh_workflow_b1) {
    ...
    feeder.init("", 0x0E0D0A0B0100, 1, int2pdsobjkey(1), PDS_NH_TYPE_UNDERLAY);
...
TEST_F(nh_test, DISABLED_nh_workflow_b2) {
    ...
    feeder1.init("", 0x0E0D0A0B0100, 1, int2pdsobjkey(1), PDS_NH_TYPE_UNDERLAY);
    feeder1A.init("", 0x0E0D0A0B0200, 1, int2pdsobjkey(1),
...
TEST_F(nh_test, nh_workflow_1) {
    ...
    if (apulu()) {
        feeder.init("", 0x0E0D0A0B0200, k_max_nh, int2pdsobjkey(1),
                    PDS_NH_TYPE_UNDERLAY);
    } else {
...
TEST_F(nh_test, nh_workflow_3) {
    ...
    if (apulu()) {
        feeder1.init("", 0x0E0D0A0B0100, 10, int2pdsobjkey(100),
                     PDS_NH_TYPE_UNDERLAY);
        feeder2.init("", 0x0E0D0A0B0200, 10, int2pdsobjkey(200),
                     PDS_NH_TYPE_UNDERLAY);
        feeder3.init("", 0x0E0D0A0B0300, 10, int2pdsobjkey(300),
                     PDS_NH_TYPE_UNDERLAY);
    } else {
        feeder1.init("10.10.1.1", 0x0E010B0A1000, 10, int2pdsobjkey(100));
        feeder2.init("20.20.1.1", 0x0E010B0A2000, 20, int2pdsobjkey(200));
        feeder3.init("30.30.1.1", 0x0E010B0A3000, 30, int2pdsobjkey(300));
    }
...
TEST_F(nh_test, nh_workflow_5) {
    ...
    if (apulu()) {
        feeder1.init("", 0x0E0D0A0B0100, 10, int2pdsobjkey(100), PDS_NH_TYPE_UNDERLAY);
        feeder2.init("", 0x0E0D0A0B0200, 10, int2pdsobjkey(200), PDS_NH_TYPE_UNDERLAY);
        feeder3.init("", 0x0E0D0A0B0300, 10, int2pdsobjkey(300), PDS_NH_TYPE_UNDERLAY);
    } else {
        feeder1.init("10.10.1.1", 0x0E010B0A1000, 10, int2pdsobjkey(100));
        feeder2.init("20.20.1.1", 0x0E010B0A2000, 20, int2pdsobjkey(200));
        feeder3.init("30.30.1.1", 0x0E010B0A3000, 30, int2pdsobjkey(300));
...
TEST_F(nh_test, nh_workflow_6) {
    ...
    if (apulu()) {
        feeder1.init("", 0x0E0D0A0B0100, k_max_nh, int2pdsobjkey(1),
                     PDS_NH_TYPE_UNDERLAY);
        feeder1A.init("", 0x0E0D0A0B0200, k_max_nh, int2pdsobjkey(1),
                      PDS_NH_TYPE_UNDERLAY);
        feeder1B.init("", 0x0E0D0A0B0300, k_max_nh, int2pdsobjkey(1),
                      PDS_NH_TYPE_UNDERLAY);
    } else {
...
TEST_F(nh_test, nh_workflow_10) {
    ...
    if (apulu()) {
        ...
        feeder2.init("", 0x0E0D0A0B0200, 10, int2pdsobjkey(200),
                     PDS_NH_TYPE_UNDERLAY);
        feeder2A.init("", 0x0E0D0A0B0210, 10, int2pdsobjkey(200),
                      PDS_NH_TYPE_UNDERLAY);
        feeder3.init("", 0x0E0D0A0B0300, 10, int2pdsobjkey(300),
                     PDS_NH_TYPE_UNDERLAY);
        feeder3A.init("", 0x0E0D0A0B0210, 10, int2pdsobjkey(300),
                      PDS_NH_TYPE_UNDERLAY);
        feeder4.init("", 0x0E0D0A0B0400, 10, int2pdsobjkey(400),
                     PDS_NH_TYPE_UNDERLAY);
    } else {
        feeder1.init("10.10.1.1", 0x0E010B0A1000, 10, int2pdsobjkey(100));
        feeder2.init("20.200.1.1", 0x0E010B0A2000, 20, int2pdsobjkey(200));
        // feeder2A =  feeder2 + different mac & ip
        feeder2A.init("20.20.1.1", 0x0E010B0A2222, 20, int2pdsobjkey(200));
        feeder3.init("30.300.1.1", 0x0E010B0A3000, 30, int2pdsobjkey(300));
        // feeder3A =  feeder3 + different mac & ip
        feeder3A.init("30.30.1.1", 0x0E010B0A3333, 30, int2pdsobjkey(300));
        feeder4.init("40.40.1.1", 0x0E010B0A4000, 40, int2pdsobjkey(400));
    }
...
TEST_F(nh_test, nh_workflow_neg_4) {
    ...
    if (apulu()) {
        ...
        feeder1.init(k_base_nh_ip, k_base_mac, 10, int2pdsobjkey(100));
        feeder2.init("60.60.1.1",  0x0E010B0A2000, 10, int2pdsobjkey(200));
    }
...
TEST_F(nh_test, nh_workflow_neg_5) {
    ...
    if (apulu()) {
        feeder1.init("", 0x0E0D0A0B0100, k_max_nh, int2pdsobjkey(1),
                     PDS_NH_TYPE_UNDERLAY);
        feeder1A.init("", 0x0E0D0A0B0100, k_max_nh, int2pdsobjkey(1),
                      PDS_NH_TYPE_UNDERLAY);
    } else {
        feeder1.init(k_base_nh_ip);
...
TEST_F(nh_test, nh_workflow_neg_6) {
    ...
    if (apulu()) {
        feeder1.init("", 0x0E0D0A0B0100, k_max_nh, int2pdsobjkey(1),
                     PDS_NH_TYPE_UNDERLAY);
        feeder1A.init("", 0x0E0D0A0B0100, k_max_nh+1, int2pdsobjkey(1),
                      PDS_NH_TYPE_UNDERLAY);
    } else {
        feeder1.init(k_base_nh_ip);
        // seed1A = seed1 + different IP & MAC
...
TEST_F(nh_test, nh_workflow_neg_7) {
    ...
    if (apulu()) {
        feeder1.init("", 0x0E0D0A0B0100, 10, int2pdsobjkey(100),
                     PDS_NH_TYPE_UNDERLAY);
        feeder1A.init("", 0x0E0D0A0B0100, 10, int2pdsobjkey(100),
                      PDS_NH_TYPE_UNDERLAY);
        feeder2.init("", 0x0E0D0A0B0200, 10, int2pdsobjkey(200),
                     PDS_NH_TYPE_UNDERLAY);
    } else {
        feeder1.init("10.10.1.1", 0x0E010B0A1000, 10, int2pdsobjkey(100));
        feeder1A.init("10.10.1.1", 0x0E010B0A1111, 10, int2pdsobjkey(100));
        feeder2.init("20.20.1.1", 0x0E010B0A2000, 20, int2pdsobjkey(200));
    }
