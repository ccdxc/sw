...
{
    nh_read_args_t *args = (nh_read_args_t *)ctxt;
    pds::NexthopGetResponse *proto_rsp = (pds::NexthopGetResponse *)(args->ctxt);

    if (args->is_read_all && (api_info->spec.type != args->type)) {
        return;
    }
...
pds_nh_group_api_info_to_proto (pds_nexthop_group_info_t *api_info,
                                void *ctxt)
{
    nh_group_read_args_t *args = (nh_group_read_args_t *)ctxt;
                        ...
                        pds::NexthopResponse *proto_rsp)
{
    ...
    for (int i = 0; i < proto_req->request_size(); i ++) {
        ...
        ret = pds_nexthop_create(&api_spec, bctxt);
        if (ret != SDK_RET_OK) {
            goto end;
        }
    }
                        ...
                        pds::NexthopResponse *proto_rsp)
{
    ...
    for (int i = 0; i < proto_req->request_size(); i ++) {
        ...
        auto proto_spec = proto_req->request(i);
        pds_nh_proto_to_api_spec(&api_spec, proto_spec);
        ret = pds_nexthop_update(&api_spec, bctxt);
        if (ret != SDK_RET_OK) {
            goto end;
        }
    }
                        ...
                        pds::NexthopGetResponse *proto_rsp)
{
    ...
    case pds::NexthopGetRequest::kType:
        {
            nh_read_args_t args;
            args.is_read_all = true;
            args.type = proto_nh_type_to_pds_nh_type(proto_req->type());
            args.ctxt = proto_rsp;
            ret = pds_nexthop_read_all(pds_nh_api_info_to_proto, &args);
            proto_rsp->set_apistatus(sdk_ret_to_api_status(ret));
        }
        break;
    case pds::NexthopGetRequest::kId:
        {
            ...
            pds_nh_api_info_to_proto(&info, &args);
        }
        break;
    default:
        proto_rsp->set_apistatus(types::ApiStatus::API_STATUS_INVALID_ARG);
                        ...
                        pds::NhGroupResponse *proto_rsp)
{
    ...
    for (int i = 0; i < proto_req->request_size(); i ++) {
        ...
        ret = pds_nexthop_group_create(&api_spec, bctxt);
        proto_rsp->set_apistatus(sdk_ret_to_api_status(ret));
        if (ret != SDK_RET_OK) {
            goto end;
        }
                        ...
                        pds::NhGroupResponse *proto_rsp)
{
    ...
    for (int i = 0; i < proto_req->request_size(); i ++) {
        ...
        ret = pds_nexthop_group_update(&api_spec, bctxt);
        proto_rsp->set_apistatus(sdk_ret_to_api_status(ret));
        if (ret != SDK_RET_OK) {
            goto end;
        }
                        ...
                        pds::NhGroupDeleteResponse *proto_rsp)
{
    ...
    for (int i = 0; i < proto_req->id_size(); i++) {
        pds_obj_key_proto_to_api_spec(&key, proto_req->id(i));
        ret = pds_nexthop_group_delete(&key, bctxt);
        proto_rsp->add_apistatus(sdk_ret_to_api_status(ret));
        if (ret != SDK_RET_OK) {
            goto end;
        }
    ...
    case pds::NhGroupGetRequest::kType:
        {
            ...
            args.ctxt = proto_rsp;
            ret = pds_nexthop_group_read_all(pds_nh_group_api_info_to_proto, &args);
            proto_rsp->set_apistatus(sdk_ret_to_api_status(ret));
        }
        break;
    case pds::NhGroupGetRequest::kId:
        {
            ...
            pds_nh_group_api_info_to_proto(&info, &args);
        }
        break;
    default:
        proto_rsp->set_apistatus(types::ApiStatus::API_STATUS_INVALID_ARG);
