...
Status
DHCPSvcImpl::DHCPRelayCreate(ServerContext *context,
                             const pds::DHCPRelayRequest *proto_req,
                             pds::DHCPRelayResponse *proto_rsp) {
    sdk_ret_t ret;
    pds_batch_ctxt_t bctxt;
    pds_dhcp_relay_spec_t api_spec;
    ...
    if (bctxt == PDS_BATCH_CTXT_INVALID) {
        ...
        if (bctxt == PDS_BATCH_CTXT_INVALID) {
            PDS_TRACE_ERR("Failed to create a new batch, DHCP relay policy "
                          "creation failed");
    ...
    for (int i = 0; i < proto_req->request_size(); i ++) {
        ...
        pds_dhcp_relay_proto_to_api_spec(&api_spec, request);
        if (!core::agent_state::state()->pds_mock_mode()) {
            ret = pds_dhcp_relay_create(&api_spec, bctxt);
            if (ret != SDK_RET_OK) {
                goto end;
            }
        }
...
Status
DHCPSvcImpl::DHCPRelayUpdate(ServerContext *context,
                             const pds::DHCPRelayRequest *proto_req,
                             pds::DHCPRelayResponse *proto_rsp) {
    sdk_ret_t ret;
    pds_batch_ctxt_t bctxt;
    ...
    for (int i = 0; i < proto_req->request_size(); i ++) {
        ...
        pds_dhcp_relay_proto_to_api_spec(&api_spec, request);
        if (!core::agent_state::state()->pds_mock_mode()) {
            ret = pds_dhcp_relay_update(&api_spec, bctxt);
            if (ret != SDK_RET_OK) {
                goto end;
            }
        }
...
Status
DHCPSvcImpl::DHCPRelayDelete(ServerContext *context,
...
Status
DHCPSvcImpl::DHCPRelayGet(ServerContext *context,
    ...
    if (proto_req->id_size() == 0) {
        // get all
        ret = pds_dhcp_relay_read_all(pds_dhcp_relay_api_info_to_proto,
                                      proto_rsp);
        proto_rsp->set_apistatus(sdk_ret_to_api_status(ret));
    }
    ...
    for (int i = 0; i < proto_req->id_size(); i ++) {
        ...
        pds_dhcp_relay_api_info_to_proto(&info, proto_rsp);
    }
    return Status::OK;
}
