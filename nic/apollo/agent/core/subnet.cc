...
subnet_create_validate (pds_subnet_spec_t *spec)
{
    ...
    if (!ipv4_prefix_within_prefix(&spec->v4_pfx, &vpc_spec->v4_pfx)) {
        PDS_TRACE_ERR("Failed to create subnet {}, IPv4 prefix invalid", spec->key.id);
        return SDK_RET_INVALID_ARG;
    }
    // IPv6 prefix for subnet must be within VPC prefix
    if (!ip_prefix_within_prefix(&spec->v6_pfx, &vpc_spec->v6_pfx)) {
        PDS_TRACE_ERR("Failed to create subnet {}, IPv6 prefix invalid", spec->key.id);
        return SDK_RET_INVALID_ARG;
    }
    // validate VR IP
    if ((spec->v4_vr_ip == 0) && (ip_addr_is_zero(&spec->v6_vr_ip))) {
        PDS_TRACE_ERR("Failed to create subnet {}, virtual router IP invalid", spec->key.id);
        return SDK_RET_INVALID_ARG;
    }
    // validate VR MAC
    if (memcmp(&spec->vr_mac, &zero_mac, sizeof(spec->vr_mac)) == 0) {
        PDS_TRACE_ERR("Failed to create subnet {}, VR MAC invalid", spec->key.id);
        return SDK_RET_INVALID_ARG;
    }
    return SDK_RET_OK;
}
...
subnet_create (pds_subnet_key_t *key, pds_subnet_spec_t *spec)
{
    ...
    if (agent_state::state()->find_in_subnet_db(key) != NULL) {
        PDS_TRACE_ERR("Failed to create subnet {}, subnet already exists", spec->key.id);
    ...
    if ((ret = agent_state::state()->add_to_subnet_db(key, spec)) != SDK_RET_OK) {
        PDS_TRACE_ERR("Failed to add subnet {} to db, err {}", spec->key.id, ret);
        return ret;
    }
    return SDK_RET_OK;
}
...
subnet_update_validate (pds_subnet_spec_t *spec)
{
    ...
    // verify VPC exists
    if ((vpc_spec = agent_state::state()->find_in_vpc_db(&spec->vpc)) == NULL) {
        PDS_TRACE_ERR("Failed to update subnet {}, vpc {} not found", spec->key.id, spec->vpc.id);
        return SDK_RET_INVALID_ARG;
    }
    // IPv4 prefix for subnet must be within VPC prefix
    if (!ipv4_prefix_within_prefix(&spec->v4_pfx, &vpc_spec->v4_pfx)) {
    ...
    // validate VR IP
    if ((spec->v4_vr_ip == 0) && (ip_addr_is_zero(&spec->v6_vr_ip))) {
        PDS_TRACE_ERR("Failed to update subnet {}, virtual router IP invalid", spec->key.id);
        return SDK_RET_INVALID_ARG;
    }
    // validate VR MAC
    if (memcmp(&spec->vr_mac, &zero_mac, sizeof(spec->vr_mac)) == 0) {
        PDS_TRACE_ERR("Failed to update subnet {}, VR MAC invalid", spec->key.id);
        return SDK_RET_INVALID_ARG;
    }
    return SDK_RET_OK;
}
...
subnet_update (pds_subnet_key_t *key, pds_subnet_spec_t *spec)
{
    ...
    if (agent_state::state()->find_in_subnet_db(key) == NULL) {
        PDS_TRACE_ERR("Failed to update subnet {}, subnet doesn't exist", spec->key.id);
    ...
    if (!agent_state::state()->pds_mock_mode()) {
        if ((ret = pds_subnet_update(spec)) != SDK_RET_OK) {
            PDS_TRACE_ERR("Failed to update subnet {}, err {}", spec->key.id, ret);
            return ret;
        }
    }
