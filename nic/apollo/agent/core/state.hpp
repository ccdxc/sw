...
typedef unordered_map<pds_vpc_key_t, pds_vpc_spec_t *, pds_obj_key_hash> vpc_db_t;
...
public:
    ...
    const pds_vpc_key_t& underlay_vpc(void) const { return underlay_vpc_; }
    void set_underlay_vpc(pds_vpc_key_t key) { underlay_vpc_ = key; }
...
public:
    ...
    pds_nexthop_group_spec_t *find_in_nh_group_db(pds_nexthop_group_key_t *key);
    sdk_ret_t add_to_nh_group_db(pds_nexthop_group_key_t *key,
                                 pds_nexthop_group_spec_t *spec);
    sdk_ret_t nh_group_db_walk(nh_group_walk_cb_t cb, void *ctxt);
    bool del_from_nh_group_db(pds_nexthop_group_key_t *key);
    ...
    const pds_vpc_key_t& underlay_vpc(void) const {
        return cfg_db_->underlay_vpc();
    }
    void set_underlay_vpc(pds_vpc_key_t key) {
