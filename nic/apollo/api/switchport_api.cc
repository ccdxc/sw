...
sdk_ret_t
pds_switchport_create (pds_switchport_spec_t *switchport)
{
    api_ctxt_t api_ctxt;
...
static inline void
pds_switchport_spec_fill (switchport_entry *entry, pds_switchport_spec_t *spec)
{
    spec->gateway_ip_addr = entry->gw_ip_addr();
}

sdk_ret_t
pds_switchport_read (pds_switchport_info_t *info)
{
    ...
    pds_switchport_spec_fill(entry, &info->spec);
