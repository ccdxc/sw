...
void
poseidon_impl::sort_mpu_programs_(std::vector<std::string>& programs) {
...
uint32_t
poseidon_impl::rxdma_symbols_init_(void **p4plus_symbols,
...
uint32_t
poseidon_impl::txdma_symbols_init_(void **p4plus_symbols,
                                 platform_type_t platform_type)
{
    uint32_t    i = 0;
...
sdk_ret_t
poseidon_impl::init_(pipeline_cfg_t *pipeline_cfg) {
    pipeline_cfg_ = *pipeline_cfg;
    return SDK_RET_OK;
}

poseidon_impl *
poseidon_impl::factory(pipeline_cfg_t *pipeline_cfg) {
    poseidon_impl    *impl;

    impl = (poseidon_impl *)SDK_CALLOC(SDK_MEM_ALLOC_PDS_PIPELINE_IMPL,
                                    sizeof(poseidon_impl));
    new (impl) poseidon_impl();
...
void
poseidon_impl::destroy(poseidon_impl *impl) {
    int i;
    sdk_table_api_params_t       tparams = { 0 };

    // Remove key native table entries
    for (i = 0; i < MAX_KEY_NATIVE_TBL_ENTRIES; i++) {
        tparams.handle = poseidon_impl_db()->key_native_tbl_hdls_[i];
        poseidon_impl_db()->key_native_tbl()->remove(&tparams);
    }
    // Remove key tunneled table entries
    for (i = 0; i < MAX_KEY_TUNNELED_TBL_ENTRIES; i++) {
        tparams.handle = poseidon_impl_db()->key_tunneled_tbl_hdls_[i];
        poseidon_impl_db()->key_tunneled_tbl()->remove(&tparams);
    }
    // Remove drop stats table entries
    for (i = P4E_DROP_REASON_MIN; i <= P4E_DROP_REASON_MAX; i++) {
        tparams.handle = poseidon_impl_db()->egress_drop_stats_tbl_hdls_[i];
        poseidon_impl_db()->egress_drop_stats_tbl()->remove(&tparams);
    }
    for (i = P4I_DROP_REASON_MIN; i <= P4I_DROP_REASON_MAX; i++) {
        tparams.handle = poseidon_impl_db()->ingress_drop_stats_tbl_hdls_[i];
        poseidon_impl_db()->ingress_drop_stats_tbl()->remove(&tparams);
    }
...
void
poseidon_impl::program_config_init(pds_init_params_t *init_params,
                                 asic_cfg_t *asic_cfg) {
...
void
poseidon_impl::asm_config_init(pds_init_params_t *init_params,
                             asic_cfg_t *asic_cfg) {
...
void
poseidon_impl::ring_config_init(asic_cfg_t *asic_cfg) {
    asic_cfg->num_rings = 0;
}

sdk_ret_t
poseidon_impl::key_native_init_(void) {
    sdk_ret_t                  ret;
    uint32_t                   idx = 0;
    ...
    ret = poseidon_impl_db()->key_native_tbl()->insert(&tparams);
    SDK_ASSERT(ret == SDK_RET_OK);
    poseidon_impl_db()->key_native_tbl_hdls_[idx++] = tparams.handle;
...
sdk_ret_t
poseidon_impl::key_tunneled_init_(void) {
    sdk_ret_t                    ret;
    uint32_t                     idx = 0;
    sdk_table_api_params_t       tparams;
    ...
    PDS_IMPL_FILL_TABLE_API_PARAMS(&tparams, &key, &mask, &data,
                                   ...
                                   sdk::table::handle_t::null());
    ret = poseidon_impl_db()->key_tunneled_tbl()->insert(&tparams);
    SDK_ASSERT(ret == SDK_RET_OK);
    poseidon_impl_db()->key_tunneled_tbl_hdls_[idx++] = tparams.handle;
...
sdk_ret_t
poseidon_impl::ingress_to_rxdma_init_(void) {
...
sdk_ret_t
poseidon_impl::egress_drop_stats_init_(void) {
    sdk_ret_t                    ret;
    ...
    for (uint32_t i = P4E_DROP_REASON_MIN; i <= P4E_DROP_REASON_MAX; i++) {
        ...
        PDS_IMPL_FILL_TABLE_API_PARAMS(&tparams, &key, &key_mask, &data,
                                       ...
                                       sdk::table::handle_t::null());
        ret = poseidon_impl_db()->egress_drop_stats_tbl()->insert(&tparams);
        poseidon_impl_db()->egress_drop_stats_tbl_hdls_[i] = tparams.handle;
        SDK_ASSERT(ret == SDK_RET_OK);
    }
    return ret;
}
...
sdk_ret_t
poseidon_impl::ingress_drop_stats_init_(void) {
    sdk_ret_t                    ret;
    ...
    for (uint32_t i = P4I_DROP_REASON_MIN; i <= P4I_DROP_REASON_MAX; i++) {
        ...
        PDS_IMPL_FILL_TABLE_API_PARAMS(&tparams, &key, &key_mask, &data,
                                       ...
                                       sdk::table::handle_t::null());
        ret = poseidon_impl_db()->ingress_drop_stats_tbl()->insert(&tparams);
        poseidon_impl_db()->ingress_drop_stats_tbl_hdls_[i] = tparams.handle;
        SDK_ASSERT(ret == SDK_RET_OK);
    }
    return ret;
}
...
sdk_ret_t
poseidon_impl::stats_init_(void) {
...
sdk_ret_t
poseidon_impl::table_init_(void) {
    sdk_ret_t     ret;
    mem_addr_t    addr;
...
sdk_ret_t
poseidon_impl::p4plus_table_init_(void) {
...
sdk_ret_t
poseidon_impl::pipeline_init(void) {
    sdk_ret_t  ret;
    p4pd_cfg_t p4pd_cfg;
    ...
    SDK_ASSERT(ret == SDK_RET_OK);
    ret = sdk::asic::pd::asicpd_toeplitz_init("poseidon_rxdma",
...
sdk_ret_t
poseidon_impl::write_to_rxdma_table(mem_addr_t addr, uint32_t tableid,
...
sdk_ret_t
poseidon_impl::write_to_txdma_table(mem_addr_t addr, uint32_t tableid,
                                  uint8_t action_id, void *actiondata) {
    uint32_t     len;
    uint8_t      packed_bytes[CACHE_LINE_SIZE];
    uint8_t      *packed_entry = packed_bytes;
...
sdk_ret_t
poseidon_impl::table_transaction_begin(void) {
    return SDK_RET_OK;
}

sdk_ret_t
poseidon_impl::table_transaction_end(void) {
    return SDK_RET_OK;
}

sdk_ret_t
poseidon_impl::table_stats(debug::table_stats_get_cb_t cb, void *ctxt) {
    return SDK_RET_OK;
}
 ...
 */
sdk_ret_t
poseidon_impl::meter_stats(debug::meter_stats_get_cb_t cb, uint32_t lowidx,
...
sdk_ret_t
poseidon_impl::session_stats(debug::session_stats_get_cb_t cb, uint32_t lowidx,
                           uint32_t highidx, void *ctxt) {
    sdk_ret_t ret;
    uint64_t offset = 0;
    uint64_t start_addr = 0;
...
sdk_ret_t
poseidon_impl::session(debug::session_get_cb_t cb, void *ctxt) {
    return SDK_RET_OK;
}

sdk_ret_t
poseidon_impl::flow(debug::flow_get_cb_t cb, void *ctxt) {
    uint32_t idx = 0;
    p4pd_error_t p4pd_ret;
...
sdk_ret_t
poseidon_impl::session_clear(uint32_t idx) {
    return SDK_RET_OK;
}

sdk_ret_t
poseidon_impl::flow_clear(uint32_t idx) {
    return SDK_RET_OK;
}

sdk_ret_t
poseidon_impl::handle_cmd(cmd_ctxt_t *ctxt) {
    switch (ctxt->cmd) {
    default:
