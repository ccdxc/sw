...
#include "nic/apollo/api/impl/poseidon/poseidon_impl_state.hpp"
...
public:
    ...
    pds_impl_state();
    ~pds_impl_state();
    poseidon_impl_state *poseidon_impl_db(void) const { return poseidon_impl_db_; }
    lif_impl_state *lif_impl_db(void) const { return lif_impl_db_; }

private:
    poseidon_impl_state          *poseidon_impl_db_;
    lif_impl_state             *lif_impl_db_;
};
extern pds_impl_state g_pds_impl_state;

static inline poseidon_impl_state *
poseidon_impl_db (void)
{
    return  g_pds_impl_state.poseidon_impl_db();
}
