...
public:
    ...
    uint32_t max_prefixes(uint8_t af) const {
        if (af == IP_AF_IPV4) {
            return v4_max_prefixes_;
        }
        return v6_max_prefixes_;
    }

private:
    indexer *idxr(uint8_t af) {
        if (af == IP_AF_IPV4) {
            return v4_idxr_;
        }
        return v6_idxr_;
