...
pds_vcn_read (pds_vcn_key_t *key, pds_vcn_info_t *info)
{
    ...
    if ((rv = pds_vcn_spec_fill(entry, &info->spec)) != SDK_RET_OK) {
        return rv;
    }

    if ((rv = pds_vcn_status_fill(entry, &info->status)) != SDK_RET_OK) {
        return rv;
    }

    if ((rv = pds_vcn_stats_fill(entry, &info->stats)) != SDK_RET_OK) {
        return rv;
    }
