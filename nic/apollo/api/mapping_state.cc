...
mapping_state::kvstore_iterate(mapping_state_cb_t cb, void *ctxt) {
    ...
    it_ctxt.cb = cb;
    it_ctxt.ctxt = ctxt;
    kvstore_->iterate(mapping_state_entry_cb_, &it_ctxt, "mapping");
}
...
mapping_state::skey(pds_obj_key_t *key, pds_mapping_key_t *skey) const {
    ...
    ret = kvstore_->find(key, sizeof(*key), skey, &skey_sz, "mapping");
    if (ret != SDK_RET_OK) {
...
mapping_state::perish(const pds_obj_key_t& key) {
    ...
    if (key.valid()) {
        ret = kvstore_->remove(&key, sizeof(key), "mapping");
        if (unlikely(ret != SDK_RET_OK)) {
