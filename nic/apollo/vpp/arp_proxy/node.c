...
#include <nic/apollo/vpp/arp_proxy/includes.h>
#include <nic/apollo/vpp/arp_proxy/pi_impl.h>
#include <nic/apollo/vpp/flow/includes.h>
#include <nic/apollo/vpp/infra/node.h>
                    ...
                    u32 flags)
{
    ...
    if (PREDICT_FALSE(flags & VLIB_NODE_FLAG_TRACE &&
           ...
           vlib_frame_t * from_frame)
{
    ...
    PDS_PACKET_LOOP_START {
        ...
        PDS_PACKET_DUAL_LOOP_START (WRITE, READ) {
            ...
            arp_proxy_internal(_b[0], &next_idx, _nexts, counter, node->flags);
            // TODO : advance based on the new header that will be added
            //vlib_buffer_advance(_b[0], -16);

            arp_proxy_internal(_b[1], &next_idx, _nexts, counter, node->flags);
