#include "../../../common-p4+/p4/common_headers.p4"
...
header_type barco_descriptor_t {
    fields {
        ...
        p42p4plus_hdr_payload_start : 16;
        ipsec_cb_key_index          : 32;
        ipsec_int_rsvd              : 32; // will put the right value later. Total has to be 512
...
// Needed only for TXDMA - lets move this to a different program
metadata barco_descriptor_t barco_desc;
metadata barco_request_t barco_req;


//dma headers below this
metadata ipsec_rxdma_cmds_t ipsec_rxdma_cmds;

//This is the 512 bits of scratch in the barco descriptor.
#if 0
action ipsec_rxdma_fill_ipsec_int_content_for_txdma(ipsec_int_in_desc,
                                                    ipsec_int_out_desc, 
                                                    ipsec_int_barco_enc_cmd, 
                                                    ipsec_int_ipsec_cb_index,
                                                    ipsec_int_headroom,
                                                    ipsec_int_tailroom,
                                                    ipsec_int_headroom_offset,
                                                    ipsec_int_tailroom_offset,
                                                    ipsec_int_pad_size, 
                                                    in_desc_entry_addr,
                                                    out_desc_entry_addr, 
                                                    p42p4plus_hdr_payload_start,
                                                    ipsec_cb_key_index,
                                                    ipsec_int_rsvd)
#endif
...
action in_page_ptr_read(in_page_value)
{
    // modify_field(rxdma_scratch.in_page_value, in_page_value);
    modify_field(te0_phv.table3_valid, 1);  
    ...
    modify_field(te_phv.table3_rsvd, 0);
}

action out_page_ptr_read(out_page_value)
{
    // modify_field(rxdma_scratch.out_page_value, out_page_value);
    modify_field(te0_phv.table2_valid, 1);   
    ...
    modify_field(te_phv.table2_rsvd, 0);
}

action out_desc_ptr_read(out_desc_value)
{
    // modify_field(rxdma_scratch.out_desc_value, out_desc_value);
    modify_field(te0_phv.table1_valid, 1);   
    ...
    modify_field(te_phv.table1_rsvd, 0);
}

action in_desc_ptr_read(in_desc_value)
{
    //modify_field(rxdma_scratch.in_desc_value, in_desc_value);

    modify_field(te0_phv.table0_valid, 1);   
    ...
    modify_field(te0_phv.table0_rsvd, 0);
} 
...
control ipsec_esp_v4_tun_h2n_create_barco_request {

}

control ipsec_esp_v4_tun_h2n_post_barco {

}

control ipsec_esp_v4_tun_h2n_barco_request_dequeue {

}

control ipsec_esp_v4_tun_h2n_build_encap_pkt {

}

control ipsec_esp_v4_tun_h2n_update_stats {

}
 
control txdma {
    // now this goes to txdma ??
    ipsec_esp_v4_tun_h2n_create_barco_request();
    ipsec_esp_v4_tun_h2n_post_barco();
    ipsec_esp_v4_tun_h2n_barco_request_dequeue();
    ipsec_esp_v4_tun_h2n_build_encap_pkt();
    ipsec_esp_v4_tun_h2n_update_stats();
}
