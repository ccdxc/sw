...
action s0_name(key0, key1, key2, key3, key4, key5, key6, key7,
    ...
    if (key >= key7) {
        if (key >= key11) {
            if (key >= key13) {
                if (key >= key14) {
                    modify_field(next_addr, curr_addr + LPM_S0_B15_OFFSET);
                    modify_field(s2_offset, LPM_S2_BLOCK15_OFFSET);
                }
                else {
                    modify_field(next_addr, curr_addr + LPM_S0_B14_OFFSET);
                    modify_field(s2_offset, LPM_S2_BLOCK14_OFFSET);
                }
            }
            else {
                if (key >= key12) {
                    modify_field(next_addr, curr_addr + LPM_S0_B13_OFFSET);
                    modify_field(s2_offset, LPM_S2_BLOCK13_OFFSET);
                }
                else {
                    modify_field(next_addr, curr_addr + LPM_S0_B12_OFFSET);
            ...
            if (key >= key9) {
                if (key >= key10) {
                    modify_field(next_addr, curr_addr + LPM_S0_B11_OFFSET);
                    modify_field(s2_offset, LPM_S2_BLOCK11_OFFSET);
                }
                else {
                    modify_field(next_addr, curr_addr + LPM_S0_B10_OFFSET);
                    modify_field(s2_offset, LPM_S2_BLOCK10_OFFSET);
                }
            }
            else {
                if (key >= key8) {
                    modify_field(next_addr, curr_addr + LPM_S0_B9_OFFSET);
                    modify_field(s2_offset, LPM_S2_BLOCK9_OFFSET);
                }
                else {
                    modify_field(next_addr, curr_addr + LPM_S0_B8_OFFSET);
        ...
        if (key >= key3) {
            if (key >= key5) {
                if (key >= key6) {
                    modify_field(next_addr, curr_addr + LPM_S0_B7_OFFSET);
                    modify_field(s2_offset, LPM_S2_BLOCK7_OFFSET);
                }
                else {
                    modify_field(next_addr, curr_addr + LPM_S0_B6_OFFSET);
                    modify_field(s2_offset, LPM_S2_BLOCK6_OFFSET);
                }
            }
            else {
                if (key >= key4) {
                    modify_field(next_addr, curr_addr + LPM_S0_B5_OFFSET);
                    modify_field(s2_offset, LPM_S2_BLOCK5_OFFSET);
                }
                else {
                    modify_field(next_addr, curr_addr + LPM_S0_B4_OFFSET);
            ...
            if (key >= key1) {
                if (key >= key2) {
                    modify_field(next_addr, curr_addr + LPM_S0_B3_OFFSET);
                    modify_field(s2_offset, LPM_S2_BLOCK3_OFFSET);
                }
                else {
                    modify_field(next_addr, curr_addr + LPM_S0_B2_OFFSET);
                    modify_field(s2_offset, LPM_S2_BLOCK2_OFFSET);
                }
            }
            else {
                if (key >= key0) {
                    modify_field(next_addr, curr_addr + LPM_S0_B1_OFFSET);
                    modify_field(s2_offset, LPM_S2_BLOCK1_OFFSET);
                }
                else {
                    modify_field(next_addr, curr_addr + LPM_S0_B0_OFFSET);
...
action s1_name(key0, key1, key2, key3, key4, key5, key6, key7,
    ...
    if (key >= key7) {
        if (key >= key11) {
            if (key >= key13) {
                if (key >= key14) {
                    modify_field(next_addr, curr_addr + s2_offset +
                                            LPM_S1_B15_OFFSET);
                }
                else {
                    modify_field(next_addr, curr_addr + s2_offset +
