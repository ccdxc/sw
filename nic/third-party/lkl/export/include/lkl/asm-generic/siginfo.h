...
#ifndef __LKL__ARCH_SI_UID_T
#define __LKL__ARCH_SI_UID_T	__lkl__kernel_uid32_t
#endif
...
typedef struct lkl_siginfo {
	...
	union {
		...
		struct {
			...
			__LKL__ARCH_SI_UID_T _uid;	/* sender's uid */
		} _kill;
		...
		struct {
			...
			char _pad[sizeof( __LKL__ARCH_SI_UID_T) - sizeof(int)];
		...
		/* LKL_SIGILL, LKL_SIGFPE, LKL_SIGSEGV, LKL_SIGBUS */
		struct {
...
/* If the arch shares siginfo, then it has LKL_SIGSYS. */
#define __LKL__ARCH_SIGSYS
#endif
