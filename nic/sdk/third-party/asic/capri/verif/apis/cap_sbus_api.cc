...
float cap_read_voltage(uint32_t sbus_addr, uint32_t sensor_num) {
  uint32_t xx = cap_read_sensor_val(sbus_addr, sensor_num, 1);
  float volt = ((xx>>1)&0x3ff) + (xx&0x1)/2.0; 
  //PLOG_MSG("cap_get_voltage :: voltage:"  << volt << endl);
  return(volt);
}

//////////////////////////////////////////////////////////////////
float cap_get_voltage(uint32_t sbus_addr, uint32_t sensor_num) {
  cap_voltage_en(sbus_addr, sensor_num);
#ifndef _CSV_INCLUDED_
  SLEEP(10000);
#endif
  return cap_read_voltage(sbus_addr, sensor_num);
}

//////////////////////////////////////////////////////////////////
void cap_voltage_en(uint32_t sbus_addr, uint32_t vsense_mode) {

  uint32_t chip_id = cpu::access()->get_cur_chip_id();
  uint32_t vdd_mode = (1<<(9+vsense_mode));
  uint32_t dd = 600;

  //core_clk=1.1G(max), div=600 assures that sensor_clk<2MHz
  cap_pp_sbus_write(chip_id, sbus_addr, 0x0, 0x1); //rst

  cap_pp_sbus_write(chip_id, sbus_addr, 0x1, dd); //divider
  cap_pp_sbus_write(chip_id, sbus_addr, 0x3, vdd_mode);

  cap_pp_sbus_write(chip_id, sbus_addr, 0x0, 0x2); //enb
  //cap_pp_sbus_write(chip_id, sbus_addr, 0x0, 0x0);
  //cap_pp_sbus_write(chip_id, sbus_addr, 0x0, 0x2);
}

float cap_read_temp(uint32_t sbus_addr, uint32_t sensor_num) {
  uint32_t tt = 0xfff & cap_read_sensor_val(sbus_addr, sensor_num, 0);
  float xx;
  if(tt & 0x800) {
    xx = ( (~( ((tt>>3)&0xff)-1 )) & 0xff ) + ((~((tt&0x7)-1))&0x7)/8.0;
  } else {
    xx = ((tt>>3)&0xff) + (tt&0x7)/8.0;
  }
  //PLOG_MSG("cap_get_temp :: temp:"  << xx << " two complement temp from sensor:0x" << hex << tt << dec << endl);
  return(xx);
}

//////////////////////////////////////////////////////////////////
float cap_get_temp(uint32_t sbus_addr, uint32_t sensor_num) {
  cap_temperature_en(sbus_addr, sensor_num);
#ifndef _CSV_INCLUDED_
  SLEEP(10000);
#endif
  return cap_read_temp(sbus_addr, sensor_num);
}

//////////////////////////////////////////////////////////////////
void cap_temperature_en(uint32_t sbus_addr, uint32_t tsense_mode) {

  uint32_t chip_id = cpu::access()->get_cur_chip_id();
  uint32_t vdd_mode = (1<<(tsense_mode));
  uint32_t dd = 600;

  //core_clk=1.1G(max), div=600 assures that sensor_clk<2MHz
  cap_pp_sbus_write(chip_id, sbus_addr, 0x0, 0x1);

  cap_pp_sbus_write(chip_id, sbus_addr, 0x1, dd);
  cap_pp_sbus_write(chip_id, sbus_addr, 0x3, vdd_mode);
...
//////////////////////////////////////////////////////////////////
uint32_t cap_read_sensor_val(uint32_t sbus_addr, uint32_t mode, uint32_t v_or_t) {

  uint32_t chip_id = cpu::access()->get_cur_chip_id();
  uint32_t rr = (mode + ((v_or_t)? 74 : 65)); //sense voltage 74, temp 65
  uint32_t timeout = 50000; //50ms at 2MHz
  uint32_t sensor_val = 0; // sensor_val[15]==1, sensor_val[11:0] = 2's complement val
  do {
    sensor_val = cap_pp_sbus_read(chip_id, sbus_addr, rr);
    timeout--;
    if ( (timeout%1000) == 0 ) {
      PLOG_MSG("cap_read_sensor_val :: Waiting for sensor_val[15]==1, sensor_val:0x"
          << hex << sensor_val << dec << endl);
    }
  } while (((sensor_val & 0x8000) == 0) && timeout);

  if (timeout == 0) {
    PLOG_ERR("cap_read_sensor_val :: Timeout while waiting for sensor_val[15]==1, sensor_val:0x"
        << hex << sensor_val << dec << endl);
    sensor_val = 0xffff;
  }
