...
string
venice::get_naples_status_file() {
    return venice_db_.naples_status_file;
}
...
venice::populate_venice(ptree &root)
{
    ...
    this->dump_venice_coordinates();
    return SDK_RET_OK;
}

sdk_ret_t
venice::dump_venice_coordinates()
{
    ...
    for (std::vector<string>::iterator it = venice_db_.controllers.begin() ; it != venice_db_.controllers.end(); ++it) {
        SDK_TRACE_DEBUG("Controller: %s", (*it).c_str());
    }
