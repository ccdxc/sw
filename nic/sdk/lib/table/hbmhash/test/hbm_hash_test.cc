...
typedef struct __attribute__((__packed__)) flow_hash_swkey {
    uint8_t flow_lkp_metadata_lkp_inst;
    uint8_t flow_lkp_metadata_lkp_type;
    uint8_t flow_lkp_metadata_lkp_dir;
    uint8_t flow_lkp_metadata_lkp_dst[16];
    uint8_t flow_lkp_metadata_lkp_src[16];
    uint16_t flow_lkp_metadata_lkp_dport;
    uint16_t flow_lkp_metadata_lkp_sport;
    uint8_t flow_lkp_metadata_lkp_proto;
    uint16_t flow_lkp_metadata_lkp_vrf;
} flow_hash_swkey_t;

typedef enum flow_hash_actions_enum {
    FLOW_HASH_FLOW_HASH_INFO_ID = 0,
    FLOW_HASH_MAX_ID = 1
} flow_hash_actions_en;

typedef struct __attribute__((__packed__)) __flow_hash_flow_hash_info {
    uint8_t entry_valid;
    uint8_t export_en;
    uint32_t flow_index;
    uint16_t hash1;
    ...
    uint8_t more_hashs;
    uint32_t more_hints;
} flow_hash_flow_hash_info_t;

typedef union __flow_hash_action_union {
    flow_hash_flow_hash_info_t flow_hash_flow_hash_info;
} flow_hash_action_union_t;

typedef struct __attribute__((__packed__)) __flow_hash_actiondata {
    uint8_t actionid;
    flow_hash_action_union_t flow_hash_action_u;
} flow_hash_actiondata;
...
TEST_F(hbm_hash_test, DISABLED_test7) {
    ...
    while (1) {
        ...
        SDK_TRACE_DEBUG("Checking:{:#x}", ft_bits);

        if (ft_bits == 0x10001) {
            SDK_TRACE_DEBUG("MATCH::");
            for (int i = 0; i < 4; i++) {
                SDK_TRACE_DEBUG("Src[{}]: {:#x}", i, key.flow_lkp_metadata_lkp_src[i]);
                SDK_TRACE_DEBUG("Dst[{}]: {:#x}", i, key.flow_lkp_metadata_lkp_dst[i]);
            }
...
TEST_F(hbm_hash_test, test17) {
    ...
    SDK_TRACE_DEBUG("tableid:{}, table_name:{}, capacity:{}, coll_capac:{}, "
                    "num_in_use:{}, coll_num_in_use:{}"
...
TEST_F(hbm_hash_test, test19) {
    ...
    SDK_TRACE_DEBUG("Took : {} tableid:{}, table_name:{}, capacity:{}, coll_capac:{}, "
                    "num_in_use:{}, coll_num_in_use:{}"
                    "num_inserts:{}, num_insert_errors:{}, "
