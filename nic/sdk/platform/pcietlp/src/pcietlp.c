...
static int pcietlp_set_error(const char *fmt, ...)
{
    ...
    if (pi->error == 0) {
        ...
        va_start(ap, fmt);
        vsnprintf(pi->error_str, sizeof(pi->error_str), fmt, ap);
        va_end(ap);
        pi->error = 1;
    }
    return -1;
...
{
    addrp[0] = htobe32(stlp->addr & ~0x3); /* DW-align addr */
}

static void
decode_addr32(pcie_stlp_t *stlp, const u_int32_t addr)
{
    stlp->addr += be32toh(addr);
}
...
{
    addrp[0] = htobe32(stlp->addr >> 32);
    addrp[1] = htobe32(stlp->addr & ~0x3); /* DW-align addr */
}

static void
decode_addr64(pcie_stlp_t *stlp, const u_int32_t *addrp)
{
    stlp->addr += ((u_int64_t)be32toh(addrp[0]) << 32) | be32toh(addrp[1]);
}
...
{
    const u_int32_t v = le32toh(*datap);
...
encode_data64(const pcie_stlp_t *stlp, u_int32_t *datap)
{
    ...
    datap[0] = htole32(v);
    datap[1] = htole32(v >> 32);
}

static void
decode_data64(pcie_stlp_t *stlp, const u_int32_t *datap)
{
    const u_int64_t v = (le32toh(datap[0]) |
                         (u_int64_t)le32toh(datap[1]) << 32);
...
decode_cmn_hdr(pcie_stlp_t *stlp, const void *rtlp)
{
    ...
    const u_int8_t ffbe = ffs(hdr->fbe);
...
pcietlp_buf(const pcie_stlp_t *stlp, void *buf, const size_t bufsz)
{
    ...
    case PCIE_STLP_MALFORMED:
        snprintf(buf, bufsz, "MALFORMED");
        break;
    default:
        snprintf(buf, bufsz, "UNKNOWN type %d", stlp->type);
        break;
    }
    return buf;
}
