...
pxb_read_target_counters (void)
{
    ...
    pal_reg_rd32w(ELB_ADDR_BASE_PXB_PXB_OFFSET +
                      ELB_PXB_CSR_STA_TGT_TOT_AXI_WR_BYTE_ADDRESS,
    ...
    pal_reg_rd32w(ELB_ADDR_BASE_PXB_PXB_OFFSET +
                      ELB_PXB_CSR_STA_TGT_TOT_AXI_RD_BYTE_ADDRESS,
...
void
pxb_reset_target_counters (void)
{
    uint64_t zero = 0;
...
void
pxb_read_target_err_counters (void)
{
...
void
elbmon_asic_data_store1 (uint64_t axi_wr, uint64_t axi_wr_64,
...
void
pxb_read_initiator_counters (void)
{
    ...
    // Write counters
    pal_reg_rd32w(ELB_ADDR_BASE_PXB_PXB_OFFSET +
                      ELB_PXB_CSR_STA_ITR_TOT_AXI_WR_BYTE_ADDRESS,
                  (uint32_t *)&axi_wr_bytes, 2);
    ...
    // Read counters
    pal_reg_rd32w(ELB_ADDR_BASE_PXB_PXB_OFFSET +
                      ELB_PXB_CSR_STA_ITR_TOT_AXI_RD_BYTE_ADDRESS,
                  (uint32_t *)&axi_rd_bytes, 2);
    ...
    elbmon_asic_data_store1(axi_wr, axi_wr64, axi_wr256, axi_wr_bytes, axi_rd,
                            axi_rd64, axi_rd256, axi_rd_bytes, atomic_req);
}

void
pxb_reset_initiator_counters (void)
{
    uint64_t zero = 64;
...
void
elbmon_asic_data_store2 (uint32_t axi_wr_pending, uint32_t axi_rd_pending,
...
void
pxb_read_initiator_status (void)
{
    ...
    elbmon_asic_data_store2(axi_wr_pending, axi_rd_pending, rd_lat0, rd_lat1,
...
pxb_read_pport_status (uint8_t port)
{
    ...
    for (int i = 0; i < polls; i++) {
        pal_reg_rd32w(ELB_ADDR_BASE_PP_PP_OFFSET +
...
pxb_read_pport_counters (uint8_t port)
{
    ...
    // TLP counters
    pal_reg_rd32w(ELB_ADDR_BASE_PP_PP_OFFSET +
                      ELB_PP_CSR_PORT_C_CNT_C_TL_RX_REQ_BYTE_ADDRESS,
                  (uint32_t *)&rx_req_tlp, 2);

    pal_reg_rd32w(ELB_ADDR_BASE_PP_PP_OFFSET +
                      ELB_PP_CSR_PORT_C_CNT_C_TL_RX_CPL_BYTE_ADDRESS,
                  (uint32_t *)&rx_cpl_tlp, 2);

    pal_reg_rd32w(ELB_ADDR_BASE_PP_PP_OFFSET +
                      ELB_PP_CSR_PORT_C_CNT_C_TL_TX_REQ_BYTE_ADDRESS,
                  (uint32_t *)&tx_req_tlp, 2);

    pal_reg_rd32w(ELB_ADDR_BASE_PP_PP_OFFSET +
                      ELB_PP_CSR_PORT_C_CNT_C_TL_TX_CPL_BYTE_ADDRESS,
...
void
elbmon_asic_data_store3 (uint64_t rx_bad_tlp, uint64_t rx_bad_dllp,
...
void
elbmon_asic_data_store4 (uint64_t fcpe, uint64_t fc_timeout,
...
void
elbmon_asic_data_store5 (uint8_t core_initiated_recovery,
...
void
elbmon_asic_data_store6 (uint64_t rx_malform_tlp, uint64_t rx_framing_err,
...
pxb_read_pport_err_counters (uint8_t port)
{
    ...
    // Error counters
    pal_reg_rd32w(ELB_ADDR_BASE_PP_PP_OFFSET +
                      ELB_PP_CSR_PORT_P_SAT_P_PORT_CNT_RX_BAD_TLP_BYTE_ADDRESS,
                  (uint32_t *)&rx_bad_tlp, 2);

    pal_reg_rd32w(ELB_ADDR_BASE_PP_PP_OFFSET +
                      ELB_PP_CSR_PORT_P_SAT_P_PORT_CNT_RX_BAD_DLLP_BYTE_ADDRESS,
                  (uint32_t *)&rx_bad_dllp, 2);

    pal_reg_rd32w(
        ELB_ADDR_BASE_PP_PP_OFFSET +
            ELB_PP_CSR_PORT_P_SAT_P_PORT_CNT_RX_NAK_RECEIVED_BYTE_ADDRESS,
        (uint32_t *)&rx_nak_received, 2);

    pal_reg_rd32w(
        ELB_ADDR_BASE_PP_PP_OFFSET +
            ELB_PP_CSR_PORT_P_SAT_P_PORT_CNT_RX_NULLIFIED_BYTE_ADDRESS,
        (uint32_t *)&rx_nullified, 2);

    pal_reg_rd32w(
        ELB_ADDR_BASE_PP_PP_OFFSET +
            ELB_PP_CSR_PORT_P_SAT_P_PORT_CNT_RXBFR_OVERFLOW_BYTE_ADDRESS,
        (uint32_t *)&rxbfr_overflow, 2);

    //
    pal_reg_rd32w(ELB_ADDR_BASE_PP_PP_OFFSET +
                      ELB_PP_CSR_PORT_P_SAT_P_PORT_CNT_TX_NAK_SENT_BYTE_ADDRESS,
                  (uint32_t *)&tx_nak_sent, 2);

    pal_reg_rd32w(
        ELB_ADDR_BASE_PP_PP_OFFSET +
            ELB_PP_CSR_PORT_P_SAT_P_PORT_CNT_TXBUF_ECC_ERR_BYTE_ADDRESS,
        (uint32_t *)&txbuf_ecc_err, 2);

    //
    pal_reg_rd32w(ELB_ADDR_BASE_PP_PP_OFFSET +
                      ELB_PP_CSR_PORT_P_SAT_P_PORT_CNT_FCPE_BYTE_ADDRESS,
                  (uint32_t *)&fcpe, 2);

    pal_reg_rd32w(ELB_ADDR_BASE_PP_PP_OFFSET +
                      ELB_PP_CSR_PORT_P_SAT_P_PORT_CNT_FC_TIMEOUT_BYTE_ADDRESS,
                  (uint32_t *)&fc_timeout, 2);

    pal_reg_rd32w(
        ELB_ADDR_BASE_PP_PP_OFFSET +
            ELB_PP_CSR_PORT_P_SAT_P_PORT_CNT_REPLAY_NUM_ERR_BYTE_ADDRESS,
        (uint32_t *)&replay_num_err, 2);

    pal_reg_rd32w(
        ELB_ADDR_BASE_PP_PP_OFFSET +
            ELB_PP_CSR_PORT_P_SAT_P_PORT_CNT_REPLAY_TIMER_ERR_BYTE_ADDRESS,
        (uint32_t *)&replay_timer_err, 2);

    core_initiated_recovery = pal_reg_rd8(
        ELB_ADDR_BASE_PP_PP_OFFSET +
            ELB_PP_CSR_PORT_P_SAT_P_PORT_CNT_CORE_INITIATED_RECOVERY_BYTE_ADDRESS);

    ltssm_state_changed = pal_reg_rd8(
        ELB_ADDR_BASE_PP_PP_OFFSET +
            ELB_PP_CSR_PORT_P_SAT_P_PORT_CNT_LTSSM_STATE_CHANGED_BYTE_ADDRESS);

    pal_reg_rd32w(
        ELB_ADDR_BASE_PP_PP_OFFSET +
            ELB_PP_CSR_PORT_P_SAT_P_PORT_CNT_8B10B_128B130B_SKP_OS_ERR_BYTE_ADDRESS,
        (uint32_t *)&skp_os_err, 2);

    pal_reg_rd32w(ELB_ADDR_BASE_PP_PP_OFFSET +
                      ELB_PP_CSR_PORT_P_SAT_P_PORT_CNT_DESKEW_ERR_BYTE_ADDRESS,
                  (uint32_t *)&deskew_err, 2);

    pal_reg_rd32w(
        ELB_ADDR_BASE_PP_PP_OFFSET +
            ELB_PP_CSR_PORT_P_SAT_P_PORT_CNT_PHYSTATUS_ERR_BYTE_ADDRESS,
        (uint32_t *)&phystatus_err, 2);

    pal_reg_rd32w(
        ELB_ADDR_BASE_PP_PP_OFFSET +
            ELB_PP_CSR_PORT_C_SAT_C_PORT_CNT_RX_MALFORM_TLP_BYTE_ADDRESS,
        (uint32_t *)&rx_malform_tlp, 2);

    pal_reg_rd32w(
        ELB_ADDR_BASE_PP_PP_OFFSET +
            ELB_PP_CSR_PORT_C_SAT_C_PORT_CNT_RX_FRAMING_ERR_BYTE_ADDRESS,
        (uint32_t *)&rx_framing_err, 2);

    pal_reg_rd32w(ELB_ADDR_BASE_PP_PP_OFFSET +
                      ELB_PP_CSR_PORT_C_SAT_C_PORT_CNT_RX_ECRC_ERR_BYTE_ADDRESS,
                  (uint32_t *)&rx_ecrc_err, 2);

    pal_reg_rd32w(ELB_ADDR_BASE_PP_PP_OFFSET +
                      ELB_PP_CSR_PORT_C_SAT_C_PORT_CNT_RX_NULLIFY_BYTE_ADDRESS,
                  (uint32_t *)&rx_nullify, 2);

    pal_reg_rd32w(
        ELB_ADDR_BASE_PP_PP_OFFSET +
            ELB_PP_CSR_PORT_C_SAT_C_PORT_CNT_RX_WATCHDOG_NULLIFY_BYTE_ADDRESS,
        (uint32_t *)&rx_watchdog_nullify, 2);

    pal_reg_rd32w(ELB_ADDR_BASE_PP_PP_OFFSET +
                      ELB_PP_CSR_PORT_C_SAT_C_PORT_CNT_RX_UNSUPP_BYTE_ADDRESS,
                  (uint32_t *)&rx_unsupp, 2);

    pal_reg_rd32w(
        ELB_ADDR_BASE_PP_PP_OFFSET +
            ELB_PP_CSR_PORT_C_SAT_C_PORT_CNT_RXBUF_ECC_ERR_BYTE_ADDRESS,
        (uint32_t *)&rxbuf_ecc_err, 2);

    pal_reg_rd32w(ELB_ADDR_BASE_PP_PP_OFFSET +
                      ELB_PP_CSR_PORT_C_SAT_C_PORT_CNT_TX_DROP_BYTE_ADDRESS,
                  (uint32_t *)&tx_drop, 2);

    pal_reg_rd32w(
        ELB_ADDR_BASE_PP_PP_OFFSET +
            ELB_PP_CSR_PORT_C_SAT_C_PORT_CNT_TXBFR_OVERFLOW_BYTE_ADDRESS,
        (uint32_t *)&txbfr_overflow, 2);

    // Show
    elbmon_asic_data_store3(rx_bad_tlp, rx_bad_dllp, rx_nak_received,
                            rx_nullified, rxbfr_overflow, tx_nak_sent,
                            txbuf_ecc_err);
    elbmon_asic_data_store4(fcpe, fc_timeout, replay_num_err, replay_timer_err);
    elbmon_asic_data_store5(core_initiated_recovery, ltssm_state_changed,
                            skp_os_err, deskew_err, phystatus_err);
    elbmon_asic_data_store6(rx_malform_tlp, rx_framing_err, rx_ecrc_err,
                            rx_nullify, rx_watchdog_nullify, rx_unsupp,
