...
if __name__ == "__main__":
    ...
    init()
    elb_info_list = pen_addr.get_elba_info_list()
    csr_utils = pen_addr.elb_csr_utils(elb_info_list)
    Registry = csr_utils.update_elb_address(0x10000000, 1 << 18)
    ...
    #
    entry_list = []
    for my_obj in elb_info_list:
        print Template(""" // start ${start} end ${end} logical ${logical} inst ${inst_name} array ${num} ring${ring}""").substitute( start="0x%0.2X" % my_obj.start[0], end="0x%0.2X" %my_obj.end, logical=my_obj.logical_name, inst_name=my_obj.inst_name, num=my_obj.num, ring=my_obj.ring_num)
        if my_obj.ring_num[0] != -1:
            entry_list.append(entryTuple(l_name=my_obj.logical_name, inst_name=my_obj.inst_name, num=my_obj.num, start_addr=my_obj.start[0], size=my_obj.msize, ring=my_obj.ring_num))
    ...
    for entry in entry_list:
        csr_name = chip_prefix + "_"+ csr_utils.get_csr_yaml_name(entry.l_name,entry.inst_name) + "_csr"
