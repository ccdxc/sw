...
void
elba_rss_table_config (uint32_t stage, uint32_t stage_tableid,
                       uint64_t tbl_base, uint64_t pc)
{
    ...
    SDK_TRACE_DEBUG("rss_indir_table stage %u stage-tableid %u table_base %lx\n",
...
static sdk_ret_t
elba_rss_table_config_ex (uint32_t stage, uint32_t stage_tableid,
...
#define ELBA_P4PLUS_HANDLE         "p4plus"
    ...
    SDK_TRACE_DEBUG("rss_indir_table stage %u stage-tableid %u table_base 0x%lx rss_indir_tbl_entry_size %u rss_indir_tbl_size %lu rss_indir_tbl_total_size %lu\n",
...
sdk_ret_t
elba_rss_table_base_pc_get (const char *handle, uint64_t *tbl_base,
                            uint64_t *pc)
{
                    ...
                    uint32_t rss_indir_tbl_entry_size)
{
    ...
    if (rss_indir_tbl_entry_size == 0) {
        rv = elba_rss_table_base_pc_get(handle, &tbl_base, &pc);
        if (rv == SDK_RET_OK)
            elba_rss_table_config(stage, stage_tableid, tbl_base, pc);
    } else
        rv = elba_rss_table_config_ex(stage, stage_tableid,
