...
elba_quiesce_init (void)
{
    ...
    SDK_ASSERT(top_csr.pb.pbc.port_6.dhs_oq_flow_control.get_depth_entry() == MAX_PORT10_FLOW_CTRL_ENTRIES);
    SDK_ASSERT(top_csr.pb.pbc.port_7.dhs_oq_flow_control.get_depth_entry() == MAX_PORT11_FLOW_CTRL_ENTRIES);

    for (int i=0; i < top_csr.pb.pbc.port_6.dhs_oq_flow_control.get_depth_entry(); i++) {
        top_csr.pb.pbc.port_6.dhs_oq_flow_control.entry[i].read();
        port_6_ref_credits[i] = top_csr.pb.pbc.port_6.dhs_oq_flow_control.entry[i].entry().convert_to<uint32_t>();
    }

    for (int i=0; i < top_csr.pb.pbc.port_7.dhs_oq_flow_control.get_depth_entry(); i++) {
        top_csr.pb.pbc.port_7.dhs_oq_flow_control.entry[i].read();
        port_7_ref_credits[i] = top_csr.pb.pbc.port_7.dhs_oq_flow_control.entry[i].entry().convert_to<uint32_t>();
    }
    return SDK_RET_OK;
}
