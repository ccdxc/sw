...
pcieport_handle_pp_intr(pcieport_t *p)
{
    ...
    if (int_pp & PP_INTREG_PERSTN(p->port)) {
        ...
        pal_reg_wr32(PP_(INT_PP_INTREG), PP_INTREG_PERSTN(p->port));
        pciesys_loginfo("pp_intr: int_pp 0x%x\n", int_pp);
#endif
        p->stats.intr_perstn++;
        /*
...
static int
pcieport_check_for_intr(const int port)
{
...
{
    return pcieport_check_for_intr(port);
}

int
pcieport_poll(const int port)
{
    int r = pcieport_check_for_intr(port);
    if (r >= 0) {
        pcieport_t *p = pcieport_get(port);
