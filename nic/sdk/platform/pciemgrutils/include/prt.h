...
#ifndef __PCIEHWUTIL_PRT_H__
#define __PCIEHWUTIL_PRT_H__
...
int prt_is_valid(const prt_t *prt);
u_int32_t prt_type(const prt_t *prt);
char *prt_type_str(const int type);
u_int32_t prt_size_encode(const u_int64_t size);
u_int64_t prt_size_decode(const u_int32_t size_enc);

#define PRT_RESF_NONE           0x0     /* no flags */
#define PRT_RESF_NOTIFY         0x1     /* notify on access */
#define PRT_RESF_INDIRECT       0x2     /* indirect on access */
#define PRT_RESF_PMVDIS         0x4     /* disable PMV check */
