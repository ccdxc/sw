...
sub getNextTopAddressmap {
    my $self = shift;

    my (@hierarchicalPrefixList, $backdoorPath);
    my ($addrmapHashRef, $parentAddrmapHashRef, $relAddrBigInt);
    ...
    if ($nextAddressmapIndex >= $self->getTopLevelAddressmapCount()-1) {
        ...
        $designDBRef->set_topAddressmapName($addrmapHashRef->{'name'});
        $designDBRef->set_topAddressmapHashRefNode($addrmapHashRef);
        my ($verilogFormat, $bigIntAddr) = $self->getVerilogBaseAndValue($addrmapHashRef->{'byteAddress'});
        my @prefixList = ();
        push (@hierarchicalPrefixList, $addrmapHashRef->{'hierarchicalIdentifier'});
        ($verilogFormat,
         $relAddrBigInt) = $self->getVerilogBaseAndValue($addrmapHashRef->{'addressmapRelativeByteAddress'});
        my $arrayIndex = undef;
...
sub getNextTopAddressmapNode {
    my $self = shift;
    # Deprecated! Use getNextTopAddressmap().
    return $self->getNextTopAddressmap();
}
...
sub _isExternalHashInHierarchy {
    ...
    while (defined($parentHashRef)) {
        if (defined($external = $parentHashRef->{'external'}) &&
            ...
            return 0 if ($parentHashRef->{'nodeType'} eq 'addressmap');
...
sub _getHashRefForName {
    ...
    if (defined($designDBRef->{'component'})) {
        ...
        if ($targetName =~ /\:\:/) {
            ...
            if (($self->getTopLevelAddressmapCount() > 1) &&
                (@levelNameList == 1)) { # target may be a top-level address map
                $parentHashRef = $self->_findAddressmap($targetName);
            } else {
...
sub _getRegHashList {
    my ($self, $addrmapHashRef) = @_;
    return undef if (!defined($addrmapHashRef->{'definition'}));
    for(my $count = 0; $count < @{$addrmapHashRef->{'definition'}}; $count++) {
	if(defined($addrmapHashRef->{'definition'}->[$count]->{'registers'})) {
...

{
	...
	if(defined($arrayIndex)) {
	    ...
	    $self->set_arrayIndex([@{$arrayIndex}]);
            # And, include array index information with identifier.
            my $tmpId = $designDBRef->_getArrayElementName($addrmapHashRef->{'name'}, $arrayIndex);
            $self->set_identifierWithIndex($tmpId);
	}
        if (defined($addrmapHashRef->{'hdlPathInfo'})) {
            my $hdlPathObj = csrHdlPath->new($addrmapHashRef->{'hdlPathInfo'});
            $self->set_hdlPathObj($hdlPathObj);
        }
    ...
    sub getHdlPath {
        my $self = shift;
        return undef if (!defined($self->get_nodeHashRef()->{'hdlPathInfo'}));

        my ($arrayIndex);
        my $hdlPathObj = $self->getHdlPathObj();
        my $hdlPathSlices = $hdlPathObj->getHdlPathSlices();
        my $hdlPath = $hdlPathSlices->[0]->getSliceName();
        if ($hdlPathObj->hasHdlPathPrefix()) {
            $hdlPath = join('.', ($hdlPathObj->getHdlPathPrefix(), $hdlPath));
        }
        if (defined($arrayIndex = $self->getArrayIndex())) {
            $hdlPath = $designDBRef->_getArrayElementName($hdlPath,
                                                          $arrayIndex);
            my $objType = $self->getNodeType();
            if ($viewName eq 'sv') {
                $hdlPath =~ s/(\[\d+\])+//g;
            } else {
                $hdlPath =~ s/\[/_/g;
    ...
    sub hasPortType {
        ...
        if ($portType =~ /external/) {
            $portListRef = $self->getExternalPorts();
        } else {
    ...
    sub isBroadcastAddressmap {
	my $self = shift;
	return 1 if (defined($self->get_broadcastMembers()));
	return 0;
    }
    sub getBroadcastMemberAddressmaps {
	my $self = shift;
	...
	foreach my $broadcastItem (@{$broadcastMemberList}) {
            my $addrmap = $designDBRef->_findAddressmap($broadcastItem);
	    push (@addrmapList, $addrmap);
	}
	return \@addrmapList;
    }
    ...
    sub getResetClassList {
        ...
        while (my $nextReg = $self->getNextRegister()) {
	...
	my ($verilogFormat, $relAddr, $arrayIndex);
        ...
        my ($addrmapObj, $parentName, $bogusAddr, $backdoorPath);
	...
	my @hierLevelNameList = split('\.', $self->get_hierarchicalNameDotted());
        ...
        my ($topLevelHashRef, $bigIntAddr) = $designDBRef->_getHashRefForName($levelNameList[0]);
	...
	my($self, $portDir) = @_;
        # This is the old API for getting ports of an address map.
        # It calls the new API - getPortsArray().
        return ($self->getPortsArray($portDir));
    }
    sub getPortsArray {
        my($self, $portDir) = @_;
        my (@portList, @nodeList);

        # replaces getInterfacePorts()
        # for ports, always expand the arrays.
        my $arrayFlag = $self->getExpandArraysFlag();
        $self->setExpandArraysFlag(1) if ($viewName ne 'sv');
        $self->do_getPortsArray(\@portList, $portDir);
        # remove duplicates
        my @newPortList;
        $self->removeDuplicatePorts(\@portList, \@newPortList);
        $self->setExpandArraysFlag($arrayFlag);
        return \@newPortList;
    }
    sub do_getPortsArray {
        my ($self, $portList, $portDir) = @_;

        my @nodeList;
        $self->saveAddrmapInfoContext();
        while(my $nextObj = $self->getNextDesignNode()) {
            next if ($nextObj->isAddressmapNode() && $nextObj->isBroadcastAddressmap());
            push(@nodeList, $nextObj);
        }
        $self->restoreAddrmapInfoContext();
        foreach my $nextObj (@nodeList) {
            my $tmpPortListRef = $nextObj->getPortList($portDir);
    ...
    sub getPortList {
        ...
        my ($portListRef, @portList);
        my $parentNode = $self->getParentNode();
        return undef if (defined($parentNode) && $parentNode->isExternal() &&
                         !$self->getHierarchicalTBFlag());
        if ($self->getNodeType() eq 'addressmapInstance') {
            $portListRef = $self->getExternalPorts();
        } else {
            $portListRef = $self->getPorts();
        }
    ...
    sub hasWideRegister {
        ...
        while(my $nextObj = $addrmapNode->getNextRegister()) {
    ...
    sub hasWideAtomicRegister {
        my $self = shift;
        $self->saveAddrmapInfoContext();
        while(my $nextObj = $self->getNextRegister()) {
            if($nextObj->isAtomic()) {
                $self->restoreAddrmapInfoContext();
                return $nextObj;
            }
...
sub getNextDesignNode {
    ...
    # get the objects in the calling addressmap. Recursively descends the hierarchy.
    # It returns the calling object and all the children, recursively.

    my @prefixList = ();
    my ($relAddrBigInt, $parentAddrmapHashRef, $nextObj);
    my ($prefixListRef, $currAddrmapInfo, $verilogFormat, $backdoorSlice);
    my ($addrmapObj, $bigIntAddr, $addrmapHashRef, @hierarchicalPrefixList);
    ...
    if(@visitedAddrmapsList == 0) {
        ...
        ($verilogFormat, $bigIntAddr) = $self->getVerilogBaseAndValue($addrmapHashRef->{'byteAddress'});
        ($verilogFormat,
         $relAddrBigInt) = $self->getVerilogBaseAndValue($addrmapHashRef->{'addressmapRelativeByteAddress'});
        push(@hierarchicalPrefixList, $addrmapHashRef->{'hierarchicalIdentifier'});
    ...
    if (($currNodeType eq 'addressmap') || ($currNodeType eq 'addressmapInstance') ||
        ...
        if ($self->moreRegistersAtThisLevel($currAddrmapInfo, $addrmapHashRef)) {
            $nextObj = $self->getRegisterObj($currAddrmapInfo, $addrmapHashRef);
            ...
            $nextObj = $self->getUnionObj($currAddrmapInfo, $addrmapHashRef);
            return $nextObj;
        } elsif (($currNodeType ne 'memory') && $self->moreMemoriesAtThisLevel($currAddrmapInfo,
                                                                               $addrmapHashRef)) {
            $nextObj = $self->getMemoryObj($currAddrmapInfo, $addrmapHashRef);
            $currAddrmapInfo = $visitedAddrmapsList[@visitedAddrmapsList-1];
            if (($currAddrmapInfo->{'currNode'})->{'nodeType'} ne 'wideMemory') {
            ...
            return $nextObj;
                                                                                ...
                                                                                $addrmapHashRef)) {
            $nextObj = $self->getAddrmapInstObj($currAddrmapInfo, $addrmapHashRef);
            return $nextObj;
        } elsif ($self->moreGroupsAtThisLevel($currAddrmapInfo, $addrmapHashRef)) {
	    $nextObj = $self->getGroupObj($currAddrmapInfo, $addrmapHashRef);
	    return $nextObj;
        } else {
        ...
        if (($currAddrmapInfo->{'currAddrmapSubRegisterIndex'} >= @{$subRegHashListRef}) &&
            ...
            $nextObj = $self->getSubRegObj($currAddrmapInfo, $addrmapHashRef);
        ...
        if ($self->moreRegistersAtThisLevel($currAddrmapInfo, $memHashRef)) {
            $nextObj = $self->getRegisterObj($currAddrmapInfo, $memHashRef);
            # We may've just processed a wide register (and pushed it in the
            # LIFO. Wide register accounting is done somewhere else.
            $currAddrmapInfo = $visitedAddrmapsList[@visitedAddrmapsList-1];
            if(($currAddrmapInfo->{'currNode'})->{'nodeType'} ne 'wideRegister') {
            ...
            $nextObj = $self->getGroupObj($currAddrmapInfo, $memHashRef);
            $currAddrmapInfo = $visitedAddrmapsList[@visitedAddrmapsList-1];
            return $nextObj;
            ...
            $nextObj = $self->getSubMemObj($currAddrmapInfo, $addrmapHashRef);
...
sub getNextChild {
    ...
    my @prefixList = ();
    my (@hierarchicalPrefixList, $backdoorSlice);
    my ($relAddrBigInt, $parentAddrmapHashRef, $nextObj);
    my ($prefixListRef, $regHashRef, $currAddrmapInfo, $verilogFormat);
...
sub getChildren {
    my $self = shift;
    # returns a reference to a list of children for the calling object (recursive).
    my @objList;
    while (my $nextObj = $self->getNextChild()) {
        push (@objList, $nextObj);
    }
    return \@objList;
}
sub getNumChildren {
    my $self = shift;
    my $childArray = $self->getChildren();
    return @{$childArray};
}
sub getNextDesignNode_NoDepth {
    my $self = shift;

    # get the objects in the calling addressmap without descending the hierarchy.
    # It returns the calling object and all the children at the calling object's level.
    ...
    if(@visitedAddrmapsList == 0) {
        ...
        $addrmapHashRef = $self->get_nodeHashRef();
        ($verilogFormat, $bigIntAddr) = $self->getVerilogBaseAndValue($addrmapHashRef->{'byteAddress'});
        ($verilogFormat, $relAddrBigInt) = $self->getVerilogBaseAndValue($addrmapHashRef->{'addressmapRelativeByteAddress'});
    ...
    if (($currNodeType eq 'addressmap') || ($currNodeType eq 'addressmapInstance') ||
       ...
       if ($self->moreRegistersAtThisLevel($currAddrmapInfo, $addrmapHashRef)) {
            ...
            if(@visitedAddrmapsList == 0) {
                ...
                $nextObj = $self->getNextDesignNode_NoDepth();
                return $nextObj;
            }
        }
    }
...
sub getNextChild_Shallow {
    my $self = shift;

    # returns a reference to an array of objects at this level. Does not descend the hierarchy.
    ...
    if (($currNodeType eq 'addressmap') || ($currNodeType eq 'addressmapInstance') ||
        ...
        if ($self->moreRegistersAtThisLevel($currAddrmapInfo, $addrmapHashRef)) {
            ...
            if(@visitedAddrmapsList == 0) {
                ...
                $nextObj = $self->getNextChild_Shallow();
                return $nextObj;
            }
        }
    }
...
sub getChildren_Shallow {
    my $self = shift;
    # returns a reference to a list of children at this object level (recursively).
    my @objList;
    while (my $nextObj = $self->getNextChild_Shallow()) {
        push (@objList, $nextObj);
    }
    return \@objList;
}
sub getNumChildren_Shallow {
    my $self = shift;
    my $childArray = $self->getChildren_Shallow();
    return @{$childArray};
}
...
sub getAddrmapInstObj {
    my ($self,
        $currAddrmapInfo,
        $addrmapHashRef) = @_;

    my ($addrmapRelByteAddr, @hierarchicalPrefixList);
    my ($bigIntAddr, @prefixList, $tmpAddrmapInfo, $backdoorPath);
    my ($verilogFormat, $newAddrmapInfo, $newAddrmapHashRef);

    my $instanceHashListRef = $designDBRef->_getInstanceHashList($addrmapHashRef);
    my $instIndex = $currAddrmapInfo->{'currAddrmapInstanceIndex'};
    my $currInstHashRef = $instanceHashListRef->[$instIndex];
    $currAddrmapInfo->{'currInstanceHashRef'} = $currInstHashRef;
    my $instHashRef = $designDBRef->_findAddressmapForInstanceHash($currInstHashRef);

    if (defined($instHashRef->{'arrayDimensionInfo'}) && $self->getExpandArraysFlag()) {
        my $addrmapObj = $self->getNextAddrmapInstArrayElement($currAddrmapInfo,
                                                               $addrmapHashRef,
                                                               $instHashRef);
        return $addrmapObj;
    } else {
        ($verilogFormat, $bigIntAddr) = $self->getVerilogBaseAndValue($instHashRef->{'byteOffset'});
	...
	if (defined($currInstHashRef->{'parameters'})) {
        ...
        my $addrmapObj = csrPerlAPIModule->new($newAddrmapHashRef,
                                               $arrayIndex,
                                               $addrmapHashRef,
                                               $bigIntAddr,
                                               $addrmapRelByteAddr,
...
sub getNextAddrmapInstArrayElement {
    my ($self,
        $currAddrmapInfo,
        $addrmapHashRef,
        $instHashRef) = @_;
    ...
    my ($instIndex, @addrmapPrefixList, $relAddr, $backdoorPath);
    ...
    my $bigIntAddr = Math::BigInt->new($instHashRef->{'arrayElementByteSize'});
...
sub getNextRegisterNode {
    my $self = shift;
    # Deprecated! Use getNextRegister().
    return $self->getNextRegister();
}
sub getNextMemory {
    my $self = shift;
    my $nextObj = $self->getNextChild();
    if(defined($nextObj)) {
        my $nodeType = $nextObj->getNodeType();
        if (($nodeType eq 'memory') ||
            ($includeWideMemoriesFlag && ($nodeType eq 'wideMemory'))) {
            return $nextObj;
        } else {
            $self->getNextMemory();
        }
...
sub getNextAddressmap_old {
    ...
    my @prefixList = ();
    my (@hierarchicalPrefixList, $backdoorSlice);
    my ($instIndex, $instHashRef, $parentAddrmapHashRef, $relAddrBigInt);
...
sub getNextAddressmap {
    ...
    my @prefixList = ();
    my (@hierarchicalPrefixList, $currNode);
    my ($parentAddrmapHashRef, $relAddrBigInt);
    my ($nextObj, $prefixListRef, $currAddrmapInfo);
    my ($verilogFormat, $addrmapObj, $bigIntAddr, $addrmapHashRef);
	...
	$currAddrmapInfo->{'prefixListRef'} = \@prefixList;
	$currAddrmapInfo->{'hierarchicalPrefixListRef'} = \@hierarchicalPrefixList;
	$currAddrmapInfo->{'currAddrmapObj'} = $self;
	my $refNameHashListRef = [];
	$designDBRef->_genRefNameList($addrmapHashRef, $refNameHashListRef);
	$currAddrmapInfo->{'refNameHashListRef'} = $refNameHashListRef;
	$self->setSkipExternalRegistersFlag($skipExtRegsFlag);
        $currNode = $addrmapHashRef;
	push(@visitedAddrmapsList, $currAddrmapInfo);
    } else {
        $currAddrmapInfo = $visitedAddrmapsList[@visitedAddrmapsList-1];
        $addrmapHashRef = $currAddrmapInfo->{'currAddrmapHashRef'};
        $prefixListRef = $currAddrmapInfo->{'prefixListRef'};
        $currNode = $currAddrmapInfo->{'currNode'};
    }
    my $currNodeType = $currNode->{'nodeType'};
...
sub getNextAddressmapNode {
    ...
    # Deprecated! Use getNextAddressmap().
    return $self->getNextAddressmap();
}
...
sub getRegisterCount {
    ...
    while(defined($nextObj = $self->getNextRegister())) {
	$regCount++;
    }
...

{
	...
	if(@{$prefixListRef} > 0) {
        ...
        if(defined($arrayIndex)) {
            # copy the array indices before calling updateGroupAccounting;
            $self->set_arrayIndex([@{$arrayIndex}]);
            # And, include array index information with the identifier.
            my $tmpId = $designDBRef->_getArrayElementName($groupHashRef->{'name'}, $arrayIndex);
            $self->set_identifierWithIndex($tmpId);
        }
        $self->assignAddressInfo($addrmapAddress,
    ...
    sub getNextDesignNode {
        ...
        my @prefixList = ();
        my ($currAddrmapInfo, $addrmapHashRef, $bigIntAddr);
        my ($nextObj, $relAddrBigInt);
        ...
        if(@visitedAddrmapsList == 0) {
            ...
            my $arrayIndex = undef;
            $addrmapHashRef = $self->get_nodeHashRef();
            my ($verilogFormat, $bigIntAddr) = $self->getVerilogBaseAndValue($self->getByteAddress());
            ($verilogFormat,
             $relAddrBigInt) = $self->getVerilogBaseAndValue($addrmapHashRef->{'addressmapRelativeByteAddress'});
            my $hierId = $self->getHierarchicalIdentifier();
    ...
    sub getNextChild {
        ...
        # Iterates thru the children of a group node, hierarchically.
        # Returns a reference to an array of object.

        my @prefixList = ();
        my ($currAddrmapInfo, $addrmapHashRef, $bigIntAddr);
        my ($subRegHashListRef, $nextObj, $relAddrBigInt);
    ...
    sub getNextDesignNode_NoDepth {
        ...
        my @prefixList = ();
        my ($parentAddrmapHashRef, $bigIntAddr);
        my ($currAddrmapInfo, $addrmapHashRef, $relAddrBigInt);
    ...
    sub getNextChild_Shallow {
        ...
        # Iterate thru the children of a group node without descending the hierarchy.
...
#                }
    ...
    sub getPortList {
        ...
        my $parentNode = $self->getParentNode();
        return undef if ($self->isExternal() &&
                         (defined($parentNode = $self->getParentNode()) && 
                         ($parentNode->isExternal() || ($parentNode->getNodeType() eq 'group'))));
        if (defined(my $portListRef = $self->getPorts())) {
            foreach my $portObj (@{$portListRef}) {
                push(@portList, $portObj) if($direction eq $portObj->getPortDirection());
            }
        }
        my $extPortListRef = $self->getExternalPorts();
        return \@portList if (!defined($extPortListRef));
        foreach my $portObj (@{$extPortListRef}) {
            push(@portList, $portObj) if ($direction eq $portObj->getPortDirection());
        }
        return \@portList;
    }
    ...
    sub getParentNode {
        ...
        my $count = 0;
	my ($verilogFormat, $relAddrBigInt, $arrayIndex, $bigIntAddr);
        my ($busProtocol, $inputPipelineStage, $outputPipelineStage);
        my (@prefixList, $parentHashRef, $parentOfParentHashRef, $backdoorPath);
...

{
	...
	if(defined($arrayIndex)) {
	    ...
	    $self->set_arrayIndex([@{$arrayIndex}]);
            # And, include array index information with the identifier.
            my $tmpId = $designDBRef->_getArrayElementName($unionHashRef->{'name'}, $arrayIndex);
            $self->set_identifierWithIndex($tmpId);
	}
	$self->assignAddressInfo($addrmapAddress,
...
#	}
	...
	my($self, $direction) = @_;
        ...
        # skip ports when userdefined group within another union, or
        # when userdefined union within a userdefined address map.
        ...
        if (defined(my $extPortListRef = $self->getExternalPorts())) {
            foreach my $portObj (@{$extPortListRef}) {
                push(@portList, $portObj) if ($direction eq $portObj->getPortDirection());
            }
        }
...

{
	...
	if (defined($regHashRef->{'aliasOf'})) {
	    ...
	    # copy the array indices before updateRegisterAccounting;
            $self->set_arrayIndex([@{$arrayIndex}]);
            # And, includle array index infromation with identifier.
            my $tmpRegId = $designDBRef->_getArrayElementName($regHashRef->{'name'}, $arrayIndex);
            $self->set_identifierWithIndex($tmpRegId);
        }
	...
	$self->setBusProtocol($regHashRef,
			      $parentHashRef,
			      $busProtocol);
        if (defined($regHashRef->{'indirectAddress'})) {
            my $indirectAddr = csrIndirectAddress->new($regHashRef->{'indirectAddress'},
                                                       $self);
            $self->set_indirectAddress($indirectAddr, $self);
        }
        if (defined($regHashRef->{'indirectAddressmap'})) {
            my ($relAddrBigInt, @hierarchicalPrefixList);
            my $addrmapHashRef = $designDBRef->_findAddressmap($regHashRef->{'indirectAddressmap'}->{'name'});
            my ($verilogFormat, $bigIntAddr) = $self->getVerilogBaseAndValue($addrmapHashRef->{'byteAddress'});
            my @prefixList = ();
            push (@hierarchicalPrefixList, $addrmapHashRef->{'hierarchicalIdentifier'});
            ($verilogFormat,
            ...
            my $addrmapObj = csrPerlAPIModule->new($addrmapHashRef,
                                                   ...
                                                   \@prefixList,
                                                   \@hierarchicalPrefixList);
            $self->set_indirectAddressmap($addrmapObj);
        }
	...
	if (($viewName ne 'sv') &&
	    ...
	    my $portListRef = [];
	    my $parentNode = $self->getParentNode();
	    my $portList = $parentNode->getExternalPorts();
	    ...
	    $self->set_externalBusPorts($portListRef);
            ...
            my $portListRef = [];
            my $parentObj = $self->getParentNode();
            my $portList = $parentObj->getExternalPorts();
            foreach my $nextP (@{$portList}) {
                if ($nextP->getPortType() eq 'externalWriteAccessPorts') {
                    my $multiPort = $nextP->get_multiPortList();
                    my $subMemIndex = $currAddrmapInfo->{'currAddrmapSubMemoryIndex'};
                    my $subMems = $parentObj->get_nodeHashRef()->{'subMemories'};
                    my $subMemHashRef = $subMems->[$subMemIndex];
                    if ($subMemHashRef->{'parentWidth'} > 0) {
...

{
	...
	$self->set_nextSubRegisterIndex(0) if($self->getNodeType() eq 'wideRegister');
        ...
        $self->addBitfields($regHashRef,
                            $tmpPrefixListRef,
                            $hierPrefixListRef);
        if (defined($regHashRef->{'hdlPathInfo'})) {
            my $hdlPathObj = csrHdlPath->new($regHashRef->{'hdlPathInfo'});
            $self->set_hdlPathObj($hdlPathObj);
        }
    ...
    sub getFieldMask {
        my $self = shift;
        my $bitfields = $self->getBitfields();
        return undef if (!defined($bitfields));
        my $mask;
        for (my $index=0; $index < @{$bitfields}; $index++) {
            my $nextField = $bitfields->[$index];
            my $fieldMask = Math::BigInt->new(2);
            $fieldMask->bpow($nextField->getWidth())->bdec();
            if(!defined($mask)) {
                $mask = Math::BigInt->new($fieldMask);
                $mask->blsft($nextField->getLsbOffsetFromLsb());
            } else {
                $fieldMask->blsft($nextField->getLsbOffsetFromLsb());
                $mask->bior($fieldMask);
            }
        }
        return $mask;
    ...
    sub hasAliasedRegister {
        ...
        $self->saveAddrmapInfoContext();
        while (my $nextRegObj = $addrmapNode->getNextRegister()) {
    ...
    sub getHdlPath {
        ...
        my $hdlPathSlices = $hdlPathObj->getHdlPathSlices();
        my $hdlPath = $hdlPathSlices->[0]->getSliceName();
        # hdlPath for a register can be derived from the path slice info of the
        # fields (generated RTL), or from the user-specified path info provided
        # by the user in the register object.
    ...
    sub getFullHdlPath {
        ...
        my ($regObj, @hdlPath);
        if ($self->isSubRegisterNode()) { 
            $regObj = $self->getParentNode();
            return undef if (!$regObj->hasHdlPath());
        } else {
            $regObj = $self;
            return undef if (!$regObj->hasHdlPath());
        }
        my $fields = $regObj->getBitfields();
        return undef if (!defined($fields));
        my $bitNumbering = $self->getBitNumbering();
        my $fieldCount = @{$fields} - 1;
        for (my $fieldIndex = ($bitNumbering eq 'lsb_0') ? $fieldCount : 0;
             ($bitNumbering eq 'lsb_0') ? $fieldIndex >= 0 : $fieldIndex <= $fieldCount;
             ($bitNumbering eq 'lsb_0') ? $fieldIndex-- : $fieldIndex++) {
            my $field = $fields->[$fieldIndex];
            my $fHdlPath = $field->getFullHdlPath();
            push(@hdlPath, $fHdlPath);
        }
        return \@hdlPath;
    }

    sub getIndirectAddressmap {
        my $self = shift;
        return undef if (!defined($self->get_indirectAddressmap));
        return $self->get_indirectAddressmap();
    }
    sub getIndirectIndexField {
        my $self = shift;
        return undef if (!$self->isIndirectDataRegister());
        my $indirectAddrObj = $self->get_indirectAddress();
        return $indirectAddrObj->get_indirectIndexField();
    }
    sub getIndirectIndexRegister {
        my $self = shift;
        return undef if (!$self->isIndirectDataRegister());
        my $indirectAddrObj = $self->get_indirectAddress();
        return $indirectAddrObj->get_indirectIndexRegister();
    }
    ...
    sub getSubRegistersList {
        ...
        my ($verilogFormat, $offset, $relAddrBigInt, @hierNameList);
        my ($parentHashRef, $wideRegHashRef, $parentName, $backdoorPath);
        my (@subRegObjectList, @aliasPrefixList, $subRegObj, @prefixList);

        my $wideRegName = $self->getHierarchicalIdentifier();
        my @levelNameList = split('\.', $wideRegName);
        my @wideRegPrefixList = split('\.', $wideRegName);
...
#	}
    ...
    sub getNextRegister {
        ...
        if($self->getNodeType ne 'wideRegister') {
            $errMsg = 'getNextRegister() is not a method of ' . ref($self) . ' class type.' . "\n";
            $self->_printError($errMsg);
            exit(1);
        }
    ...
    sub addBitfields {
        ...
        my ($fieldHashListRef, @hierNameList, $fieldIndex);
        my ($tmpFieldName, @hierarchicalPrefixList, $backdoorPath);
        ...
        for (my $hashIndex = ($bitNumbering eq 'lsb_0') ? $fieldCount : 0;
			...
			push (@$fieldObjList, $fieldObj);
                    ...
                    $fieldIndex = $designDBRef->_getArrayMaxIndices($fieldHash->{'arrayDimensionInfo'});
                    @hierarchicalPrefixList = ();
    ...
    sub getRegPortList {
        ...
        my $currPortIndex = 0;
        while (my $portObj = $self->getNextPortNode($regPortListRef,
                                                    \$currPortIndex)) {
            if(defined(my $portDir = $portObj->getPortDirection())) {
                if ($self->isExternal() && defined(my $portType = $portObj->getPortType())) {
                ...
                if (defined(my $sourceType = $portObj->getSourceType())) {
	...
	return \@portListRef if (!defined($extPortListRef));
        foreach my $portObj (@{$extPortListRef}) {
            my $portDir = $portObj->getPortDirection();
            push(@portListRef, $portObj) if($portDir eq $direction);
        }
        return \@portListRef;
    }
    sub getPortList {
        ...
        return undef if (($self->childOfExternalObject() && !$self->getHierarchicalTBFlag()) ||
                         ($self->isBroadcastRegister() && $self->isExternal()));
        my $portListRef = $self->getRegPortList($direction);
	while (my $fieldObj = $self->getNextFieldNode()) {
            next if(!defined($fieldObj->getPorts()));
            if(defined(my $tmpPortList = $fieldObj->getPortList($direction))) {
    ...
    sub getNextFieldNode {
        ...
        # Deprecated! Use getNextField().
        return $self->getNextField();
    }
    ...
    sub regHasOutputPort {
        my $self = shift;
        my $portsListRef;
        if(defined($portsListRef = $self->getRegPortList('output'))) {
    ...
    sub getParentNode {
        ...
        my ($relAddrBigInt, $verilogFormat, $bigIntOffset);
        my (@aliasPrefixList, $parentObject, $bogusAddr, $backdoorPath);
        my ($parentName, $parentHashRef, $parentOfParentHashRef);
...

{
	...
	my ($verilogFormat, $relAddr, $aliasPrefixListRef, $expandedAliasName);
        my $self = $_[0]->SUPER::new($memHashRef,
				     ...
				     $aliasPrefixListRef,
				     $expandedAliasName);
        if (defined($memHashRef->{'hdlPathInfo'})) {
            my $hdlPathObj = csrHdlPath->new($memHashRef->{'hdlPathInfo'});
            $self->set_hdlPathObj($hdlPathObj);
        }
        $self->set_nextExternalBusPortIndex(-1);
        return $self;
    ...
    sub getFullHdlPath {
        ...
        my ($hdlPath);
        return undef if (!$self->hasHdlPath());
        my $hdlPathObj = $self->getHdlPathObj();
        my $parentObj = $self->getParentNode();
        my $hierId = $self->getHierarchicalIdentifier();
        my $memHdlPath = $self->getHdlPath();
        my $hdlPathStyle = $hdlPathObj->getHdlPathStyle();
        if ($hdlPathStyle eq 'rtlVerilog') {
            $hdlPath = $self->getFullHdlPath_RTLVerilog($memHdlPath,
                                                        $hierId);
        } elsif ($hdlPathStyle eq 'hierarchicalIdentifier') {
            $hdlPath = $self->getFullHdlPath_HierarchicalIdentifier($memHdlPath,
                                                                    $hierId);
        } elsif ($hdlPathStyle eq 'identifier') {
            $hdlPath = $self->getFullHdlPath_Identifier($memHdlPath,
                                                        $hierId);
        } elsif ($hdlPathStyle eq 'addressmapRelative') {
            $hdlPath = $self->getFullHdlPath_AddressmapRelative($memHdlPath,
                                                                $hierId);
        } else {
            my $i=0;
            while (my($pack, $file, $line, $subname, $hasargs, $wantarray) = caller($i++)) {
            ...
            my $errMsg = "\thdlPathStyle : " . $hdlPathStyle . ' not implemeneted';
            $self->_printError($errMsg);
            exit (1);
        }
        return $hdlPath;
    }
    sub getFullHdlPath_RTLVerilog {
        my ($self,
            $memHdlPath,
            $hierId) = @_;
        my $i=0;
        ...
        my $errMsg = "\t rtl-verilog hdlPathStyle not implemeneted";
        $self->_printError($errMsg);
        exit (1);
    }
    sub getFullHdlPath_HierarchicalIdentifier {
        my ($self,
            $memHdlPath,
            $hierId) = @_;
        ...
        my $errMsg = "\t hierarchical-identifier hdlPathStyle not implemeneted";
        $self->_printError($errMsg);
        exit (1);
    }
    sub getFullHdlPath_Identifier {
        my ($self,
            $memHdlPath,
            $hierId) = @_;
        ...
        push (@hdlPath, $memHdlPath);
        return \@hdlPath;
    }
    sub getFullHdlPath_AddressmapRelative {
        my ($self,
            $memHdlPath,
            $hierId) = @_;
        ...
        my $errMsg = "\t addressmap-relative hdlPathStyle not implemeneted";
        $self->_printError($errMsg);
        exit (1);
    }
    sub getNextDesignNode {
        ...
        if(@visitedAddrmapsList == 0) {
            ...
            ($verilogFormat,
             $relAddrBigInt) = $self->getVerilogBaseAndValue($addrmapHashRef->{'addressmapRelativeByteAddress'});
            $bigIntOffset = $self->getByteOffset();
            my $hierId = $self->getHierarchicalIdentifier();
            my @hierarchicalPrefixList = split('\.', $hierId);
            my @prefixList = split('\.', $hierId);
		...
		return $nextObj;
                ...
                if(@visitedAddrmapsList == 0) {
                    ...
                    if ($currNodeType eq 'group') {
                        $self->updateGroupAccounting($currAddrmapInfo);
                    } elsif (($currNodeType eq 'memory') || ($currNodeType eq 'wideMemory')) {
                        $self->updateMemoryAccounting($currAddrmapInfo);
                    } else {
    ...
    sub getNextChild {
        ...
        # Iterates thru the virtual registers of a memory node, if they exist.
        # Returns a reference to an list of objects.

        my @prefixList = ();
        my ($currAddrmapInfo, $addrmapHashRef, $bigIntOffset);
        my ($nextObj, $relAddrBigInt, $verilogFormat);
        return if ($self->isSubMemoryNode());
        ...
        if(@visitedAddrmapsList == 0) {
            ...
            my $arrayIndex = undef;
            $addrmapHashRef = $self->get_nodeHashRef();
            my $bigIntAddr = $self->getByteAddress();
            ($verilogFormat,
             $relAddrBigInt) = $self->getVerilogBaseAndValue($addrmapHashRef->{'addressmapRelativeByteAddress'});
            $bigIntOffset = $self->getByteOffset();
            my $hierId = $self->getHierarchicalIdentifier();
    ...
    sub getPortList {
        ...
        my (@portListRef, $extPortListRef);

        return undef if (!defined($extPortListRef = $self->getExternalPorts()));
        return undef if ($self->childOfExternalObject() && !$self->getHierarchicalTBFlag());
    ...
    sub getSubMemoriesList {
        ...
        my ($wideMemName, $subMemObj, @prefixList, $bigIntAddr);
    ...
    sub getParentNode {
        ...
        my $count = 0;
	my ($bigIntOffset, $arrayIndex, $bigIntAddr);
	my ($verilogFormat, $relAddrBigInt, @hiearchicalPrefixList);
        my ($busProtocol, $inputPipelineStage, $outputPipelineStage);
        my (@prefixList, $parentHashRef, $parentOfParentHashRef);
...

{
    ...
    sub new {
        ...
        my ($intObj, @portType, $resetTiming);
        ...
        if (($parentHashRef->{'declaredExternal'} eq 'false') &&
            ($parentHashRef->{'nodeType'} eq 'wideRegister') &&
            ($fieldHash->{'fieldType'} eq 'status') && ($parentHashRef->{'virtualRegister'} eq 'false')) {
            my $portListRef = $fieldHash->{'ports'};
            # virtual register fields don't have ports
            for(my $count = 0; $count < @$portListRef; $count++) {
        ...
        if (defined($fieldHash->{'aliasOf'})) {
            my $aliasPrefixListRef = $self->expandAliasArrayMembers($fieldHash,
                                                                    $prefixListRef);
            my $aliasHierId = join('.', @$aliasPrefixListRef);
            my $expandedAliasName = join('_', @$aliasPrefixListRef);
            if ($viewName ne 'sv') {
                $expandedAliasName =~ s/\[/_/g;
                $expandedAliasName =~ s/\]//g;
            } else {
        ...
        if(exists($fieldHash->{'segmentOf'})) {
            ...
            $self->do_setFieldSegmentOf($fieldHash,
                                        $prefixListRef);
        }
        ...
        if (exists($fieldHash->{'resetValueInfo'})) {
            $self->do_setFieldResetValueNode($fieldHash,
                                             $designDBRef,
                                             $fieldIndex);
        }
        ...
        if (defined($fieldHash->{'hdlPathInfo'})) {
            my $hdlPathObj = csrHdlPath->new($fieldHash->{'hdlPathInfo'});
            $self->set_hdlPathObj($hdlPathObj);
        }
        if (defined($fieldIndex)) {
            # set arrayIndex by copying the fieldIndex array
            $self->set_arrayIndex([@{$fieldIndex}]);
            my $tmpFieldName = $designDBRef->_getArrayElementName($fieldHash->{'name'},
                                                                  $fieldIndex);
            $self->set_identifierWithIndex($tmpFieldName);
            my $multiplier = $designDBRef->_getMultiplierFromArrayIndex($fieldIndex,
                                                                        $fieldHash->{'arrayDimensionInfo'});
            my $lsbOffsetFromLsb = $self->getLsbOffsetFromLsb() + ($multiplier * $self->getWidth());
            $self->set_lsbOffsetFromLsb($lsbOffsetFromLsb);
            my $msbOffsetFromLsb = $self->getMsbOffsetFromLsb() + ($multiplier * $self->getWidth());
            $self->set_msbOffsetFromLsb($msbOffsetFromLsb);
        }
        if(defined($fieldHash->{'interruptInfo'})) {
        ...
        if (($parentHashRef->{'declaredExternal'} eq 'false') &&
            ...
            ($parentHashRef->{'virtualRegister'} eq 'false')) {
            ...
            my $fieldPorts = $wideRegFieldHash->{'ports'};
            my $portListRef = csrPorts->new($self,
                                            $fieldHash,
    ...
    sub do_setFieldSegmentOf {
        my ($self,
            $fieldHash,
            $prefixListRef) = @_;

        my @prefixNameList = @$prefixListRef;
        pop(@prefixNameList); pop(@prefixNameList);
        my $wideRegName = join('.', @prefixNameList);
        my ($wideRegHashRef, $bogusAddr) = $designDBRef->_getHashRefForName($wideRegName);
        my $bitfieldListRef = $wideRegHashRef->{'bitfields'};
        for (my $count=0; $count < @$bitfieldListRef; $count++) {
    ...
    sub getIdentifierWithIndex {
        my $self = shift;
        # get object identifier which includes array index.
        return $self->getIdentifier() if (!$self->isArray());
        return $self->get_identifierWithIndex();
    }
    ...
    sub hasHdlPath {
        my $self = shift;
        if (defined($self->getSegmentOf())) { # wide field
            my $parentField = $self->getParentWideFieldObject();
            return 1 if (defined($parentField->get_nodeHashRef()->{'hdlPathInfo'}));
        } else {
            return 1 if (defined($self->get_nodeHashRef()->{'hdlPathInfo'}));
        }
        return 0;
    }
    sub getHdlPath {
        ...
        my (@hdlPath, $arrayIndex, $hdlPathObj);
        if (defined($self->getSegmentOf())) { # wide field
            my $parentField = $self->getParentWideFieldObject();
            return undef if (!defined($parentField->get_nodeHashRef()->{'hdlPathInfo'}));
            $hdlPathObj = $parentField->getHdlPathObj();
            if (defined($parentField->get_nodeHashRef()->{'arrayDimensionInfo'})) {
                $arrayIndex = $parentField->getArrayIndex();
            }
        } else {
            return undef if (!defined($self->get_nodeHashRef()->{'hdlPathInfo'}));
            $hdlPathObj = $self->getHdlPathObj();
            $arrayIndex = $self->getArrayIndex();
        }
        my $slices = $hdlPathObj->getHdlPathSlices();
        my $hdlPathName = $slices->[0]->getSliceName();
        if (defined($arrayIndex)) {
            $hdlPathName = $designDBRef->_getArrayElementName($hdlPathName,
                                                              $arrayIndex);
            if ($viewName ne 'sv') {
                $hdlPathName =~ s/\[/_/g;
                $hdlPathName =~ s/\]//g;
    ...
    sub getFullHdlPath {
        ...
        my ($hdlPath, $hdlPathObj, $parentObj, $hierId, $id, $fieldHdlPath);
        if (defined($self->getSegmentOf())) { # wide field
            my $parentField = $self->getParentWideFieldObject();
            return undef if (!defined($parentField->get_nodeHashRef()->{'hdlPathInfo'}));
            $hdlPathObj = $parentField->getHdlPathObj();
            $parentObj = $parentField->getParentNode();
            $hierId = $parentField->getHierarchicalIdentifier();
            $fieldHdlPath = $parentField->getHdlPath();
            $id = $parentField->getIdentifier();
        } else {
            return undef if (!defined($self->get_nodeHashRef()->{'hdlPathInfo'}));
            $hdlPathObj = $self->getHdlPathObj();
            $parentObj = $self->getParentNode();
            $hierId = $self->getHierarchicalIdentifier();
            $fieldHdlPath = $self->getHdlPath();
            $id = $self->getIdentifier();
        }
        my $hdlPathStyle = $hdlPathObj->getHdlPathStyle();
        if ($hdlPathStyle eq 'rtlVerilog') {
            $hdlPath = $self->getFullHdlPath_RTLVerilog($fieldHdlPath,
                                                        $hierId,
                                                        $parentObj);
        } elsif ($hdlPathStyle eq 'hierarchicalIdentifier') {
            $hdlPath = $self->getFullHdlPath_HierarchicalIdentifier($fieldHdlPath,
                                                                    $hierId,
                                                                    $parentObj);
        } elsif ($hdlPathStyle eq 'identifier') {
            $hdlPath = $self->getFullHdlPath_Identifier($fieldHdlPath,
                                                        $hierId,
                                                        $parentObj);
        } elsif ($hdlPathStyle eq 'addressmapRelative') {
            $hdlPath = $self->getFullHdlPath_AddressmapRelative($fieldHdlPath,
                                                                $hierId,
                                                                $parentObj);
        } else {
            my $i=0;
    ...
    sub getFullHdlPath_RTLVerilog {
        ...
        my (@hdlPath, @hierIdSegs);

        $fieldHdlPath = join('_', ($parentReg->getHdlPath(), $fieldHdlPath));
        # get the parent object of the register
        my $parentObj = $parentReg->getParentNode();
        while ($parentObj->isGroupNode()) {
            my $parentHierId = $parentObj->getHierarchicalIdentifier();
            if ($viewName eq 'sv') {
                $parentHierId =~ s/(\[\d+\])+//g;
            } else {
                $parentHierId =~ s/\[/_/g;
            ...
            my @tmpHierIdSegs = split('\.', $parentHierId);
            unshift(@hierIdSegs, $tmpHierIdSegs[@tmpHierIdSegs-1]);
            $parentObj = $parentObj->getParentNode();
        }
        my $fullHdlPath = join('_', (@hierIdSegs), $fieldHdlPath);
        $fullHdlPath = join('_', ('csr_internal_field', $fullHdlPath));
        # get the path to the top-level RTL
        while ($parentObj) {
            my $topHdlPath = $parentObj->getHdlPath();
            if (defined($topHdlPath)) {
                $fullHdlPath = join('.', ($topHdlPath, $fullHdlPath));
            }
            last if ($parentObj->getNodeType() eq 'addressmap');
            $parentObj = $parentObj->getParentNode();
        }
        if ($viewName eq 'sv') {
            if ($hierId =~ /((\[\d+\])+)/) {
    ...
    sub getFullHdlPath_HierarchicalIdentifier {
        ...
        my $fullHdlPath = join('.', ($parentReg->getHdlPath(), $fieldHdlPath));
        # get the parent object of the register
        my $parentObj = $parentReg->getParentNode();
        # get the path to the top-level RTL
        while ($parentObj) {
            my $parentHdlPath = $parentObj->getHdlPath();
            if (defined($parentHdlPath)) {
                $fullHdlPath = join('.', ($parentHdlPath, $fullHdlPath));
            }
            $parentObj = $parentObj->getParentNode();
        }
        if ($viewName ne 'sv') {
    ...
    sub getFullHdlPath_Identifier {
        ...
        while ($parentObj) {
            my $type = $parentObj->getNodeType();
            last if (($type eq 'addressmap') || ($type eq 'addressmapInstance'));
            $parentObj = $parentObj->getParentNode();
        }
        while ($parentObj) {
            my $topHdlPath = $parentObj->getHdlPath();
            if (defined($topHdlPath)) {
                $fieldHdlPath = join('.', ($topHdlPath, $fieldHdlPath));
            }
            $parentObj = $parentObj->getParentNode();
        }
        if ($viewName ne 'sv') {
        ...
        if ($viewName eq 'sv') {
            if ($hierId =~ /((\[\d+\])+)/) {
                my $indices = $1;
                $fieldHdlPath =~ s/\[\d+\]//g;
                $fieldHdlPath .= $indices;
            }
        }             
        push (@hdlPath, $fieldHdlPath);
        return \@hdlPath;
    }
    sub getFullHdlPath_AddressmapRelative {
        my ($self,
        ...
        my $fullHdlPath = join('_', ($parentReg->getHdlPath(), $fieldHdlPath));
        my $parentObj = $parentReg->getParentNode();
        while ($parentObj) {
            if ($parentObj->getNodeType() eq 'addressmap') {
                my $addrmapHdlPath = $parentObj->getHdlPath();
                $fullHdlPath = join('.', ($addrmapHdlPath, $fullHdlPath));
            }
            $parentObj = $parentObj->getParentNode();
        }
        if ($viewName ne 'sv') {
    ...
    sub getMsbOffsetFromLsb {
        my $self = shift;
        return $self->get_msbOffsetFromLsb();
    }
    sub getLsb {
        my $self = shift;
        return $self->get_lsb();
    }
    sub getLsbOffsetFromLsb {
        my $self = shift;
        return $self->get_lsbOffsetFromLsb();
    }
    sub getMsbLsb {
        my $self = shift;
        return ($self->get_msb(), $self->get_lsb());
    ...
    sub hasTestConstraint {
        my $self = shift;
        return 0 if (!defined($self->getTestConstraint()));
        return 0 if ($self->getTestConstraint() eq 'none');
        return 1;
    }
    sub getTestConstraint {
        my $self = shift;
        return $self->get_testConstraint();
    }
    sub hasInputSynchronizer {
	my $self = shift;
    ...
    sub getPortList {
        ...
        return if (!defined($self->getPorts()) && !defined($self->getExternalPorts()));

        my $fieldPorts = $self->getPorts() if (defined($self->getPorts()));
        my $fieldExtPorts = $self->getExternalPorts();
        if (defined($fieldExtPorts)) {
            push (@{$fieldPorts}, @{$fieldExtPorts});
        }

        foreach my $portObj (@{$fieldPorts}) {
            my $portType = $portObj->getPortType();
            if($self->isExternal()) {
    ...
    sub getExpressionStr {
        my $self = shift;
        if ($self->hasResetValue()) {
            if ($self->resetValueHasExpression()) {
                my $resetObj = $self->getResetValueNode();
                return $resetObj->getExpressionStr();
            }
        }
        return undef;
    }
    sub resetValueHasParameter {
        my $self = shift;
        if ($self->hasResetValue()) {
            if ($self->resetValueHasExpression()) {
                my $resetObj = $self->getResetValueNode();
                my $exprStr = $resetObj->get_expressionStr();
                return 1 if ($exprStr =~ /Parameter/);
            }
        }
        return 0;
    }
    sub getParametersInResetValue {
        my $self = shift;
        my @paramList;
        return undef if (!$self->resetValueHasParameter());
        my $resetObj = $self->getResetValueNode();
        my $exprStr = $resetObj->get_expressionStr();
        @paramList = ($exprStr =~ /\<Parameter\s+\'([A-Za-z0-9_]+)\'\s+value\>/g);
        return \@paramList;
    }
    sub getParameterValue {
        ...
        while ($parent) {
            if ($parent->getNodeType() eq 'addressmapInstance') {
                if ($parent->hasParameters()) {
                    my $parameters = $parent->getParameters();
                    foreach my $nextParam (@{$parameters}) {
                        if ($nextParam->getName() eq $paramName) {
                            my $paramVal = $nextParam->getValue();
                            return $paramVal;
                        }
                    }
                }
	...
	return if (!defined($resetObj));
        ...
        if ($resetValueType eq 'constant') {
            if ($resetObj->hasExpression()) {
                my $exprHash = $resetObj->getExpression();
                my $exprValue = $self->getExpressionValue($exprHash);
                ($verilogFormat, $exprValue) = $self->getVerilogBaseAndValue($exprValue);
                $exprValue = Math::BigInt->new($exprValue);
                return ($exprValue);
            } else {
	...
	my $portList;
	if ($portType =~ /external/) {
	    $portList = $self->getExternalPorts();
	} else {
    ...
    sub getParentNode {
        ...
        my ($relAddrBigInt, $backdoorPath);
        my ($aliasPrefixList, $parentObject, $bogusAddr);
        my ($parentName, $parentHashRef, $parentOfParentHashRef);
        my ($busProtocol, $inputPipelineStage, $outputPipelineStage);
    ...
    sub getParentWideFieldObject {
        ...
        my (@levelNameList, $fieldIndex, $backdoorPath);
        ...
        my $fieldHash = $self->get_nodeHashRef();
        my $segmentOf = $self->getSegmentOf();
        if (defined($fieldHash->{'wideRegisterName'}) &&
            defined($fieldHash->{'parentFieldIsArray'})) {
            my @segmentsIndex = split(', ', $fieldHash->{'segmentParentArrayUserIndex'});
            my (@indeces, $tmpIndeces);
            for (my $index = 0; $index < @segmentsIndex; $index++) {
                push (@{$fieldIndex}, $segmentsIndex[$index]);
                my $nextIndex = join('', ('[', $segmentsIndex[$index], ']'));
                push(@indeces, $nextIndex);
            }
            $segmentOf .= join('', @indeces);
        }
        push (@levelNameList, ($parentWideRegObj->getHierarchicalIdentifier(), $segmentOf));
...

{
    ...
    sub getParentNode {
        ...
        my ($verilogFormat, $arrayIndex, $bigIntAddr, $backdoorPath);
        my ($topLevelHashRef, @aliasPrefixList, $parentHashRef, $relAddrBigInt);
	...
	my $hasConcatFlag = 0;
		...
		for (my $portIndex=0; $portIndex < @{$portInfo[0]}; $portIndex++) {
		    ...
		    $nextPortMember->setPortName($referentHash,
						 ...
						 $expandedName);
                    $nextPortMember->set_parentHashRef($referentHash);
		    my $newPortType;
...
package csrIndirectAddress;

use strict;
@csrIndirectAddress::ISA = qw(csrPerlAPIBase);

{
    my %indirectAddressAttrs = (_referentObj => '',
                               );

    sub _defaultAttrKeys {
        my $self = shift;
        return ($self->SUPER::_defaultAttrKeys(), map { s/^_//; $_ } keys %indirectAddressAttrs);
    }
    ...
    sub new {
        ...
        my ($sourceType);
        my $indirectAddr = $_[0]->SUPER::new($indirectAddrInfo);
        if (defined($indirectAddrInfo->{'hierarchicalRefName'})) {
            $indirectAddr->set_hierarchicalRefName($indirectAddrInfo->{'hierarchicalRefName'},
                                                   $referentObj);
        }
        if (defined($sourceType = $indirectAddrInfo->{'sourceType'})) {
            $indirectAddr->set_sourceType($sourceType);
        }
        $indirectAddr->set_topLevelAddressmapName($indirectAddrInfo->{'topLevelAddressmapName'});
        my ($regRef, $fieldRef) = $indirectAddr->getReferenceInfo();
        $indirectAddr->set_indirectIndexRegister($regRef);
        $indirectAddr->set_indirectIndexField($fieldRef);
        return ($indirectAddr);
    }
    sub getHierarchicalRefName {
        my $self = shift;
...

{
    ...
    sub new {
        ...
        if (defined($resetHashRef->{'expression'})) {
            $self->set_expression($resetHashRef->{'expression'});
#            my $expr = csrExpression->new($resetHashRef->{'expression'});
#            $self->set_expressionHash($resetHashRef->{'expression'});
#            $self->set_expressionObj($expr);
            $self->set_expressionStr($resetHashRef->{'expressionStr'});
        }
        return $self;
    }
    sub getValue {
    ...
    sub hasExpression {
        my $self = shift;
        return 1 if (defined($self->get_expression()));
        return 0;
    }
    sub getExpression {
        my $self = shift;
    ...
    sub getExpressionStr {
        my $self = shift;
        return $self->get_expressionStr();
    }
    sub hasParameter {
        my $self = shift;
        my $str = $self->get_expressionStr();
        return 0 if (!defined($str));
        return 1 if ($str =~ /Parameter/);
        return 0;
    }
}
...
@csrHdlPath::ISA = qw(csrPerlAPIBase);
...

{
    ...
    sub _defaultAttrKeys {
        my $self = shift;
        return ($self->SUPER::_defaultAttrKeys(), map { s/^_//; $_ } keys %hdlPath);
    }
    sub new {
        my ($caller, $hdlPathInfo) = @_;
        my $self = $_[0]->SUPER::new($hdlPathInfo);
        return $self;
    }
    sub getPath {
        my $self = shift;
    ...
    sub getHdlPathSlices {
        my $self = shift;
        my @sliceObjs;
        my $slices = $self->get_path();
        for (my $index=0; $index < @{$slices}; $index++) {
            my $nextSlice = $slices->[$index];
            my $nextSliceObj = csrHdlPathSlice->new($nextSlice);
            push (@sliceObjs, $nextSliceObj);
        }
        return \@sliceObjs;
    }
    sub hasHdlPathStyle {
        my $self = shift;
        return 1 if ($self->get_style() ne '');
        return 0;
    }
    sub getHdlPathStyle {
        my $self = shift;
    ...
    sub hasHdlPathPrefix {
        my $self = shift;
        return 1 if ($self->get_prefix() ne '');
        return 0;
    }
    sub getHdlPathPrefix {
        my $self = shift;
        return $self->get_prefix();
    }
}
package csrHdlPathSlice;

@csrHdlPathSlice::ISA = qw(csrPerlAPIBase);

use strict;
use vars '$AUTOLOAD';

{
    my %pathSlice = (_sliceName => '',
                     _lsb => '',
                     _width => '',
                    );
    ...
    sub _defaultAttrKeys {
        my $self = shift;
        return ($self->SUPER::_defaultAttrKeys(), map { s/^_//; $_ } keys %pathSlice);
    }
    sub new {
        my ($caller, $hdlPathSlice) = @_;
        my $self = $_[0]->SUPER::new($hdlPathSlice);
        return $self;
    }
    sub getSliceName {
        my $self = shift;
        return $self->get_sliceName();
    }
    sub getSliceLsbOffset {
        my $self = shift;
        return $self->get_lsbOffsetFromLsb();
    }
    sub getSliceWidth {
...
package csrExpression;
@csrExpression::ISA = qw(csrPerlAPIBase);

{
    ...
    sub new {
        ...
        my $self;
        my $type = $expHash->{'type'};
        if (($type eq '+') || ($type eq '-') || ($type eq '*') || ($type eq '/')) {
            my $value1 = csrExpression->new($expHash->{'value1'});
            my $value2 = csrExpression->new($expHash->{'value2'});
            my $class = ref($caller) || $caller;
            $self = bless {}, $class;
            $self->_init($caller, $expHash);
            $self->set_value1($value1);
            $self->set_value2($value2);
            $self->set_type($expHash->{'type'});
        } elsif (($type eq 'integerLiteral') || ($type eq 'parameter') || ($type eq 'index')) {
            $self = $_[0]->SUPER::new($expHash);
        }
        return $self;
    }            
    sub getType {
    ...
    sub getValueType {
        my $self = shift;
        return $self->get_valueType();
    }
    sub isValueParameter {
        my $self = shift;
        # is the value a parameter?
        return 1 if ($self->getValueType() eq 'parameter');
        return 0;
    }
...

{
    ...
    sub getSourceFileVersion {
        my $self = shift;
        return $self->get_sourceFileVersion();
    }
    sub getName {
        my $self = shift;
        return $self->get_name();
    ...
    sub isIndirectDataRegister {
        my $self = shift;
        return 1 if ($self->isRegisterNode() && defined($self->get_indirectAddress()));
        return 0;
    }
    sub isSubRegisterNode {
	my $self = shift;
        # Deprecated! Use isSubRegister().
        return $self->isSubRegister();
    }
    sub isSubRegister {
	my $self = shift;
    ...
    sub isSubMemoryNode {
	my $self = shift;
        # Deprecated! Use isSubMemory().
        return $self->isSubMemory();
    }
    sub isSubMemory {
	my $self = shift;
	...
	my $self = shift;
        # Deprecated! Use isWideMemory().
        return $self->isWideMemory();
    }
    sub isWideMemory {
	my $self = shift;
    ...
    sub hasHdlPath {
        my $self = shift;
        if ($self->isSubRegisterNode() || $self->isSubMemoryNode()) {
            my $parentObj = $self->getParentNode();
            return 0 if (!defined($parentObj->get_nodeHashRef()->{'hdlPathInfo'}));
        } else {
            return 0 if (!defined($self->get_nodeHashRef()->{'hdlPathInfo'}));
        }
        return 1;
    }
    sub getHdlPath {
        ...
        my ($arrayIndex);
        my $hdlPathObj = $self->getHdlPathObj();
        my $hdlPath = $hdlPathObj->getHdlPathName();
        if ($self->isRegisterNode() || $self->isWideRegisterNode()) {
            $hdlPath = join('_', ('csr_internal_field', $hdlPath));
        }
        if ($hdlPathObj->hasHdlPathPrefix()) {
            $hdlPath = join('.', ($hdlPathObj->getHdlPathPrefix(), $hdlPath));
        }
        if (defined($arrayIndex = $self->getArrayIndex())) {
            $hdlPath = $designDBRef->_getArrayElementName($hdlPath,
                                                          $arrayIndex);
            my $objType = $self->getNodeType();
            if ((($objType eq 'addressmapInstance') || ($objType eq 'group')) && ($viewName eq 'sv')) {
                $hdlPath =~ s/(\[\d+\])+//g;
            } else {
                $hdlPath =~ s/\[/_/g;
                $hdlPath =~ s/\]//g;
    ...
    sub getExpressionValue {
        ...
        my $format;
        if ($exprHash->{'type'} eq '+') {
            my $value1Hash = $exprHash->{'value1'};
            my $value1 = $self->getExpressionValue($value1Hash);
            ($format, $value1) = $self->getVerilogBaseAndValue($value1);
            $value1 = Math::BigInt->new($value1);
            my $value2Hash = $exprHash->{'value2'};
            my $value2 = $self->getExpressionValue($value2Hash);
            ($format, $value2) = $self->getVerilogBaseAndValue($value2);
            $value2 = Math::BigInt->new($value2);
            $value1->badd($value2);
            return $value1;
        } elsif ($exprHash->{'type'} eq '-') {
            my $value1Hash = $exprHash->{'value1'};
            my $value1 = $self->getExpressionValue($value1Hash);
            ($format, $value1) = $self->getVerilogBaseAndValue($value1);
            $value1 = Math::BigInt->new($value1);
            ...
            $value1->bdiv($value2);
            return $value1;
        } elsif ($exprHash->{'type'} eq 'index') {
            my $levelCnt = 0;
            my $currObj = $self;
            my $level = $exprHash->{'value1'};
            while ($levelCnt < $level) {
                my $parentReg = $currObj->getParentNode();
                $currObj = $parentReg;
                $levelCnt++;
            }
            my $multiplier = $designDBRef->_getMultiplierFromArrayIndex($currObj->getArrayIndex(),
                                                                        $currObj->get_nodeHashRef->{'arrayDimensionInfo'});
            my $value = Math::BigInt->new($multiplier);
            return $value;
        } elsif (($exprHash->{'type'} eq 'integerLiteral') || ($exprHash->{'type'} eq 'numberLiteral')) {
            return $exprHash->{'value1'};
        } elsif ($exprHash->{'type'} eq 'parameter') {
            return $exprHash->{'value1'};
        }
    }
    ...
    sub getNextPortNode {
        ...
        # Deprecated! Use getNextPort().
        return $self->getNextPort($portListRef,
                                  $currPortIndexRef);
    }
    sub hasAttributes {
        my $self = shift;
    ...
    sub hasResetAllFlops {
        my $self = shift;
        return 1 if ($self->get_resetAllFlops() eq 'true');
        return 0;
    }
    sub isAddressmapNode {
        my $self = shift;
        # Deprecated! Use isAddressmap().
        return $self->isAddressmap();
    }
    sub isAddressmap {
        my $self = shift;
    ...
    sub isMemoryNode {
	my $self = shift;
        # Deprecated! Use isMemory().
        return $self->isMemory();
    }
    sub isMemory {
	my $self = shift;
	my $nodeType;
    ...
    sub isWideRegisterNode {
        my $self = shift;
        # Deprecated! Use isWideRegister().
        return $self->isWideRegister();
    }
    sub isWideRegister {
        my $self = shift;
        my $nodeType;
    ...
    sub getParentAddressmapOrGroupNode {
        ...
        my $count = 0;
	my ($verilogFormat, $relAddrBigInt, $arrayIndex, $backdoorPath);
        my ($busProtocol, $inputPipelineStage, $outputPipelineStage);
        my (@prefixList, $parentHashRef, $parentOfParentHashRef);
        my ($topLevelHashRef, $addrmapObj, $parentName, $bogusAddr);
    ...
    sub getExternalParentObject {
        ...
        # get the external parent object for the calling object.

        my $count = 0;
        my ($relAddrBigInt, $bigIntOffset, $verilogFormat, $arrayIndex);
        my (@prefixList, $parentHashRef, $parentOfParentHashRef);
        my ($topLevelHashRef, $extObj, $parentName, $bogusAddr);
        my (@aliasPrefixList, $bigIntAddr, $backdoorPath);
        ...
        while(@levelNameList) {
            ...
            if ((($parentHashRef->{'nodeType'} eq 'addressmap') ||
               ($parentHashRef->{'nodeType'} eq 'addressmapInstance') ||
               ($parentHashRef->{'nodeType'} eq 'wideRegister') || # for virtual regs
               ($parentHashRef->{'nodeType'} eq 'memory') || # for virtual registers
               ($parentHashRef->{'nodeType'} eq 'wideMemory') ||
               ($parentHashRef->{'nodeType'} eq 'group') ||
               ($parentHashRef->{'nodeType'} eq 'union')) &&
		...
		if (($parentHashRef->{'nodeType'} eq 'addressmap') ||
		    ...
		    ($verilogFormat, $bigIntOffset) = $self->getVerilogBaseAndValue($parentHashRef->{'byteOffset'});
                    ...
                    $extObj = csrPerlAPIRegister->new($parentHashRef,
                                                      $arrayIndex,
                                                      $parentOfParentHashRef,
                                                      $bigIntAddr,
                                                      $bigIntOffset,
    ...
    sub getRegisterNodeByName {
        ...
        my ($arrayIndex, $bigIntAddr, $backdoorPath);
        my (@aliasPrefixList, $registerObj, $bogusAddr);
        my ($regHashRef, $parentName, $parentHashRef, $relAddrBigInt);
	...
	my @hierNameList = split('\.', $regName);
        ...
        my @tmpLevelNameList = @levelNameList;
        my $topLevelHashRef = $designDBRef->_findAddressmapNodeByName($self->get_topLevelAddressmapName(),
    ...
    sub getTopLevelAddressmapNode {
        my $self = shift;
        return $self->getTopLevelAddressmap();
    }
    sub getTopLevelAddressmap {
        my $self = shift;
        my @prefixList = ();
	my $arrayIndex = undef;
        my $backdoorPath;
        my ($parentAddrmapHashRef, $relAddrBigInt, @hierarchicalPrefixList);
    ...
    sub assignAddressInfo {
	   ...
	   $addrmapRelByteAddr) = @_;
        ...
        my $addrBigInt = Math::BigInt->new($objAddr);
        $self->set_byteAddress($addrBigInt);
        my ($verilogFormat, $value) = $self->getVerilogBaseAndValue($objByteOffset);
        $self->set_byteOffset($value);
	...
	my ($portObj, @portList, $portListRef);
        ...
        $portListRef = $self->getExternalPorts();
        return undef if(!defined($portListRef = $self->getExternalPorts()));
        my $currPortIndex = 0;
...
sub getRegisterObj {
    ...
    my $prefixListRef = ();
    my ($relAddrBigInt, @hierarchicalPrefixList, $backdoorPath);
    my ($bigIntOffset, $verilogFormat, $offset, $newAddrmapInfo);
    my ($bigIntAddr, @prefixList, @aliasPrefixList, $registerObj);

    my $registerHashListRef = $designDBRef->_getRegHashList($addrmapHashRef);
    my $regIndex = $currAddrmapInfo->{'currAddrmapRegisterIndex'};
    my $regHashRef = $registerHashListRef->[$regIndex];
    $currAddrmapInfo->{'currRegisterHashRef'} = $regHashRef;
    my $parentIsArray = 0;

    if (defined($regHashRef->{'arrayDimensionInfo'}) && $self->getExpandArraysFlag()) {
	$registerObj = $self->getRegArrayElement($currAddrmapInfo, $addrmapHashRef);
    } else {
        ($verilogFormat, $bigIntOffset) = $self->getVerilogBaseAndValue($regHashRef->{'byteOffset'});
        $bigIntAddr = Math::BigInt->new($bigIntOffset);
        if ($addrmapHashRef->{'nodeType'} eq 'group') {
            ($verilogFormat, $offset) = $self->getVerilogBaseAndValue($addrmapHashRef->{'byteOffset'});
            $bigIntOffset->badd($offset);
        }
        $bigIntAddr->badd($currAddrmapInfo->{'parentAddr'});
...
sub getRegArrayElement {
    my ($self, $currAddrmapInfo, $addrmapHashRef) = @_;

    my $prefixListRef = ();
    my ($registerObj, $newAddrmapInfo, $addrmapRelByteAddrBigInt);
    my (@prefixList, @aliasPrefixList, $backdoorPath, @hierarchicalPrefixList);
    ...
    ($verilogFormat, $addrmapRelByteAddrBigInt) =
	$self->getVerilogBaseAndValue($regHashRef->{'addressmapRelativeByteAddress'});
    ...
    $prefixListRef = $currAddrmapInfo->{'prefixListRef'};
    my $tmpRegName = $designDBRef->_getArrayElementName($regHashRef->{'name'},
                                                        $arrayIndex);
...
sub getMemoryObj {
	...
	$memoryObj = $self->getMemArrayElement($currAddrmapInfo, $addrmapHashRef);
    } else {
...
sub getSubMemObj {
    ...
    my ($relAddrBigInt, @hierarchicalPrefixList, $addrmapRelByteAddr);
    ...
    ($verilogFormat, $offset) = $self->getVerilogBaseAndValue($subMemHashRef->{'byteOffset'});
    ($verilogFormat, $relAddrBigInt) = $self->getVerilogBaseAndValue($subMemHashRef->{'addressmapRelativeByteAddress'});
    $prefixListRef = $currAddrmapInfo->{'prefixListRef'};
    my $hierPrefixListRef = $currAddrmapInfo->{'hierarchicalPrefixListRef'};
...
sub getGroupObj {
    ...
    my ($verilogFormat, $newAddrmapInfo, $newGroupHashRef);
	...
	$groupObj = $self->getGroupArrayElement($currAddrmapInfo,
                                                $addrmapHashRef,
                                                $groupHashRef);
    } else {
        ($verilogFormat, $bigIntAddr) = $self->getVerilogBaseAndValue($groupHashRef->{'byteOffset'});
	$bigIntAddr->badd($currAddrmapInfo->{'parentAddr'});
...
sub getUnionObj {
	...
	$unionObj = $self->getUnionArrayElement($currAddrmapInfo,
                                                $addrmapHashRef,
                                                $unionHashRef);
    } else {
...
sub getSubRegObj {
    ...
    my $prefixListRef = ();
    my ($tmpSubRegName, $arrayIndex, $backdoorPath);
    my ($relAddrBigInt, @hierarchicalPrefixList, $verilogFormat, $offset);
    my ($bigIntAddr, @prefixList, $registerObj, @aliasPrefixList);

    my $regMultiplier = 0;
    my $regHashRef = $currAddrmapInfo->{'currRegisterHashRef'};
    my $subRegHashListRef = $regHashRef->{'subRegisters'};
    my $subRegIndex = $currAddrmapInfo->{'currAddrmapSubRegisterIndex'};
    my $subRegHashRef = $subRegHashListRef->[$subRegIndex];
    ($verilogFormat, $offset) = $self->getVerilogBaseAndValue($subRegHashRef->{'byteOffset'});
    ($verilogFormat, $relAddrBigInt) = $self->getVerilogBaseAndValue($subRegHashRef->{'addressmapRelativeByteAddress'});
    $prefixListRef = $currAddrmapInfo->{'prefixListRef'};
    my $hierPrefixListRef = $currAddrmapInfo->{'hierarchicalPrefixListRef'};
...
sub getNextWideRegArrayElement {
    ...
    my $registerObj = $self->getRegArrayElement($oldWideRegRecord, $addrmapHashRef);
...
sub getNextWideMemArrayElement {
    ...
    my $memoryObj = $self->getMemArrayElement($oldWideMemRecord, $addrmapHashRef);
...
sub resetValueHasExpression {
    my $self = shift;

    return 0 if (!$self->hasResetValue());
    my $resetObj = $self->getResetValueNode();
    my $resetValueType = $resetObj->getSourceType();
    if ($resetValueType eq 'constant') {
        return 1 if ($resetObj->hasExpression());
    }
    return 0;
}
