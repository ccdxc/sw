...
pxb_read_target_err_counters()
{
    ...
    pal_reg_rd32w(CAP_ADDR_BASE_PXB_PXB_OFFSET +
                      CAP_PXB_CSR_CNT_TGT_RSP_CA_UR_BYTE_ADDRESS,
                  (uint32_t *)&ur_cpl, 2);

    // Target errors
    pal_reg_rd32w(CAP_ADDR_BASE_PXB_PXB_OFFSET +
                      CAP_PXB_CSR_SAT_TGT_RX_DROP_BYTE_ADDRESS,
                  (uint32_t *)&tlp_drop, 2);
...
pxb_read_pport_status(uint8_t port)
{
    ...
    for (int i = 0; i < polls; i++) {
        pal_reg_rd32w(CAP_ADDR_BASE_PP_PP_OFFSET +
                          CAP_PP_CSR_STA_DEBUG_STALL_BYTE_ADDRESS,
                      &stall, 1);
...
pxb_read_pport_counters(uint8_t port)
{
    ...
    // TLP counters
    pal_reg_rd32w(CAP_ADDR_BASE_PP_PP_OFFSET +
                      CAP_PP_CSR_PORT_C_CNT_C_TL_RX_REQ_BYTE_ADDRESS,
                  (uint32_t *)&rx_req_tlp, 2);

    pal_reg_rd32w(CAP_ADDR_BASE_PP_PP_OFFSET +
                      CAP_PP_CSR_PORT_C_CNT_C_TL_RX_CPL_BYTE_ADDRESS,
                  (uint32_t *)&rx_cpl_tlp, 2);

    pal_reg_rd32w(CAP_ADDR_BASE_PP_PP_OFFSET +
                      CAP_PP_CSR_PORT_C_CNT_C_TL_TX_REQ_BYTE_ADDRESS,
                  (uint32_t *)&tx_req_tlp, 2);

    pal_reg_rd32w(CAP_ADDR_BASE_PP_PP_OFFSET +
                      CAP_PP_CSR_PORT_C_CNT_C_TL_TX_CPL_BYTE_ADDRESS,
...
pxb_read_pport_err_counters(uint8_t port)
{
    ...
    //
    pal_reg_rd32w(CAP_ADDR_BASE_PP_PP_OFFSET +
                      CAP_PP_CSR_PORT_P_SAT_P_PORT_CNT_FCPE_BYTE_ADDRESS,
                  (uint32_t *)&fcpe, 2);
