...
print_mch_sta_data (uint64_t mc_base_addr, uint8_t channel)
{
    ...
    for (uint32_t counter = 0; counter < ARRAY_SIZE(mc_sta_reg_value);
         counter++) {
        sdk::asic::asic_reg_read(mc_base_addr +
                                 CAP_MC_CSR_MCH_MC_STA_MC_STA_0_7_BYTE_ADDRESS +
                                 (counter * MC_STA_OFFSET),
                                 &mc_sta_reg_value[counter], 1, true);
    }
    // Data from STA 0
...
print_slave_err_regs (void)
{
    ...
    bool eccerr = false;
    for (uint32_t t_offset = 0; t_offset < ARRAY_SIZE(slave_err_reg);
         t_offset++) {
        sdk::asic::asic_reg_read(CAP_ADDR_BASE_MS_RBM_OFFSET +
                                 slave_err_reg[t_offset],
...
print_master_err_regs (void)
{
    ...
    bool eccerr = false;
    for (uint32_t t_offset = 0; t_offset < ARRAY_SIZE(master_err_reg);
         t_offset++) {
        sdk::asic::asic_reg_read(CAP_ADDR_BASE_MS_RBM_OFFSET +
...
sdk_ret_t
capri_unravel_hbm_intrs (bool *iscattrip, bool *iseccerr, bool logging)
{
    ...
    for (uint8_t counter = 0; counter < MAX_CHANNEL; counter++)
    {
        ...
        for(uint32_t t_offset = 0; t_offset < ARRAY_SIZE(hbmerrcause_table);
            t_offset++)
        {
            eccerr |= read_hbmerrcause_table(&hbmerrcause_table[t_offset],
...
capri_adjust_perf (int chip_id, int inst_id,
...
void
capri_set_half_clock (int chip_id, int inst_id)
{
