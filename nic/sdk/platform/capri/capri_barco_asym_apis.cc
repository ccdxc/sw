...
void
capri_barco_wait_for_resp(uint32_t req_tag, 
        bool async_en,
...
sdk_ret_t capri_barco_asym_ecc_point_mul(uint16_t key_size, uint8_t *p, uint8_t *n,
        uint8_t *xg, uint8_t *yg, uint8_t *a, uint8_t *b, uint8_t *x1, uint8_t *y1,
        uint8_t *k, uint8_t *x3, uint8_t *y3)
{
    sdk_ret_t                   ret = SDK_RET_OK;
    uint64_t                    ilist_dma_descr_addr1 = 0, olist_dma_descr_addr = 0;
    uint64_t                    ilist_dma_descr_addr2 = 0;
    uint64_t                    ilist_mem_addr1 = 0, olist_mem_addr = 0, curr_ptr = 0;
...
#define CAPRI_BARCO_API_NAME "ECC Point Mul: "
    ...
    SDK_TRACE_DEBUG(CAPRI_BARCO_API_NAME "Allocated Key Descr @ %d", ecc_pm_key_idx);
    ...
    if (ret != SDK_RET_OK) {
        SDK_TRACE_ERR(CAPRI_BARCO_API_NAME "Failed to write key: %d", ecc_pm_key_idx);
        goto cleanup;
    }
    ...
    SDK_TRACE_DEBUG(CAPRI_BARCO_API_NAME " Allocated memory for ilist DMA Descr1 @ 0x%llx", ilist_dma_descr_addr1);
    if (operand_size > 64) {
        ret = capri_barco_res_alloc(CRYPTO_BARCO_RES_ASYM_DMA_DESCR,
        ...
        SDK_TRACE_DEBUG(CAPRI_BARCO_API_NAME " Allocated memory for ilist DMA Descr2 @ 0x%llx", ilist_dma_descr_addr2);
    }
    ...
    SDK_TRACE_DEBUG(CAPRI_BARCO_API_NAME " Allocated memory for input mem1 @ 0x%llx", ilist_mem_addr1);
    ...
    if (operand_size > 64) {
        ...
        SDK_TRACE_DEBUG(CAPRI_BARCO_API_NAME " Allocated memory for input mem2 @ 0x%llx", ilist_mem_addr2);
    }
    ...
    if (operand_size > 64) {
        ...
        if (sdk::asic::asic_mem_write(ilist_dma_descr_addr1, (uint8_t*)&ilist_dma_descr,
    ...
    if (operand_size > 64) {
        ...
        if (sdk::asic::asic_mem_write(ilist_dma_descr_addr2, (uint8_t*)&ilist_dma_descr,
                    sizeof(ilist_dma_descr))) {
    ...
    if (sdk::asic::asic_mem_read(olist_mem_addr + pad_size, (uint8_t*)x3, key_size)) {
        SDK_TRACE_ERR(CAPRI_BARCO_API_NAME " Failed to read x3 output from memory @ 0x%llx",
                (uint64_t) olist_mem_addr + pad_size);
        ret = SDK_RET_INVALID_ARG;
        goto cleanup;
    }
    if (sdk::asic::asic_mem_read(olist_mem_addr + operand_size + pad_size, (uint8_t*)y3, key_size)) {
...
cleanup:
    ...
    if (ilist_mem_addr2) {
        ret = capri_barco_res_free(CRYPTO_BARCO_RES_HBM_MEM_512B, ilist_mem_addr2);
        if (ret != SDK_RET_OK) {
    ...
    if (ilist_mem_addr1) {
        ret = capri_barco_res_free(CRYPTO_BARCO_RES_HBM_MEM_512B, ilist_mem_addr1);
        if (ret != SDK_RET_OK) {
    ...
    if (ilist_dma_descr_addr2) {
        ret = capri_barco_res_free(CRYPTO_BARCO_RES_ASYM_DMA_DESCR, ilist_dma_descr_addr2);
        if (ret != SDK_RET_OK) {
    ...
    if (ilist_dma_descr_addr1) {
        ret = capri_barco_res_free(CRYPTO_BARCO_RES_ASYM_DMA_DESCR, ilist_dma_descr_addr1);
        if (ret != SDK_RET_OK) {
...
sdk_ret_t capri_barco_asym_ecdsa_p256_sig_gen(int32_t key_idx, uint8_t *p, uint8_t *n,
...
#define CAPRI_BARCO_API_NAME "ECDSA Sig Gen: "
    ...
    if (sdk::asic::asic_mem_write(curr_ptr, (uint8_t*)k, 32)) {
        SDK_TRACE_ERR(CAPRI_BARCO_API_NAME "Failed to write ECC param k into ilist memory @ 0x%llx", (uint64_t) curr_ptr);
        ret = SDK_RET_INVALID_ARG;
        goto cleanup;
    }
    curr_ptr += 32;
...
sdk_ret_t capri_barco_asym_rsa_setup_priv_key(uint16_t key_size, uint8_t *n, uint8_t *d,
                                                            int32_t* key_idx)
{
    sdk_ret_t                   ret = SDK_RET_OK;
...
#define CAPRI_BARCO_API_NAME "RSA 2K Encrypt: "
    ...
    SDK_TRACE_DEBUG(CAPRI_BARCO_API_NAME "Allocated Key Descr @ 0x%llx", ecc_p256_key_idx);
...
sdk_ret_t capri_barco_asym_rsa_encrypt(uint16_t key_size, uint8_t *n, uint8_t *e,
                                         uint8_t *m,  uint8_t *c,
                                         bool async_en, const uint8_t *unique_key)
{
    sdk_ret_t                   ret = SDK_RET_OK;
...
sdk_ret_t capri_barco_asym_rsa2k_crt_decrypt(int32_t key_idx, uint8_t *p, uint8_t *q, uint8_t *dp,
...
sdk_ret_t capri_barco_asym_rsa2k_sig_gen(int32_t key_idx, uint8_t *n, uint8_t *d,
                                         uint8_t *h, uint8_t *s,
                                         bool async_en, const uint8_t *unique_key)
{
    sdk_ret_t                   ret = SDK_RET_OK;
...
#define CAPRI_BARCO_API_NAME "RSA 2K Sig Verify: "
    ...
    if (sdk::asic::asic_mem_write(curr_ptr, (uint8_t*)s, 256)) {
        SDK_TRACE_ERR(CAPRI_BARCO_API_NAME "Failed to write RSA param s into ilist memory @ 0x%llx", (uint64_t) curr_ptr);
        ret = SDK_RET_INVALID_ARG;
        goto cleanup;
    }
    curr_ptr += 256;
