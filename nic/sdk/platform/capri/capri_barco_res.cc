...
capri_barco_indexer_get (capri_barco_res_type_t res)
{
    indexer     *idxer = NULL;
...
sdk_ret_t
capri_barco_obj_alloc(capri_barco_resources_t *capri_barco_res,
                      int32_t *res_id, uint64_t *res)
{
    ...
    if (is != indexer::SUCCESS) {
        SDK_TRACE_ERR("%s: Failed to allocate",
                      capri_barco_res->allocator_name);
...
sdk_ret_t
capri_barco_obj_free_by_id(capri_barco_resources_t *capri_barco_res,
                           int32_t res_id)
{
    indexer::status     is = indexer::SUCCESS;
...
sdk_ret_t
capri_barco_obj_free(capri_barco_resources_t *capri_barco_res, uint64_t res)
{
    int32_t             res_id = 0;
...
sdk_ret_t
capri_barco_res_region_get(capri_barco_res_type_t region_type,
                           uint64_t *region)
{
...
sdk_ret_t
capri_barco_res_region_size_get(capri_barco_res_type_t region_type,
...
sdk_ret_t
capri_barco_res_obj_count_get(capri_barco_res_type_t region_type,
                              uint32_t *obj_count)
{
    if ((region_type >= CRYPTO_BARCO_RES_MIN) &&
            (region_type < CRYPTO_BARCO_RES_MAX)) {
...
sdk_ret_t
capri_barco_res_allocator_init(void)
{
    uint16_t                idx;
    uint64_t                region = 0;
    ...
    if(!g_hal_capri_barco_pend_req_pd_slab) {
        g_hal_capri_barco_pend_req_pd_slab =
            slab::factory("CRYPTO PEND-REQ PD",
                          sdk::lib::SDK_SLAB_ID_CRYPTO_PEND_REQ_PD,
                          sizeof(crypto_pend_req_t), 128,
                          true, true, true);
        SDK_ASSERT_RETURN(g_hal_capri_barco_pend_req_pd_slab != NULL,
                          SDK_RET_OOM);
    }
    ...
    for (idx = CRYPTO_BARCO_RES_MIN; idx < CRYPTO_BARCO_RES_MAX; idx++) {
        ...
        if ((region_size/capri_barco_resources[idx].obj_size)
            ...
            SDK_TRACE_ERR("Memory region not large enough for %s, got %d, required %d",
                          ...
                          region_size,
                          capri_barco_resources[idx].obj_count *
                          capri_barco_resources[idx].obj_size);
            return SDK_RET_ERR;
        }
...
sdk_ret_t
capri_barco_res_alloc (capri_barco_res_type_t res_type, int32_t *res_id,
                       uint64_t *res)
{
...
sdk_ret_t
capri_barco_res_free (capri_barco_res_type_t res_type, uint64_t res)
{
    if ((res_type < CRYPTO_BARCO_RES_MIN) ||
            (res_type >= CRYPTO_BARCO_RES_MAX)) {
        return SDK_RET_INVALID_ARG;
...
sdk_ret_t
capri_barco_res_free_by_id (capri_barco_res_type_t res_type, int32_t res_id)
{
    if ((res_type < CRYPTO_BARCO_RES_MIN) ||
            (res_type >= CRYPTO_BARCO_RES_MAX)) {
        return SDK_RET_INVALID_ARG;
...
sdk_ret_t
capri_barco_res_get_by_id (capri_barco_res_type_t region_type,
                           int32_t res_id, uint64_t *res)
{
