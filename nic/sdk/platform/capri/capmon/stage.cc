...
sdp_read_counters(int verbose, uint8_t pipeline, uint8_t stage)
{
    ...
    if (verbose) {
        ...
        // extract from sdp[0]
        phv_fifo_depth =
            CAP_MPU_CSR_CNT_SDP_CNT_SDP_0_3_PHV_FIFO_DEPTH_GET(sdp[0]);
    }
...
te_read_counters(int verbose, uint8_t pipeline, uint8_t stage)
{
    ...
    if (verbose) {
        pal_reg_rd32w(base + CAP_TE_CSR_CNT_PHV_IN_SOP_BYTE_ADDRESS,
                      &te_phv_cnt, 1);
        pal_reg_rd32w(base + CAP_TE_CSR_CNT_AXI_RDREQ_BYTE_ADDRESS, &te_axi_cnt,
                      1);
        pal_reg_rd32w(base + CAP_TE_CSR_CNT_TCAM_REQ_BYTE_ADDRESS, &te_tcam_cnt,
                      1);
        pal_reg_rd32w(base + CAP_TE_CSR_CNT_MPU_OUT_BYTE_ADDRESS, &te_mpu_cnt,
                      1);
    }
    stage_t *stage_ptr = NULL;
    stage_ptr = &(asic->pipelines[pipeline].stages[stage]);
...
mpu_read_counters(int verbose, uint8_t pipeline, uint8_t stage, uint8_t mpu)
{
    ...
    if (verbose) {
        ...
        pal_reg_rd32w(mpu_offset + base +
                      ...
                      &icache_fill_stall, 1);
        pal_reg_rd32w(mpu_offset + base + CAP_MPU_CSR_CNT_CYCLES_BYTE_OFFSET,
                      &cycles, 1);
        pal_reg_rd32w(mpu_offset + base +
...
mpu_reset_counters(int verbose, uint8_t pipeline, uint8_t stage, uint8_t mpu)
{
    ...
    pal_reg_wr32w(mpu_offset + base + CAP_MPU_CSR_CNT_INST_EXECUTED_BYTE_OFFSET,
                  &zero, 1);
    pal_reg_wr32w(mpu_offset + base + CAP_MPU_CSR_CNT_ICACHE_MISS_BYTE_OFFSET,
                  &zero, 1);
    pal_reg_wr32w(mpu_offset + base +
                      CAP_MPU_CSR_CNT_ICACHE_FILL_STALL_BYTE_OFFSET,
                  &zero, 1);
    pal_reg_wr32w(mpu_offset + base + CAP_MPU_CSR_CNT_CYCLES_BYTE_OFFSET, &zero,
                  1);
    pal_reg_wr32w(mpu_offset + base + CAP_MPU_CSR_CNT_PHV_EXECUTED_BYTE_OFFSET,
                  &zero, 1);
    pal_reg_wr32w(mpu_offset + base + CAP_MPU_CSR_CNT_HAZARD_STALL_BYTE_OFFSET,
                  &zero, 1);
    pal_reg_wr32w(mpu_offset + base + CAP_MPU_CSR_CNT_PHVWR_STALL_BYTE_OFFSET,
                  &zero, 1);
    pal_reg_wr32w(mpu_offset + base + CAP_MPU_CSR_CNT_MEMWR_STALL_BYTE_OFFSET,
                  &zero, 1);
    pal_reg_wr32w(mpu_offset + base + CAP_MPU_CSR_CNT_TBLWR_STALL_BYTE_OFFSET,
                  &zero, 1);
    pal_reg_wr32w(mpu_offset + base + CAP_MPU_CSR_CNT_FENCE_STALL_BYTE_OFFSET,
                  &zero, 1);
}
...
mpu_read_table_addr(int verbose, uint8_t pipeline, uint8_t stage, uint8_t mpu)
{
    ...
    if (verbose) {
        pal_reg_rd32w(
            base + CAP_MPU_CSR_STA_TBL_ADDR_BYTE_OFFSET +
                (CAP_MPU_CSR_STA_TBL_ADDR_ARRAY_ELEMENT_SIZE * mpu_offset),
            table_addr, 2);
        addr = ((uint64_t)
                    CAP_MPU_CSR_STA_TBL_ADDR_STA_TBL_ADDR_0_2_IN_MPU_31_0_GET(
                        table_addr[0]) +
                ((uint64_t)
                     CAP_MPU_CSR_STA_TBL_ADDR_STA_TBL_ADDR_1_2_IN_MPU_63_32_GET(
                         table_addr[1])
                 << 32));
...
stg_poll(int verbose, uint8_t pipeline, uint8_t stage)
{
    ...
    for (i = 0; i < polls; i++) {
        ...
        for (mpu = 0; mpu < 4; mpu++) {
            pal_reg_rd32w(
                base + CAP_MPU_CSR_STA_CTL_MPU_STA_CTL_MPU_0_2_BYTE_OFFSET +
                    mpu * CAP_MPU_CSR_STA_CTL_MPU_BYTE_SIZE,
                sta_ctl[mpu], 2);
            pal_reg_rd32w(
                base + (CAP_MPU_CSR_STA_TABLE_ARRAY_ELEMENT_SIZE * 4 * mpu) +
        ...
        for (mpu = 0; mpu < 4; mpu++) {
            ...
            phv_data_depth[mpu] +=
                CAP_MPU_CSR_STA_CTL_MPU_STA_CTL_MPU_1_2_PHV_DATA_FIFO_DEPTH_GET(
                    sta_ctl[mpu][1]);
            phv_cmd_depth[mpu] +=
                CAP_MPU_CSR_STA_CTL_MPU_STA_CTL_MPU_1_2_PHV_CMD_FIFO_DEPTH_GET(
                    sta_ctl[mpu][1]);
            stall[mpu][0] +=
                CAP_MPU_CSR_STA_CTL_MPU_STA_CTL_MPU_1_2_STALL_VECTOR_GET(
                    sta_ctl[mpu][1]) &
                0x1;
            stall[mpu][1] +=
                (CAP_MPU_CSR_STA_CTL_MPU_STA_CTL_MPU_1_2_STALL_VECTOR_GET(
            ...
            stall[mpu][3] +=
                ...
                0x1;
            mpu_processing[mpu] +=
                (CAP_MPU_CSR_STA_STG_MPU_PROCESSING_GET(sta_stg) >> mpu) & 0x1;
            latency_val[mpu] =
                CAP_MPU_CSR_STA_TABLE_MPU_PROCESSING_TABLE_LATENCY_GET(
                    latency[mpu]);
            table_type[mpu] =
                CAP_MPU_CSR_STA_TABLE_MPU_PROCESSING_TABLE_PCIE_GET(
                    latency[mpu])
                    ? TABLE_PCI
                    : CAP_MPU_CSR_STA_TABLE_MPU_PROCESSING_TABLE_SRAM_GET(
                          latency[mpu])
                          ? TABLE_SRAM
                          : TABLE_HBM;
            tblwr_valid[mpu] +=
