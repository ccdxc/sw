...
public:
    ...
    static events_recorder* init(const char *, int, const char *, const ::google::protobuf::EnumDescriptor*); // shm_name, shm_size, component, event types descriptor

    // tear down events recorder
    void deinit();

    // record event using the given args
    int event(events::Severity, const int, const char*, const ::google::protobuf::Message&, const char*...); // severity, type, kind, object key, message
private:
