...
#include "ftlv4_p4pd_mock.hpp"
#include "nic/utils/ftlv4/ftlv4_structs.hpp"

typedef struct ftlv4_mock_table_s {
    ftlv4_entry_t *entries;
} ftlv4_mock_table_t;

static ftlv4_mock_table_t mocktables[FTL_TBLID_MAX];
...
ftlv4_mock_init ()
{
    mocktables[FTL_TBLID_H5].entries =
        (ftlv4_entry_t *)calloc(table_size_get(FTL_TBLID_H5), sizeof(ftlv4_entry_t));
    assert(mocktables[FTL_TBLID_H5].entries);
    mocktables[FTL_TBLID_H5_OHASH].entries =
        (ftlv4_entry_t *)calloc(table_size_get(FTL_TBLID_H5_OHASH), sizeof(ftlv4_entry_t));
