...
//------------------------------------------------------------------------------
#ifndef __FTLV4_GTEST_BASE_HPP__
#define __FTLV4_GTEST_BASE_HPP__
...
class Ftlv4GtestBase: public ::testing::Test {
protected:
    ftlv4 *table;
    uint32_t num_insert;
    uint32_t num_remove;
...
protected:
    ...
    sdk_ret_t Get(uint32_t count, sdk_ret_t expret,
        ...
        for (auto i = 0; i < count; i++) {
            ...
            MHTEST_CHECK_RETURN(rs == expret, sdk::SDK_RET_MAX);
            assert(params->handle.valid());
            if (memcmp(params->entry, params2->entry, sizeof(ftlv4_entry_t))) {
    ...
    IterateCallback(sdk_table_api_params_t *params) {
        ...
        FTLV4_ENTRY_STR(((ftlv4_entry_t *)params->entry), buff, 512);
