...
crcFast::init(uint8_t num_polys, bool thread_safe)
{
    ...
    crcTable_ = (uint32_t **)HAL_CALLOC(HAL_MEM_ALLOC_LIB_CRCFAST,
                                        num_polys * sizeof(uint32_t *));
    if (crcTable_ == NULL) {
        return HAL_RET_OOM;
    }
    for (int i = 0; i < CRC_POLY_MAX; i++) {
        crcTable_[i] = (uint32_t *)HAL_CALLOC(HAL_MEM_ALLOC_LIB_CRCFAST,
                                              256 * sizeof(crc));
        if (crcTable_[i] == NULL) {
            return HAL_RET_OOM;
        }
...
crcFast::factory(uint8_t num_polys, bool thread_safe)
{
    ...
    mem = HAL_CALLOC(HAL_MEM_ALLOC_LIB_CRCFAST, sizeof(crcFast));
    if (mem == NULL) {
        return NULL;
    }
    ...
    if (ret != HAL_RET_OK) {
        new_crc->~crcFast();
        HAL_FREE(HAL_MEM_ALLOC_LIB_CRCFAST, mem);
        return NULL;
    }
