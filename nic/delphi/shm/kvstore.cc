...
KvstoreMgr::KvstoreMgr(int32_t *kvstore_root,  DelphiShmPtr shm_ptr) {
    ...
    root_table_ = unique_ptr<TableMgr>(new TableMgr(root_ht, shm_ptr));
}

// Init initializes kvstore root
error KvstoreMgr::Init(int32_t *kvstore_root, DelphiShmPtr shm_ptr) {
...
TableMgrUptr KvstoreMgr::Table(string kind) {
    // some error checks
    if (kind == "") {
    ...
    htable_t *ht = (htable_t *)tptr;
    return unique_ptr<TableMgr>(new TableMgr(ht, shm_ptr_));
}

// CreateTable creates a table of specified size
TableMgrUptr KvstoreMgr::CreateTable(string kind, int32_t size) {
...
TableMgrUptr KvstoreMgr::createTable(string kind, int32_t size) {
    ...
    if (tptr == NULL) {
        ...
        if (size <= MAX_SEQ_BUCKETS) {
            ...
            table_ptr = unique_ptr<TableMgr>(new TableMgr(ht, shm_ptr_));
        } else {
