...
func (r *Registry) loadServices(file *File) error {
	glog.V(2).Infof("Loading services from %s", file.GetName())
	var svcs []*Service
...
func (r *Registry) newMethod(svc *Service, md *descriptor.MethodDescriptorProto, opts *options.HttpRule) (*Method, error) {
	...
	newBinding := func(opts *options.HttpRule, idx int) (*Binding, error) {
		...
		glog.V(8).Infof("pathTemplate is %s", pathTemplate)
...
func (r *Registry) newParam(meth *Method, path string) (Parameter, error) {
	...
	fields, err := r.resolveFilePath(msg, path)
	if err != nil {
...
// resolveFilePath resolves "path" into a list of fieldDescriptor, starting from "msg".
func (r *Registry) resolveFilePath(msg *Message, path string) ([]FieldPathComponent, error) {
	if path == "" {
		return nil, nil
	}
