...
uint64_t pal_moveable_sbrk(int32_t size) {
    top += size;
    cout<<"\nTOP RETURN "<< top - base_address << " SIZE REQUESTED " << size;
    return top;
}
...
pal_ret_t pal_mem_move(uint64_t from, uint64_t to, uint32_t size) {
    if(from != to) {
        printf("\npal_mem_move FROM %lx TO %lx SIZE %d", from, to, size);
...
uint64_t setup_memory() {
    ...
    cout<<"\nBASE ADDRESS : "<< std::hex << base_address << endl;
...
pal_ret_t pal_mem_write_region(uint64_t region_address, const char *region_name, uint64_t start_address, uint64_t entry_size, uint64_t size) {
    ...
    region.size = size;
    printf("\nInside pal mem_write_region REGION ADDRESS : %lx BASE : %lx", region_address, base_address);
...
uint64_t pal_get_region(const char *region_name) {
    ...
    while(address < top) {
        ...
        printf("\tREGION NAME : %s", reg.region_name);
