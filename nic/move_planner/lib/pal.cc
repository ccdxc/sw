...
uint64_t pal_moveable_sbrk(int32_t size) {
    top += size;
    UPG_LOG_DEBUG("TOP RETURN {}", top - base_address);
    return top;
}
...
pal_ret_t pal_mem_move(uint64_t from, uint64_t to, uint32_t size) {
    if(from != to) {
        UPG_LOG_DEBUG("pal_mem_move FROM {} TO {} SIZE {}", from, to, size);
...
uint64_t setup_memory() {
    ...
    UPG_LOG_DEBUG("BASE ADDRESS : {}", base_address);
...
pal_ret_t pal_mem_write_region(uint64_t region_address, const char *region_name, uint64_t start_address, uint64_t entry_size, uint64_t size) {
    ...
    UPG_LOG_DEBUG("Inside pal mem_write_region REGION ADDRESS : {} BASE : {}",
                  region_address,
                  base_address);
...
uint64_t pal_alloc_memory(const char *name, uint32_t size) {
    ...
    UPG_LOG_DEBUG("RET : {}", return_address);
...
vector<region_t> pal_get_map() {
    ...
    while(address < top) {
        ...
        UPG_LOG_DEBUG("get_map NAME : {} SIZE = {}", reg.region_name, reg.size);
    }
...
uint64_t pal_get_region(const char *region_name) {
    ...
    while(address < top) {
        ...
        UPG_LOG_DEBUG("REGION NAME : {}", reg.region_name);
