...
int create_socket() {
  ...
  src_addr.sin6_port=htons(0xbaba);

  if (from_localhost) {
      inet_pton(AF_INET6, "2000::1:0:4", &src_addr.sin6_addr);
  } else {
      inet_pton(AF_INET6, "2000::2", &src_addr.sin6_addr);
  }
  ...
  if (from_localhost) {
      inet_pton(AF_INET6, "2000::1", &dest_addr.sin6_addr);
  } else {
      inet_pton(AF_INET6, "2000::1:0:3", &dest_addr.sin6_addr);
  }
