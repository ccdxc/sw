...
docker/shell: build-shell-image
	mkdir -p ${SW_DIR}/bazel-cache;
...
docker/build-shell-image: install_box
...
docker/coverage: build-runtime-image
	mkdir -p ${SW_DIR}/bazel-cache;
...
docker/e2e-sanity-build: build-runtime-image
	mkdir -p ${SW_DIR}/bazel-cache;
...
docker/e2e-sanity-tls-build: build-runtime-image-skip-box
	mkdir -p ${SW_DIR}/bazel-cache;
...
docker/test: build-runtime-image
...
docker/build: build-runtime-image
	docker run -it --rm --sysctl net.ipv6.conf.all.disable_ipv6=0 --privileged --name ${CONTAINER_NAME} -v $(SW_DIR):/sw -v $(SW_DIR)/bazel-cache:/root/.cache -w /sw/nic pensando/nic bash -c 'make gtest'

docker/clean-docker: build-runtime-image
...
docker/build-runtime-image: install_box
	if [ "x${NO_PULL}" = "x" ]; then docker pull $(REGISTRY)/pensando/nic:1.26; fi
