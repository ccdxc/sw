...
ifeq ($(PIPELINE),$(filter $(PIPELINE),apulu))
docker/customer-shell: docker/build-customer-shell-image
	docker run -it --rm --sysctl net.ipv6.conf.all.disable_ipv6=1 --privileged --name ${CONTAINER_NAME} -v $(SW_DIR):/sw  -v /vol/builds:/vol/builds -w /sw/nic pensando/nic su -l $(CUR_USER)
else ifeq ($(PIPELINE),$(filter $(PIPELINE),athena))
docker/customer-shell: docker/build-athena-customer-shell-image
	docker run -it --rm --sysctl net.ipv6.conf.all.disable_ipv6=1 --privileged --name ${CONTAINER_NAME} -v $(SW_DIR):/sw  -v /vol/builds:/vol/builds -w /sw/nic pensando/nic su -l $(CUR_USER)
else
...
docker/build-shell-image: docker/install_box
	...
	cd .. && BOX_INCLUDE_ENV="USER USER_UID USER_GID GROUP_NAME HOST_HOSTNAME HOST_WORKSPACE" USER_UID=$$(id -u) USER_GID=$$(id -g) GROUP_NAME=$$(id -gn) HOST_HOSTNAME=$$(hostname) HOST_WORKSPACE=$$(git rev-parse --show-toplevel) box -t pensando/nic box.rb

docker/build-customer-shell-image: docker/install_box
	echo "custom install_box"
	if [ "x${NO_PULL}" = "x" ]; then docker pull $(REGISTRY)/pensando/nic:${NIC_CONTAINER_VERSION}; fi
	cd .. && BOX_INCLUDE_ENV="USER USER_UID USER_GID GROUP_NAME" USER_UID=$$(id -u) USER_GID=$$(id -g) GROUP_NAME=$$(id -gn) box -t pensando/nic nic/apollo/tools/$(PIPELINE)/sanity/customer-docker/box.rb

docker/build-athena-customer-shell-image: docker/install_box
	echo "custom install_box"
	if [ "x${NO_PULL}" = "x" ]; then docker pull $(REGISTRY)/pensando/nic:${NIC_CONTAINER_VERSION}; fi
	cd .. && BOX_INCLUDE_ENV="USER USER_UID USER_GID GROUP_NAME" USER_UID=$$(id -u) USER_GID=$$(id -g) GROUP_NAME=$$(id -gn) box -t pensando/nic nic/apollo/tools/$(PIPELINE)/customer-docker/cshell_make_box.rb
...
docker/build-runtime-image: docker/install_box
	if [ "x${NO_PULL}" = "x" ]; then docker pull $(REGISTRY)/pensando/nic:${NIC_CONTAINER_VERSION}; fi
	cd .. && BOX_INCLUDE_ENV="NO_COPY USER USER_UID USER_GID GROUP_NAME" NO_COPY=1 USER_UID=$$(id -u) USER_GID=$$(id -g) GROUP_NAME=$$(id -gn) box -t pensando/nic box.rb

docker/build-runtime-image-skip-box:
	if [ "x${NO_PULL}" = "x" ]; then docker pull $(REGISTRY)/pensando/nic:${NIC_CONTAINER_VERSION}; fi
	cd .. && BOX_INCLUDE_ENV="NO_COPY" NO_COPY=1 box -t pensando/nic box.rb
