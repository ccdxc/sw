...
if  [[ $1 = "--install" ]]; then
    ...
    i=0
    for FILENAME in `(find drivers/ionic_en/build/vib/ -name \*.vib)`
    do
        i=$((i + 1))
        if [ $i -gt 1 ]; then
    ...
    vib=`(find drivers/ionic_en/build/vib/ -name \*.vib)`
    ...
    cd drivers/ionic_en/ && cp Makefile_6.5 Makefile && make && rm -rf .build
