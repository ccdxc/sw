...
// HAL syslog macros
#define HAL_SYSLOG_ERR(args...)     iris_c::utils::hal_syslogger()->error(args)
#define HAL_SYSLOG_WARN(args...)    iris_c::utils::hal_syslogger()->warn(args)
#define HAL_SYSLOG_INFO(args...)    iris_c::utils::hal_syslogger()->info(args)

#define HAL_TRACE_ERR(args, ...)                                               \
do {                                                                           \
    if (iris_c::utils::hal_logger()) {                                            \
        iris_c::utils::hal_logger()->error("[{}:{}] " args, __func__, __LINE__,   \
                                        ##__VA_ARGS__);                        \
        iris_c::utils::hal_logger()->flush();                                   \
    }                                                                          \
} while (0)
...
do {                                                                           \
    if (iris_c::utils::hal_logger()) {                                            \
        iris_c::utils::hal_logger()->error(args);                                 \
    }                                                                          \
} while (0)
...
do {                                                                           \
    if (iris_c::utils::hal_logger()) {                                            \
        iris_c::utils::hal_logger()->warn("[{}:{}] " args, __func__, __LINE__,    \
                                       ##__VA_ARGS__);                         \
    }                                                                          \
} while (0)
...
do {                                                                           \
    if (iris_c::utils::hal_logger()) {                                            \
        iris_c::utils::hal_logger()->info("[{}:{}] " args, __func__, __LINE__,    \
                                       ##__VA_ARGS__);                         \
    }                                                                          \
} while (0)
...
do {                                                                           \
    if (iris_c::utils::hal_logger()) {                                            \
        iris_c::utils::hal_logger()->debug("[{}:{}] " args, __func__, __LINE__,   \
                                        ##__VA_ARGS__);                        \
        iris_c::utils::hal_logger()->flush();                                   \
    }                                                                          \
} while (0)
...
do {                                                                           \
    if (iris_c::utils::hal_logger()) {                                            \
        iris_c::utils::hal_logger()->debug(args);                                 \
    }                                                                          \
} while (0)
...
do {                                                                           \
    if (iris_c::utils::hal_logger()) {                                            \
        iris_c::utils::hal_logger()->error_if("[{}:{}] "  args,                   \
...
do {                                                                           \
    if (iris_c::utils::hal_logger()) {                                            \
        iris_c::utils::hal_logger()->warn_if("[{}:{}] "  args,                    \
                                           __func__, __LINE__, ##__VA_ARGS__); \
    }                                                                          \
} while (0)
...
do {                                                                           \
    if (iris_c::utils::hal_logger()) {                                            \
        iris_c::utils::hal_logger()->info_if("[{}:{}] "  args,                    \
                                           __func__, __LINE__, ##__VA_ARGS__); \
    }                                                                          \
} while (0)
...
do {                                                                           \
    if (iris_c::utils::hal_logger()) {                                            \
        iris_c::utils::hal_logger()->debug_if("[{}:{}] "  args,                   \
                                           __func__, __LINE__, ##__VA_ARGS__); \
    }                                                                          \
} while (0)
