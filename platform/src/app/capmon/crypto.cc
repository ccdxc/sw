...
int
read_num_entries(uint32_t base, uint32_t pi_addr, uint32_t ci_addr, uint32_t ring_size_addr)
{
...
crypto_read_queues(int verbose)
{
    ...
    if (verbose) {
        ...
        for (int i = 0; i < 9; i++) {
            pal_reg_rd32w(CAP_ADDR_BASE_MD_HENS_OFFSET +
                              CAP_HENS_CSR_CNT_AXI_AW_GCM0_BYTE_ADDRESS + (i * stride),
                          (uint32_t *)&aw, 1);
            pal_reg_rd32w(CAP_ADDR_BASE_MD_HENS_OFFSET +
                              CAP_HENS_CSR_CNT_AXI_DW_GCM0_BYTE_ADDRESS + (i * stride),
                          (uint32_t *)&dw, 1);
            pal_reg_rd32w(CAP_ADDR_BASE_MD_HENS_OFFSET +
                              CAP_HENS_CSR_CNT_AXI_WRSP_GCM0_BYTE_ADDRESS + (i * stride),
                          (uint32_t *)&wrsp, 1);
            pal_reg_rd32w(CAP_ADDR_BASE_MD_HENS_OFFSET +
                              CAP_HENS_CSR_CNT_AXI_AR_GCM0_BYTE_ADDRESS + (i * stride),
                          (uint32_t *)&ar, 1);
            pal_reg_rd32w(CAP_ADDR_BASE_MD_HENS_OFFSET +
                              CAP_HENS_CSR_CNT_AXI_DR_GCM0_BYTE_ADDRESS + (i * stride),
                          (uint32_t *)&dr, 1);
            pal_reg_rd32w(CAP_ADDR_BASE_MD_HENS_OFFSET +
                              CAP_HENS_CSR_CNT_AXI_WRSP_ERR_GCM0_BYTE_ADDRESS + (i * stride),
                          (uint32_t *)&wrsp_err, 1);
            pal_reg_rd32w(CAP_ADDR_BASE_MD_HENS_OFFSET +
                              CAP_HENS_CSR_CNT_AXI_RRSP_ERR_GCM0_BYTE_ADDRESS + (i * stride),
                          (uint32_t *)&rrsp_err, 1);
            printf(" offload=%s AW=%ld DW=%ld WRSP=%ld AR=%ld DR=%ld WR_ERR=%ld RR_ERR=%ld\n",
...
crypto_read_counters(int verbose)
{
    ...
    pal_reg_rd32w(CAP_ADDR_BASE_MD_HENS_OFFSET + CAP_HENS_CSR_CNT_DOORBELL_XTS_BYTE_ADDRESS,
                  (uint32_t *)&xts_cnt, 2);

    pal_reg_rd32w(CAP_ADDR_BASE_MD_HENS_OFFSET + CAP_HENS_CSR_CNT_DOORBELL_XTS_ENC_BYTE_ADDRESS,
                  (uint32_t *)&xtsenc_cnt, 2);

    pal_reg_rd32w(CAP_ADDR_BASE_MD_HENS_OFFSET + CAP_HENS_CSR_CNT_DOORBELL_GCM0_BYTE_ADDRESS,
                  (uint32_t *)&gcm0_cnt, 2);

    pal_reg_rd32w(CAP_ADDR_BASE_MD_HENS_OFFSET + CAP_HENS_CSR_CNT_DOORBELL_GCM1_BYTE_ADDRESS,
                  (uint32_t *)&gcm1_cnt, 2);

    pal_reg_rd32w(CAP_ADDR_BASE_MD_HENS_OFFSET + CAP_HENS_CSR_CNT_DOORBELL_PK_BYTE_ADDRESS,
                  (uint32_t *)&pk_cnt, 2);

    printf("  Doorbells:\n");
    printf("    XTS %ld XTS ENC %ld GCM0 %ld GCM1 %ld PK %ld\n", xts_cnt, xtsenc_cnt, gcm0_cnt,
           gcm1_cnt, pk_cnt);
}
...
crypto_reset_counters(int verbose)
{
    ...
    pal_reg_wr32w(CAP_ADDR_BASE_MD_HENS_OFFSET + CAP_HENS_CSR_CNT_DOORBELL_XTS_BYTE_ADDRESS, zero,
                  2);
    pal_reg_wr32w(CAP_ADDR_BASE_MD_HENS_OFFSET + CAP_HENS_CSR_CNT_DOORBELL_XTS_ENC_BYTE_ADDRESS,
                  zero, 2);
    pal_reg_wr32w(CAP_ADDR_BASE_MD_HENS_OFFSET + CAP_HENS_CSR_CNT_DOORBELL_GCM0_BYTE_ADDRESS, zero,
                  2);
    pal_reg_wr32w(CAP_ADDR_BASE_MD_HENS_OFFSET + CAP_HENS_CSR_CNT_DOORBELL_GCM1_BYTE_ADDRESS, zero,
                  2);
    pal_reg_wr32w(CAP_ADDR_BASE_MD_HENS_OFFSET + CAP_HENS_CSR_CNT_DOORBELL_PK_BYTE_ADDRESS, zero,
                  2);
}
