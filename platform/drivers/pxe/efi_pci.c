	...
	for ( i = 0 ; i < num_handles ; i++ ) {
		*handle = handles[i];
		...
		if ( u.root->SegmentNumber == PCI_SEG ( pci->busdevfn ) ) {
			...
			if ( ( efirc = u.root->Configuration ( u.root, (VOID **) &Configuration ) ) != 0 ) {
				rc = -EEFI ( efirc );
				DBGC ( pci, "EFIPCI " PCI_FMT " cannot query configuration %s: %s\n",
				       PCI_ARGS ( pci ), efi_handle_name ( *handle ),
				       strerror ( rc ) );
				continue;
			}
			...
			while ( Configuration->Desc == ACPI_ADDRESS_SPACE_DESCRIPTOR ) {
				if ( Configuration->ResType == ACPI_ADDRESS_SPACE_TYPE_BUS ) {
					...
					if ( ( Configuration->AddrRangeMin <= PCI_BUS ( pci->busdevfn ) ) &&
					     ( Configuration->AddrRangeMax >= PCI_BUS ( pci->busdevfn ) ) ) {
