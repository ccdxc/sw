                   ...
                   unsigned int lif_index, unsigned int intr_index)
{
    ...
    DBG("adminq_init.pid %d\n", cmd.adminq_init.pid);
    DBG("adminq_init.index %d\n", cmd.adminq_init.index);
    DBG("adminq_init.ring_base %llx\n", cmd.adminq_init.ring_base);
    DBG("adminq_init.ring_size %d\n", cmd.adminq_init.ring_size);
    ionic_dev_cmd_go(idev, &cmd);
}
...
int ionic_identify(struct ionic *ionic)
{
    ...
    ident->drv.os_type = OS_TYPE_IXPE;
    ident->drv.os_dist = 0;
...
static int ionic_lif_adminq_init(struct lif *lif)
{
    ...
    ionic_dev_cmd_adminq_init(idev, q, lif->index, DEFAULT_INTR_INDEX);
    err = ionic_dev_cmd_wait_check(idev, devcmd_timeout);
    if (err) {
