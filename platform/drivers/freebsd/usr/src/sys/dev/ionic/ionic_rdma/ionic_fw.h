...
static inline void ionic_v1_set_send_ugl(union ionic_v1_ugl *ugl,
					 int num_sge, int sge_i,
					 __le64 va, __le32 length, __le32 lkey)
{
	int start, even, odd;
	...
	ugl[even].length[odd] = length;
	ugl[start + 3 * even + 1].lkey[odd] = lkey;
	ugl[start + 3 * even + 2 * odd].va = va;
}
