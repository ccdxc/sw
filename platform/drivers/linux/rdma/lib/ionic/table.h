...
static inline void tbl_insert(struct tbl_root *tbl, void *val, uint32_t key)
{
	...
	assert(tbl->free_node != NULL);
	assert(key >> TBL_KEY_SHIFT == 0);
	assert(tbl_lookup(tbl, key) == NULL);

	if (!tbl->node[node_i]) {
		tbl->node[node_i] = tbl->free_node;
		tbl->free_node = NULL;
	}

	++tbl->refcount[node_i];
	tbl->node[node_i]->val[key & TBL_NODE_MASK] = val;
}
...
static inline void tbl_delete(struct tbl_root *tbl, uint32_t key)
{
	...
	tbl->node[node_i]->val[key & TBL_NODE_MASK] = NULL;

	--tbl->refcount[node_i];

	if (!tbl->refcount[node_i]) {
		tbl->free_node = tbl->node[node_i];
		tbl->node[node_i] = NULL;
	}
}
