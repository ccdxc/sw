...
enum ionic_v1_cqe_qtf_bits {
	IONIC_V1_CQE_COLOR		= (1u << 31),
	IONIC_V1_CQE_ERROR		= (1u << 30),
...
{
	return !!(cqe->qid_type_flags & htole32(IONIC_V1_CQE_COLOR));
}

static inline uint32_t ionic_v1_cqe_qtf(struct ionic_v1_cqe *cqe)
{
	return le32toh(cqe->qid_type_flags);
}

static inline bool ionic_v1_cqe_qtf_error(uint32_t qtf)
{
	return !!(qtf & IONIC_V1_CQE_ERROR);
}

static inline bool ionic_v1_cqe_qtf_type(uint32_t qtf)
{
