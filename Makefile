...
naples-firmware-tarball:
	@if [ "x${RELEASE}" = "x" ]; then echo "RELEASE is not set"; else cd ../ ; mv sw sw-${PIPELINE}; asset-push --assets-server-colo NULL builds hourly ${RELEASE} sw-${PIPELINE} || mv sw-${PIPELINE} sw; cd sw; fi
...
naples-firmware-tarball-iris: NAPLES_FW_TAR=naples_fw_all.tgz PIPELINE=iris
naples-firmware-tarball-iris: naples-firmware-tarball

naples-firmware-tarball-apulu: NAPLES_FW_TAR=naples_fw_all_apulu.tgz PIPELINE=apulu
