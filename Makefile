...
BUILD_CONTAINER ?= pens-bld:v0.15
...
e2e-retest:
	docker exec -it node0 sh -c 'PENS_SKIP_BOOTSTRAP=1 PENS_SKIP_AUTH=1 E2E_TEST=1 CGO_LDFLAGS_ALLOW="-I/usr/share/libtool" go test -v ./test/e2e/cluster -configFile=/import/src/github.com/pensando/sw/${E2E_CONFIG} -ginkgo.v -timeout 90m ${E2E_SEED}'
...
e2e-test:
	docker exec -it node0 sh -c 'E2E_TEST=1 CGO_LDFLAGS_ALLOW="-I/usr/share/libtool" go test -v ./test/e2e/cluster -configFile=/import/src/github.com/pensando/sw/${E2E_CONFIG} -ginkgo.v -timeout 90m ${E2E_SEED}'
...
naples-firmware:
	$(MAKE) -C nic PLATFORM=hw ARCH=aarch64 clean
	$(MAKE) ws-tools
	$(MAKE) -C nic PLATFORM=hw ARCH=aarch64 ASIC=${ASIC} firmware
...
naples-firmware-tarball:
	@if [ "x${RELEASE}" = "x" ]; then echo "RELEASE is not set"; else cd ../ ; if [ "${ASIC}" = "capri" ]; then REMOTE_NAME=sw-${PIPELINE}; else REMOTE_NAME=sw-${PIPELINE}-${ASIC}; fi; asset-push --assets-server-colo NULL --remote-name sw-${PIPELINE}.tar.gz builds hourly ${RELEASE} sw || cd sw; fi
