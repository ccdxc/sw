...
helper-containers:
	...
	@cd tools/docker-files/elasticsearch; docker build -t ${REGISTRY_URL}/elasticsearch-cluster:v0.9 .
...
e2e-retest:
	docker exec -it node0 sh -c 'PENS_SKIP_BOOTSTRAP=1 PENS_SKIP_AUTH=1 E2E_TEST=1 CGO_LDFLAGS_ALLOW="-I/usr/share/libtool" go test -v ./test/e2e/cluster -configFile=/import/src/github.com/pensando/sw/${E2E_CONFIG} -ginkgo.v -timeout 25m ${E2E_SEED}'
e2e-test:
...
naples-firmware-tarball:
	tar -zcf naples_fw_all.tgz nic/naples_*.tar platform/gen/drivers-*-eth.tar.xz platform/goldfw/naples/naples_fw.tar platform/hosttools/x86_64/*/goldfw/latest/drivers-*-eth.tar.xz platform/hosttools/x86_64/*/goldfw/old/drivers-*-eth.tar.xz nic/host.tar nic/box.rb nic/entrypoint.sh tools/test-build
