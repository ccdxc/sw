...
func TestAgentService_Node_Naples_Add_Delete(t *testing.T) {
	...
	iotaNode.Node = &iota.Node{Type: iota.PersonalityType_PERSONALITY_NAPLES, NodeName: "naples"}
	iotaNode.NodeInfo = &iota.IotaNode_NaplesConfig{NaplesConfig: &iota.NaplesConfig{ControlIntf: "eth1",
		DataIntfs: []string{"eth2"}, ControlIp: "10.1.1.2/24", DataIps: []string{"192.168.1.2/24"},
		VeniceIps: []string{"10.1.1.3/24"}, NaplesIps: []string{"192.168.1.2/24", "192.168.1.3/24"}}}

	nodeResp, err := agentClient.DeleteNode(context.Background(), iotaNode.GetNode())
	if err != nil {
	...
	TestUtils.Assert(t, resp.Node.GetNodeStatus().ApiStatus == iota.APIResponseType_API_STATUS_OK, "Add node failed")
	...
	TestUtils.Assert(t, resp.Node.GetNodeStatus().ApiStatus == iota.APIResponseType_API_BAD_REQUEST, "Duplicate add node success!")
...
func TestAgentService_Node_Venice_Add_Delete(t *testing.T) {
	...
	iotaNode.Node = &iota.Node{Type: iota.PersonalityType_PERSONALITY_VENICE, NodeName: "venice"}
	iotaNode.NodeInfo = &iota.IotaNode_VeniceConfig{VeniceConfig: &iota.VeniceConfig{ControlIntf: "eth1",
		ControlIp: "10.1.1.3/24", VeniceNodes: []*iota.VeniceNode{&iota.VeniceNode{HostName: "test", IpAddress: "1.2.3.4/24"},
			&iota.VeniceNode{HostName: "Test", IpAddress: "1.2.3.4/24"}}}}
