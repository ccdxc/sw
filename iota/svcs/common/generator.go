...
// GenMACAddresses generates the required number of pseudo random mac addresses
func GenMACAddresses(count int) ([]string, error) {
	// Fixed seed to generate repeatable IP Addresses
	var macAddresses []string
	//numbers := make([]int64, count)

	rand.Seed(IOTARandomSeed)
	for i := 0; i < count; i++ {
		b := make([]byte, 8)
		num := uint64(rand.Int63n(math.MaxInt64))
		binary.BigEndian.PutUint64(b, num)
		// HAL will complain if EP have a multicast MAC Address. This will ensure that we have unicast MAC
		b[0] = (b[0] | 2) & 0xfe
