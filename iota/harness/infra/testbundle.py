    ...
    def __execute_testcases(self):
        ...
        for _i in range(GlobalOptions.bundle_stress):
            ...
            if not selected_list:
                ...
                tcResult.closeResult(types.status.SKIPPED, time.strftime("%Y-%m-%dT%H:%M:%SZ",time.gmtime()), 0, "{}")
            ...
            for selected in selected_list:
                ...
                for tc in self.__testcases:
                    ...
                    startTime=time.time()
                    ret = tc.Main()
                    tcResult.closeResult(ret, time.strftime("%Y-%m-%dT%H:%M:%SZ",time.gmtime()), time.time()-startTime, tc.iterRes.toJson())
