...
function init_host() {
    (rm -rf /etc/docker/daemon.json && mkdir -p /etc/docker && echo "{\"insecure-registries\" : [\"registry.test.pensando.io:5000\"]}" >> /etc/docker/daemon.json && sync && systemctl restart docker)
