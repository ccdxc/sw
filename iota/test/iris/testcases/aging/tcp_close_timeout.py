...
def Trigger(tc):
    ...
    api.Trigger_AddCommand(req, server.node_name, server.workload_name, "nc -l 35535", background=True)
    tc.cmd_cookies1.append("start server")

    #Step 2: Start TCP Client
    api.Trigger_AddCommand(req, client.node_name, client.workload_name, 
                        "nc {} 35535 -p 45535".format(server.ip_address), background=True)
    tc.cmd_cookies1.append("start client")

    #Step 3: Get the session out from naples
    api.Trigger_AddNaplesCommand(req, naples.node_name, 
                "/nic/bin/halctl show session --dstport 35535 --dstip {} --yaml".format(server.ip_address))
    ...
    api.Trigger_AddCommand(req, client.node_name, client.workload_name, 
               "hping3 -c 1 -s 45535 -p 35535 -M {}  -L {} --ack --tcp-timestamp {} -d 0 -F".format(iseq_num+1, iack_num, server.ip_address))
    tc.cmd_cookies2.append("Send FIN")

    #Step 6: Send FIN ACK now from other side after timeout
    api.Trigger_AddCommand(req, server.node_name, server.workload_name,
               "hping3 -c 1 -s 35535 -p 45535 -M {}  -L {} --ack --tcp-timestamp {} -d 0 -F".format(rseq_num+1, rack_num, client.ip_address))
    tc.cmd_cookies2.append("Send FIN ACK")

    api.Trigger_AddCommand(req, client.node_name, client.workload_name,
               "hping3 -c 1 -s 45535 -p 35535 -M {}  -L {} --ack --tcp-timestamp {} -d 0".format(iseq_num, iack_num+1, server.ip_address))
    tc.cmd_cookies2.append("Send ACK")

    #Step 6: Check if session is up in FIN_RCVD state
    api.Trigger_AddNaplesCommand(req, naples.node_name, "/nic/bin/halctl show session --dstport 35535 --dstip {} --srcip {} | grep BIDIR_FIN".format(server.ip_address, client.ip_address))
