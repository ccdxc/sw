...
def Trigger(tc):
    ...
    api.Trigger_AddCommand(req, server.node_name, server.workload_name, "nc -l 23435 -i 600s", background=True)
    ...
    api.Trigger_AddCommand(req, client.node_name, client.workload_name, "hping3 -c 1 -s 52252 -p 23435 -S -M 1000 {}".format(server.ip_address))
    tc.cmd_cookies1.append("Send SYN");

    #Step 4: Check if session is up in SYN_RCVD state
    api.Trigger_AddNaplesCommand(req, naples.node_name, "/nic/bin/halctl show session --dstport 23435 --dstip {} --srcip {} | grep SYN".format(server.ip_address, client.ip_address))
    ...
    api.Trigger_AddCommand(req, client.node_name, client.workload_name, "hping3 -c 1 -s 52252 -p 23435 -A -M 1000 {}".format(server.ip_address))
    ...
    api.Trigger_AddCommand(req, server.node_name, server.workload_name, "grep \"52252 > {}.23435\" out.txt | grep \"\[.\]\"".format(server.ip_address))
...
def Verify(tc):
    ...
    for cmd in tc.resp.commands:
        ...
        if cmd.exit_code != 0 and not api.Trigger_IsBackgroundCommand(cmd):
            ...
            if (tc.cmd_cookies1[cookie_idx].find("After timeout") != -1 and cmd.stdout == ''):
                result = api.types.status.SUCCESS
            else:
