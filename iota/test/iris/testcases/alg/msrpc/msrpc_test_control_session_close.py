...
def Trigger(tc):
    ...
    msrpcpcap = dir_path + '/' + "msrpc_multi_bind.pcap"
    ...
    api.Trigger_AddCommand(req, server.node_name, server.workload_name,
                           "sudo nc -l 49302", background=True)
    tc.cmd_cookies.append("msrpc start-server")

    api.Trigger_AddCommand(req, client.node_name, client.workload_name,
                           "sudo nc %s 49302 -p %s"%(server.ip_address, api.AllocateTcpPort()), background=True)
    ...
    trig_resp = api.Trigger(req)
    term_resp = api.Trigger_TerminateAllCommands(trig_resp)
    tc.resp3 = api.Trigger_AggregateCommandsResponse(trig_resp, term_resp)
