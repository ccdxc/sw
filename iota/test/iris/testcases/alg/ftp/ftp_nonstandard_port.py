...
def Setup(tc):
    update_sgpolicy('ftp_nonstandard_port') 
...
def Trigger(tc):
    ...
    listenport=2300
    SetupFTPServer(server.node_name, server.workload_name, listen_port=listenport, restart=True)
    ...
    api.Trigger_AddCommand(req, client.node_name, client.workload_name,
                           "cd ftpdir && lftp -p %d -u admin,linuxpassword %s -e 'get ftp/ftp_server.txt; exit'"%(listenport, server.ip_address))
    tc.cmd_cookies.append("Run FTP GET")

    api.Trigger_AddCommand(req, client.node_name, client.workload_name,
                           "cd ftpdir && lftp -p %d -u admin,linuxpassword %s -e 'put ftp_client.txt; exit'"%(listenport, server.ip_address))
