...
def Trigger(tc):
    ...
    fullpath = dir_path + '/' + "ftp.py"
    api.Logger.info("fullpath %s" % (fullpath))
    resp = api.CopyToWorkload(w1.node_name, w1.workload_name, [fullpath], 'ftpdir')
    if resp is None:
       return api.types.status.FAILURE

    server_cmd = "python3 -m pyftpdlib -i %s -p 21 -u admin -P root -d ftpdir" % (w1.ip_address)
