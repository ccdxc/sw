...
def Trigger(tc):
    ...
    for pair in tc.workload_pairs:
       ...
       api.Trigger_AddCommand(req, w2.node_name, w2.workload_name,
                           "cd tftpdir && atftp  -p -l tftp_client.txt -r tftp_client.txt --option \"blksize 6546\" --trace %s" % w1.ip_address)
       tc.cmd_cookies.append("TFTP put Server: %s(%s) <--> Client: %s(%s)" %\
                           (w1.workload_name, w1.ip_address, w2.workload_name, w2.ip_address))

       api.Trigger_AddCommand(req, w2.node_name, w2.workload_name,
                           "cd tftpdir && atftp  -g -l tftp_server.txt -r tftp_server.txt --option \"blksize 6546\" --trace %s" % w1.ip_address)
