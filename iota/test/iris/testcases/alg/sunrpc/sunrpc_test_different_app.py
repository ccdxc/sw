...
def Trigger(tc):
    ...
    api.Trigger_AddCommand(req, client.node_name, client.workload_name,
                           "hping3 -c 1 -s 45535 -p {} -d 0 -S {}".format(2048, server.ip_address))
    tc.cmd_cookies.append("Send SYN for different dport")

    api.Trigger_AddNaplesCommand(req, naples.node_name,
                           "/nic/bin/halctl show session | grep INIT")
    tc.cmd_cookies.append("show drop session")
    ...
    for cmd in tc.resp.commands:
        ...
        if cmd.exit_code != 0 and not api.Trigger_IsBackgroundCommand(cmd):
            if tc.cmd_cookies[cookie_idx].find("Before") != -1 or\
               tc.cmd_cookies[cookie_idx].find("Send SYN for different dport") != -1 or \
        ...
        if tc.cmd_cookies[cookie_idx].find("show session") != -1 and \
           cmd.stdout == '':
           result = api.types.status.FAILURE
        if tc.cmd_cookies[cookie_idx].find("show drop session") != -1 and \
           cmd.stdout == '':
           result = api.types.status.FAILURE
