...
def Trigger(tc):
    ...
    api.Trigger_AddCommand(req2, server.node_name, server.workload_name,
                           "hping3 -c 1 -s 45535 -p {} -d 0 -S {}".format(dport, client1.ip_address))
    ...
    api.Trigger_AddNaplesCommand(req2, naples.node_name,
                           "/nic/bin/halctl show session --srcip %s | grep SYN"%(client1.ip_address))
