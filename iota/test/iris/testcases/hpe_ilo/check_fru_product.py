...
def Trigger(tc):
    ...
    for instance in tbspec.Instances:
        ...
        try :
            ...
            child.expect(">\r\n>")
            child.sendline("i2c a 0xa0 w 0x25 r 9")
            child.expect(">\r\n>")
            fru_product = child.before
            fru_product_data = fru_product.decode("utf-8")
            fru_product_str = fru_product_data.split('Data length 9:')[1]
            fru_product_str = fru_product_str.replace("\r\n", "")
            fru_product_str = fru_product_str.replace("\x1b[0m", "")
            fru_product_str = fru_product_str.replace("\t", "")
            fru_product_str = fru_product_str.strip()
            manu_list = fru_product_str.split(" ")
            res = ""
            for i in range(len(manu_list)):
                res = res + chr(int(manu_list[i], 16))
            api.Logger.info("Product string from fru is {}".format(res))
            if (res == "NAPLES 25"):
                result = api.types.status.SUCCESS
            else:
