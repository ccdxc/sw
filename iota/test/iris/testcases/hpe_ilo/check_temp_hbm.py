...
def Trigger(tc):
    ...
    for instance in tbspec.Instances:
        ...
        try :
            ...
            child.expect(">\r\n>")
            child.sendline("i2c a 0x94 w 0x17 r 1")
            child.expect(">\r\n>")
            cpld_temp_hbm = child.before
            cpld_temp_hbm_data = cpld_temp_hbm.decode("utf-8")
            cpld_temp_hbm_str = cpld_temp_hbm_data.split('Response Data length 1:')[1]
            cpld_temp_hbm_str = cpld_temp_hbm_str.replace("\r\n", "")
            cpld_temp_hbm_str = cpld_temp_hbm_str.replace("\x1b[0m", "")
            cpld_temp_hbm_str = cpld_temp_hbm_str.replace("\t", "")
            cpld_temp_hbm_str = cpld_temp_hbm_str.strip()
            res = int(cpld_temp_hbm_str, 16)
            api.Logger.info("HBM temperature is {}".format(res))
            if (res != 0):
                result = api.types.status.SUCCESS
            else:
