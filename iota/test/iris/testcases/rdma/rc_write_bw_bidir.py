...
def Trigger(tc):
    ...
    # cmd for server
    cmd = "ib_write_bw -d " + tc.devices[i] + " -n 10 -F -x " + tc.gid[i] + " -s 1024 -b -q " + str(tc.iterators.num_qp) + " --report_gbits"
    ...
    # cmd for client
    cmd = "ib_write_bw -d " + tc.devices[j] + " -n 10 -F -x " + tc.gid[j] + " -s 1024 -b -q " + str(tc.iterators.num_qp) + " --report_gbits " + w1.ip_address
    api.Trigger_AddCommand(req, 
                           w2.node_name, 
                           w2.workload_name,
                           tc.ib_prefix[j] + cmd)
