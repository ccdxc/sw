...
def Trigger(tc):
    ...
    # cmd for server
    cmd = "ib_read_bw -d " + tc.w1_device + " -n 10 -F -x " + tc.w1_gid + " -s 1024 -b --report_gbits"
    ...
    # cmd for client
    cmd = "ib_read_bw -d " + tc.w2_device + " -n 10 -F -x " + tc.w2_gid + " -s 1024 -b --report_gbits " + w1.ip_address
    api.Trigger_AddCommand(req, 
                           w2.node_name, 
                           w2.workload_name,
                           tc.ib_prefix + cmd)
