...
def Trigger(tc):
    ...
    # cmd for server
    cmd = "ib_send_bw -d " + tc.devices[i] + " -n 10 -F -x " + tc.gid[i] + " -s 65536 -b -q " + str(tc.iterators.num_qp) + " --report_gbits"
    ...
    # cmd for client
    cmd = "ib_send_bw -d " + tc.devices[j] + " -n 10 -F -x " + tc.gid[j] + " -s 65536 -b -q " + str(tc.iterators.num_qp) + " --report_gbits " + w1.ip_address
    # append num_sge's option only if node is naples
    if w2.IsNaples():
        cmd += ' -W 16'
