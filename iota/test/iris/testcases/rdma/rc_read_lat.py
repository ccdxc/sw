...
def Trigger(tc):
    ...
    # cmd for server
    cmd = "ib_read_lat -d " + tc.devices[i] + " -n 10 -F -x " + tc.gid[i] + " -s 1024 --report_gbits"
    ...
    # cmd for client
    cmd = "ib_read_lat -d " + tc.devices[j] + " -n 10 -F -x " + tc.gid[j] + " -s 1024 --report_gbits " + w1.ip_address
    api.Trigger_AddCommand(req, 
                           w2.node_name, 
                           w2.workload_name,
                           tc.ib_prefix[j] + cmd)
