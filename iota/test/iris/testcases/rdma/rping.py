...
def Trigger(tc):
    ...
    while (i < 2):
        ...
        # cmd for server
        cmd = "rping -s -a %s -C 10 -S %d " % (w1.ip_address, tc.iterators.pktsize)
        ...
        # cmd for client
        cmd = "rping -c -a %s -C 10 -S %d " % (w1.ip_address, tc.iterators.pktsize)
        api.Trigger_AddCommand(req,
                               w2.node_name,
                               w2.workload_name,
                               tc.ib_prefix[j] + cmd)
...
def Verify(tc):
    ...
    api.Logger.info("rping results for %s" % (tc.cmd_descr))
