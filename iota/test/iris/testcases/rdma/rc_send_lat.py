...
def Trigger(tc):
    ...
    api.Logger.info("Starting ib_send_lat test from %s" % (tc.cmd_descr))

    # cmd for server
    cmd = "ib_send_lat -d " + tc.w1_device + " -n 10 -F -x " + tc.w1_gid + " -s 1024 --report_gbits"
    ...
    # cmd for client
    cmd = "ib_send_lat -d " + tc.w2_device + " -n 10 -F -x " + tc.w2_gid + " -s 1024 --report_gbits " + w1.ip_address
    api.Trigger_AddCommand(req, 
                           w2.node_name, 
                           w2.workload_name,
                           tc.ib_prefix + cmd)
...
def Verify(tc):
    ...
    api.Logger.info("ib_send_lat results for %s" % (tc.cmd_descr))
