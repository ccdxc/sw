...
def Trigger(tc):
    ...
    # cmd for server
    cmd = "ib_send_bw -d " + w1_device + " -n 10 -F -x " + w1_gid + " -s 1024 -b -q 2 --report_gbits"
    api.Trigger_AddCommand(req, 
                           w1.node_name, 
                           w1.workload_name,
                           tc.ib_prefix + cmd,
    ...
    # cmd for client
    cmd = "ib_send_bw -d " + w2_device + " -n 10 -F -x " + w2_gid + " -s 1024 -b -q 2 --report_gbits " + w1.ip_address
    api.Trigger_AddCommand(req, 
                           w2.node_name, 
                           w2.workload_name,
                           tc.ib_prefix + cmd)
