...
def Trigger(tc):
    ...
    # cmd for server
    cmd = "ib_send_bw -d " + tc.w1_device + " -n 5 -F -x " + tc.w1_gid + " -m 4096 -s 4500 -b -q 2 --report_gbits"
    ...
    # cmd for client
    cmd = "ib_send_bw -d " + tc.w2_device + " -n 5 -F -x " + tc.w2_gid + " -m 4096 -s 4500 -b -q 2 --report_gbits " + w1.ip_address
    api.Trigger_AddCommand(req, 
                           w2.node_name, 
                           w2.workload_name,
                           tc.ib_prefix + cmd)
