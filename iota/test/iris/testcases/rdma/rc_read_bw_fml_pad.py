...
def Trigger(tc):
    ...
    while (i < 2):
        ...
        # cmd for server
        cmd = "ib_read_bw -d " + tc.devices[i] + " -n 10 -F -x " + tc.gid[i] + " -m 4096 -s 12601 -q " + str(tc.iterators.num_qp) + " --report_gbits"
        ...
        # cmd for client
        cmd = "ib_read_bw -d " + tc.devices[j] + " -n 10 -F -x " + tc.gid[j] + " -m 4096 -s 12601 -q " + str(tc.iterators.num_qp) + " --report_gbits " + w1.ip_address
        api.Trigger_AddCommand(req, 
                               w2.node_name, 
                               w2.workload_name,
                               tc.ib_prefix[j] + cmd)
