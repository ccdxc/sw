...
def verify_sessions_old_dsc(tc, wl, node):
    ...
    wl_ips    = []
    for wl in workloads:
        wl.append(wl.ip_address)
        
    for session in sessions:
        if session != None:
            i_flow_key  = session['spec']['initiatorflow']['flowkey']['flowkey']['v4key']
            #i_sip = str(ipaddress.ip_address(sip))
            i_sip       = i_flow_key['sip']
            i_dip       = i_flow_key['dip']
            if i_sip == wl.ip_address:
                ip = i_dip
            else:
                ip = i_sip
            if ip not in wl_ips:
                api.Logger.info("Session found on old node with local wl sip {} dip {}".format(i_sip, i_dip))
                return api.types.status.FAILURE
    return api.types.status.SUCCESS
...
def Verify(tc):
    ...
    for wl_info in tc.move_info:
        #delete ep on old node
        delete_ep_info(tc, wl_info.wl, wl_info.old_node)
        ret = verify_sessions_old_dsc(tc, wl_info.wl, wl_info.old_node)
        if ret != api.types.status.SUCCESS:
            return ret
    return api.types.status.SUCCESS
