...
def Setup(tc):
    ...
    add_command(req, tc, 'server', tc.server, cmd_cookie)
    add_command(req, tc, 'client', tc.client, cmd_cookie)
...
def Trigger(tc):
    ...
    if tc.resp_flow:
        ...
        cmd_cookie = "hping3 -c 1 -s 1237 -p 52255 -M {}  -L {} --ack --tcp-timestamp {} -d 10".format(wrap_around(tc.pre_ctrckinf.r_tcpseqnum, -200), tc.pre_ctrckinf.r_tcpacknum, tc.client.ip_address)    
        ...
        #left out of window - retransmit
        cmd_cookie = "hping3 -c 1 -s 52255 -p 1237 -M {}  -L {} --ack --tcp-timestamp {} -d 10".format(wrap_around(tc.pre_ctrckinf.i_tcpseqnum, -200), tc.pre_ctrckinf.i_tcpacknum, tc.server.ip_address)   
    ...
    #Serve may or may not be naples. Hence this check
