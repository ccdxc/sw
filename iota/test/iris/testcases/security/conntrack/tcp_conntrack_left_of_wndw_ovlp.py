...
def Trigger(tc):
    ...
    if resp_flow:
        ...
        cmd_cookie = "hping3 -c 1 -s 1237 -p 52255 -M {}  -L {} --ack --tcp-timestamp {} -d 100".format(wrap_around(rseq_num, -10), rack_num, client.ip_address)    
        add_command(req, tc, 'fail ping', server, cmd_cookie)
    else:
        #left of window partial overlap
        cmd_cookie = "hping3 -c 1 -s 52255 -p 1237 -M {}  -L {} --ack --tcp-timestamp {} -d 100".format(wrap_around(iseq_num,-10), iack_num, server.ip_address)   
