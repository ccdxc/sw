...
def Trigger(tc):
    ...
    if tc.resp_flow:
        ...
        cmd_cookie = "hping3 -c 1 -s 1237 -p 52255 -M {}  -L {} --ack --tcp-timestamp {} -d 100".format(wrap_around(tc.pre_ctrckinf.r_tcpseqnum, -10), tc.pre_ctrckinf.r_tcpacknum, tc.client.ip_address)    
        add_command(req, tc, 'fail ping', tc.server, cmd_cookie)
    else:
        #left of window partial overlap
        cmd_cookie = "hping3 -c 1 -s 52255 -p 1237 -M {}  -L {} --ack --tcp-timestamp {} -d 100".format(wrap_around(tc.pre_ctrckinf.i_tcpseqnum,-10), tc.pre_ctrckinf.i_tcpacknum, tc.server.ip_address)   
