...
def Trigger(tc):
    ...
    if tc.resp_flow:
        ...
        cmd_cookie = "hping3 -c 1 -s {} -p {} -M {}  -L {} --ack --tcp-timestamp {} -d 100".format(server_port, client_port, wrap_around(tc.pre_ctrckinf.r_tcpseqnum, -10), tc.pre_ctrckinf.r_tcpacknum, tc.client.ip_address)    
        add_command(req, tc, 'fail ping', tc.server, cmd_cookie)
    else:
        #left of window partial overlap
        cmd_cookie = "hping3 -c 1 -s {} -p {} -M {}  -L {} --ack --tcp-timestamp {} -d 100".format(client_port, server_port, wrap_around(tc.pre_ctrckinf.i_tcpseqnum,-10), tc.pre_ctrckinf.i_tcpacknum, tc.server.ip_address)   
