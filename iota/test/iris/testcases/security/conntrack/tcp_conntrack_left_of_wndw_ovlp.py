...
def Trigger(tc):
    ...
    cmd_cookie = "nc {} 1236 -p 52254".format(server.ip_address)
    ...
    cmd_cookie = "/nic/bin/halctl show session --dstport 1236 --dstip {} --yaml".format(server.ip_address)
    ...
    cmd_cookie = "hping3 -c 1 -s 52254 -p 1236 -M {}  -L {} --ack --tcp-timestamp {} -d 100".format(iseq_num - 10, iack_num, server.ip_address)    
    api.Trigger_AddCommand(req, client.node_name, client.workload_name, cmd_cookie)
    tc.cmd_cookies['fail ping'] = cmd_cookie

    cmd_cookie = "sleep 3 && /nic/bin/halctl show session --dstport 1236 --dstip {} --yaml".format(server.ip_address)
    api.Trigger_AddNaplesCommand(req, client.node_name, cmd_cookie)
