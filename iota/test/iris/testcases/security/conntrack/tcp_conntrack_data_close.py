...
def Trigger(tc):
    ...
    if tc.resp_flow:
        ...
        cmd_cookie = "hping3 -c 1 -s 1237 -p 52255 -M {}  -L {} --ack --fin --tcp-timestamp {} ".format(
        ...
        add_command(req, tc, 'show after1', tc.client, cmd_cookie, naples=True)

        cmd_cookie = "hping3 -c 1 -s 1237 -p 52255 -M {}  -L {} --ack -d 10 --tcp-timestamp {} ".format(
                wrap_around(tc.pre_ctrckinf.r_tcpseqnum+1, 0), tc.pre_ctrckinf.r_tcpacknum, tc.client.ip_address)    
        add_command(req, tc, 'syn after close', tc.server, cmd_cookie)
    else:
        new_seq_num = tc.pre_ctrckinf.i_tcpseqnum + tc.pre_ctrckinf.r_tcpwinsz * (2 ** tc.pre_ctrckinf.r_tcpwinscale)
        cmd_cookie = "hping3 -c 1 -s 52255 -p 1237 -M {}  -L {} --ack --tcp-timestamp {} --fin".format(
        ...
        cmd_cookie = "sleep 3 && /nic/bin/halctl show session --dstport 1237 --dstip {} --yaml".format(tc.server.ip_address)
        add_command(req, tc, 'show after2', tc.client, cmd_cookie, naples=True)
	
        cmd_cookie = "hping3 -c 1 -s 52255 -p 1237 -M {}  -L {} --ack --tcp-timestamp {} -d 10".format(
                wrap_around(tc.pre_ctrckinf.i_tcpseqnum+1, 0), tc.pre_ctrckinf.i_tcpacknum, tc.server.ip_address)   
        add_command(req, tc,"syn after close", tc.client, cmd_cookie)
...
def Verify(tc):
    ...
    for cmd in tc.resp.commands:
        ...
        if tc.cmd_cookies['show after'] == cmd.command:     
            ...
            if (excep['tcpdataafterfin'] == 'false'):
