...
def Trigger(tc):
    ...
    if resp_flow:
        ...
        add_command(req, tc, 'show after1', client, cmd_cookie, naples=True)

        cmd_cookie = "hping3 -c 1 -s 1237 -p 52255 -M {}  -L {} --ack -d 10 --tcp-timestamp {} ".format(wrap_around(rseq_num+1, 0), rack_num, client.ip_address)    
        add_command(req, tc, 'syn after close', server, cmd_cookie)
    else:
        ...
        cmd_cookie = "sleep 3 && /nic/bin/halctl show session --dstport 1237 --dstip {} --yaml".format(server.ip_address)
        add_command(req, tc, 'show after2', client, cmd_cookie, naples=True)
	
        cmd_cookie = "hping3 -c 1 -s 52255 -p 1237 -M {}  -L {} --ack --tcp-timestamp {} -d 10".format(wrap_around(iseq_num+1, 0), iack_num, server.ip_address)   
        add_command(req, tc,"syn after close", client, cmd_cookie)
