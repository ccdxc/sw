...
def Trigger(tc):
    ...
    if tc.resp_flow:
        cmd_cookie = "hping3 -c 1 -s {} -p {} -M {}  -L {} --rst --ack --tcp-timestamp {}".format(tc.server_port, 
         tc.client_port, wrap_around(tc.pre_ctrckinf.r_tcpseqnum,100), tc.pre_ctrckinf.r_tcpacknum, tc.client.ip_address)    
        add_command(req, tc, 'fail ping', tc.server, cmd_cookie)
    else:
        cmd_cookie = "hping3 -c 1 -s {} -p {} -M {}  -L {} --rst --ack --tcp-timestamp {}".format(tc.client_port, 
         tc.server_port, wrap_around(tc.pre_ctrckinf.i_tcpseqnum,100), tc.pre_ctrckinf.i_tcpacknum, tc.server.ip_address)   
