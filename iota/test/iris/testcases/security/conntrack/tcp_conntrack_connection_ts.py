...
def Trigger(tc):
    ...
    if resp_flow:
        ...
        cmd_cookie = "hping3 -c 1 -s 1237 -p 52255 -M {}  -L {} --ack  {} -d 900 --tcp-timestamp".format(wrap_around(rseq_num, 0), rack_num, client.ip_address)    
        add_command(req, tc, 'fail ping', server, cmd_cookie)
    else:
        
        cmd_cookie = 'tcpdump -nni {} "src host {} and dst host {} and src port 1237"'.format(client.interface, client.ip_address, server.ip_address)
        print(cmd_cookie)
        add_command(req, tc, 'tcpdump', client, cmd_cookie, True)
