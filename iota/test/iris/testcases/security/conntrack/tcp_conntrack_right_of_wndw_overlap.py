...
def Trigger(tc):
    ...
    if resp_flow:
        ...
        cmd_cookie = "hping3 -c 1 -s 1237 -p 52255 -M {}  -L {} --ack --tcp-timestamp {} -d {}".format(wrap_around(rseq_num + iwindo_size, -200), rack_num, client.ip_address, 500)    
        add_command(req, tc, 'fail ping', server, cmd_cookie)
    else:
        rwindo_size= rwindo_sz * (2 ** rwinscale)
        #right of window overlap
        cmd_cookie = "hping3 -c 1 -s 52255 -p 1237 -M {}  -L {} --ack --tcp-timestamp {} -d {}".format(wrap_around(iseq_num + rwindo_size, -200), iack_num, server.ip_address, 500)    
