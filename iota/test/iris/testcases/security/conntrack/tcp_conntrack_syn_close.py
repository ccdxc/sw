...
def Trigger(tc):
    ...
    if tc.resp_flow:
        ...
        cmd_cookie = "hping3 -c 1 -s 1237 -p 52255 -M {}  -L {} --ack --syn --tcp-timestamp {} ".format(wrap_around(tc.pre_ctrckinf.r_tcpseqnum+1, 0), tc.pre_ctrckinf.r_tcpacknum, tc.client.ip_address)    
        ...
        cmd_cookie = "hping3 -c 1 -s 52255 -p 1237 -M {}  -L {} --ack --tcp-timestamp {} --syn".format(wrap_around(tc.pre_ctrckinf.i_tcpseqnum+1, 0), tc.pre_ctrckinf.i_tcpacknum, tc.server.ip_address)   
