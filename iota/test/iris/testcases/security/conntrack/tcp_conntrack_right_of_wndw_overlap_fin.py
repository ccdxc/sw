...
def Trigger(tc):
    ...
    if tc.resp_flow:
        ...
        cmd_cookie = "hping3 -c 1 -s {} -p {} -M {}  -L {} --fin --ack --tcp-timestamp {} -d {}".format(tc.server_port, tc.client_port, wrap_around(tc.pre_ctrckinf.r_tcpseqnum + iwindo_size, -200), tc.pre_ctrckinf.r_tcpacknum, tc.client.ip_address, 500)    
        add_command(req, tc, 'fail ping', tc.server, cmd_cookie)
    else:
        rwindo_size= tc.pre_ctrckinf.r_tcpwinsz * (2 ** tc.pre_ctrckinf.r_tcpwinscale)
        #right of window overlap
        cmd_cookie = "hping3 -c 1 -s {} -p {} -M {}  -L {} --fin --ack --tcp-timestamp {} -d {}".format(tc.client_port, tc.server_port, wrap_around(tc.pre_ctrckinf.i_tcpseqnum + rwindo_size, -200), tc.pre_ctrckinf.i_tcpacknum, tc.server.ip_address, 500)    
