...
MIN_NUMBER_OF_CLIENT_SERVER_OBJECTS   = 2
        ...
        if "show flow-drop: Create case" in tc.cmd_cookies[cookie_idx]:
            ...
            for line in cmd.stdout.split('\n'):
                ...
                if "IPv4FlowDropMetrics" in newline:
                    ...
                    for s in newline.split():
                        if s.isdigit():
                            if int(s) != INVALID_ICMP_TYPE_4 and\
                               int(s) != INVALID_ICMP_TYPE_6:
                                print("************ERROR************")
                                print(line)
                                result = api.types.status.FAILURE
                            else:
                    ...
                    for s in newline.split():
                        if s.isdigit():
                            if (int(s) & DROP_MALFORMED_PKT_TTL_0) == 0:
                                ...
                                validations += 1
            if objects < MIN_NUMBER_OF_CLIENT_SERVER_OBJECTS:
                print("************ ERROR objects ************", objects)
                api.PrintCommandResults(cmd)
                result = api.types.status.FAILURE
            if validations != (NUMBER_OF_VALIDATIONS_PER_OBJECT * objects):
                print("*********** ERROR validations ***********", validations)
                api.PrintCommandResults(cmd)
                result = api.types.status.FAILURE
        cookie_idx += 1
