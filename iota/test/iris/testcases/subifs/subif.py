...
def __generate_traffic_pairs():
    ...
    for w1 in subif_utils.getNativeWorkloads():
        ret_lst_subif = subif_utils.GetSubifs(w1.interface)
    ...
    for n1, n1_all_subifs in __curr_subifs.items():
        for n2, n2_all_subifs in __curr_subifs.items():
            ...
            for n1_if in n1_all_subifs:
                w1 = subif_utils.getWorkloadForInf(n1_if)
                for n2_if in n2_all_subifs:
                    w2 = subif_utils.getWorkloadForInf(n2_if)
...
def Trigger(tc):
    ...
    subif_count = tc.iterators.subif_count
    ...
    # as per <subif_count>
    __create_subifs(subif_count)

    # Generate traffic pairs with same vlan id
    all_traffic_pairs = []
    all_traffic_pairs.extend(tc.workload_pairs)
    all_traffic_pairs.extend(__generate_traffic_pairs())
    tc.workload_pairs = all_traffic_pairs
    ...
    tc.workloadpairs = api.GetRemoteWorkloadPairs()
