...
class Tcp:
    ...
    def match(self, **kwargs):
        ...
        if not isinstance(sp,int):
            logger.warning("While matching TCP, source port field 'sp' is must.")
            return False

        if not isinstance(dp,int):
            logger.warning("While matching TCP, destination port field 'dp' is must.")
            return False
