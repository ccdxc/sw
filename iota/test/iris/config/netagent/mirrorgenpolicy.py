...
def Main():
    ...
    GlobalOptions.cfg_dir = GlobalOptions.cfg_dir + '/gen/telemetry/mirror'
    ...
    for protocol in GlobalOptions.protocols:
        for action in GlobalOptions.actions:
            ...
            collector = mirrorpolicy_template['objects'][0]['spec']['collectors'][0]['export-config']['destination']
            del match_rules[:]
            verif =[]
            ...
            for i in range(0, len(EP)):
                for j in range(i+1, len(EP)):
                    for k in GlobalOptions.ports:
                        ...
                        match_rules.append(rule)
                        verif.append(get_verif(EP[j], EP[i], protocol, k, collector, action))
    json.dump(verif, open(GlobalOptions.cfg_dir +"/{}/mirror_mixed_verif.json".format('mixed'), "w"), indent=4)
    json.dump(mirrorpolicy, open(GlobalOptions.cfg_dir + "/{}/mirror_mixed_policy.json".format('mixed'), "w"), indent=4)
