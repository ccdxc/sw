...
class PolicyObjectClient(base.ConfigClientBase):
    ...
    def GenerateObjects(self, node, parent, vpc_spec_obj):
        ...
        #TODO: handle other objs
        self.__v4ingressobjs[node][vpcid] = dict()
        self.__v6ingressobjs[node][vpcid] = dict()
        self.__v4egressobjs[node][vpcid] = dict()
        self.__v6egressobjs[node][vpcid] = dict()
        self.__v4ipolicyiter[node][vpcid] = dict()
        self.__v6ipolicyiter[node][vpcid] = dict()
        self.__v4epolicyiter[node][vpcid] = dict()
        self.__v6epolicyiter[node][vpcid] = dict()
        ...
        def __add_v4policy(direction, v4rules, policytype, overlaptype):
            ...
            if direction == 'ingress':
                self.__v4ingressobjs[node][vpcid].update({obj.PolicyId: obj})
            else:
                self.__v4egressobjs[node][vpcid].update({obj.PolicyId: obj})
            self.Objs[node].update({obj.PolicyId: obj})
        ...
        if len(self.__v4ingressobjs[node][vpcid]) != 0:
            self.__v4ipolicyiter[node][vpcid] = utils.rrobiniter(self.__v4ingressobjs[node][vpcid].values())

        if len(self.__v6ingressobjs[node][vpcid]) != 0:
            self.__v6ipolicyiter[node][vpcid] = utils.rrobiniter(self.__v6ingressobjs[node][vpcid].values())

        if len(self.__v4egressobjs[node][vpcid]) != 0:
            self.__v4epolicyiter[node][vpcid] = utils.rrobiniter(self.__v4egressobjs[node][vpcid].values())

        if len(self.__v6egressobjs[node][vpcid]) != 0:
            self.__v6epolicyiter[node][vpcid] = utils.rrobiniter(self.__v6egressobjs[node][vpcid].values())
