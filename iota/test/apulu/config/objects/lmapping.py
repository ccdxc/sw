...
class LocalMappingObjectClient(base.ConfigClientBase):
    ...
    def GenerateObjects(self, node, parent, vnic_spec_obj):
        ...
        while c < lmap_spec.count:
            if isV6Stack:
                obj = LocalMappingObject(parent, lmap_spec, utils.IP_VERSION_6, v6c)
                self.Objs[node].update({obj.MappingId: obj})
                c = c + 1
                v6c = v6c + 1
            if c < lmap_spec.count and isV4Stack:
                obj = LocalMappingObject(parent, lmap_spec, utils.IP_VERSION_4, v4c)
                self.Objs[node].update({obj.MappingId: obj})
                c = c + 1
                v4c = v4c + 1
        return
