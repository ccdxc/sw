...
class RouteObjectClient:
    ...
    def CreateObjects(self):
        ...
        for device in self.devices:
            logger.info("Creating route objects for device :", device)
            device.client.Create(agent_api.ObjectTypes.ROUTE, msgs)
...
class RouteTableObjectHelper:
    ...
    def __is_mapping_match(self, route_obj, lobj):
    ...
    def GetMatchingConfigObjects(self, selectors, all_objs):
        ...
        for route_obj in client.Objects():
            ...
            if rtype != 'empty' and 0 == len(route_obj.routes):
                ...
                continue
            for lobj in mapping.GetMatchingObjects(selectors):
                if self.__is_mapping_match(route_obj, lobj):
...
client = None
def NewRouteObjectClient(devices):
    global client
    client = RouteObjectClient(devices)
