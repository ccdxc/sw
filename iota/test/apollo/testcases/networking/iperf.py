...
def Trigger(tc):
    tc.cmd_cookies, tc.resp = traffic_utils.iperfWorkloads(tc.workload_pairs, tc.iterators.ipaf, tc.iterators.proto, tc.iterators.pktsize)
    return api.types.status.SUCCESS

def Verify(tc):
    return traffic_utils.verifyIPerf(tc.cmd_cookies, tc.resp)
