...
func setup(t *testing.T, eventsStorePath string, dedupInterval, batchInterval time.Duration) (*RPCServer,
	*rpckit.RPCClient, *exporters.MockExporter, epgrpc.EventsProxyAPIClient) {
	...
	return rpcServer, rpcClient, mockExporter, proxyClient
}
...
func TestEventsProxyRPCServer(t *testing.T) {
	...
	rpcServer, rpcClient, mockExporter, proxyClient := setup(t, eventsStorePath, testDedupInterval, testSendInterval)
	...
	// make sure the exporter received all the events;
	// ensure all the events are received and they're deduped
	AssertEventually(t, func() (bool, interface{}) {
		numEvents := mockExporter.GetTotalEvents()
		if numEvents == 10 {
			return true, nil
		}
...
// TestDedupedEvents tests to make sure the events are deduped before sent to the exporters.
func TestDedupedEvents(t *testing.T) {
	...
	rpcServer, rpcClient, mockExporter, proxyClient := setup(t, eventsStorePath, testDedupInterval, testSendInterval)
	defer rpcServer.Stop()
	// all the exporters will be stopped once the server stops -> dispatcher shutdown
...
func TestEventsProxyShutdown(t *testing.T) {
	...
	rpcServer, rpcClient, mockExporter, proxyClient := setup(t, eventsStorePath, 100*time.Second, 30*time.Second)
	defer rpcServer.Stop() // will shutdown the dispatcher and exporters
	defer rpcClient.ClientConn.Close()
	...
	// send interval is set large; no events received by the exporter so far
	Assert(t, mockExporter.GetTotalEvents() == 0, "expected 0 events")
