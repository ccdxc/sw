...
export function getSwagggerFiles() {
  ...
  generatedApiFiles.forEach( (generatedApiFile) => {
    if (fs.statSync(generatedApi+'/'+generatedApiFile).isDirectory() && 
      ...
      const swaggerFiles = fs.readdirSync(generatedApi+'/'+generatedApiFile+'/swagger/internal').filter(function (file) {
      ...
      swaggerFiles.forEach( (swaggerFile) => {
        ...
        if (generatedApiFile === 'search') {
          config = genConfig('v1', folderName, generatedApi+'/'+generatedApiFile+'/swagger/internal/'+ swaggerFile, true);
        } else {
          config = genConfig('v1', folderName, generatedApi+'/'+generatedApiFile+'/swagger/internal/'+ swaggerFile);
        }
