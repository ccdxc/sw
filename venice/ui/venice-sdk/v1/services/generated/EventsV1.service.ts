...
export class EventsV1Service extends AbstractService {
  ...
  */
  ...
  public GetEvent(UUID):Observable<{body: IEventsEvent, statusCode: number}> {
    let url = this['baseUrlAndPort'] + '/events/v1/event/{UUID}';
    url = url.replace('{UUID}', UUID);
    return this.invokeAJAXGetCall(url, 'GetEvent') as Observable<{body: IEventsEvent, statusCode: number}>;
  }
  
  /** http://<...>/v1/events/ */
  public GetEvents():Observable<{body: IEventsEventList, statusCode: number}> {
    let url = this['baseUrlAndPort'] + '/events/v1/events';
    return this.invokeAJAXGetCall(url, 'GetEvents') as Observable<{body: IEventsEventList, statusCode: number}>;
  }
