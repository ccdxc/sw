...
func (v *VCHStore) handleWorkload(m defs.StoreMsg) {
	meta := &api.ObjectMeta{
	...
	existingWorkload, err := v.apicl.WorkloadV1().Workload().Get(context.Background(), meta)
	if err != nil {
	...
	if toDelete {
		...
		v.apicl.WorkloadV1().Workload().Delete(context.Background(), &workloadObj.ObjectMeta)
		return
	}
	...
	if existingWorkload == nil {
		v.apicl.WorkloadV1().Workload().Create(context.Background(), workloadObj)
	} else {
		v.apicl.WorkloadV1().Workload().Update(context.Background(), workloadObj)
	}
...
func (v *VCHStore) processVMConfig(workload *workload.Workload, prop types.PropertyChange) {
	...
	if !ok {
		log.Errorf("Expected VirtualMachineConfigInfo, got %s", reflect.TypeOf(prop.Val).Name())
		return
	}
	interfaces := extractInterfaces(vmConfig)
...
func extractInterfaces(vmConfig types.VirtualMachineConfigInfo) []workload.WorkloadIntfSpec {
...
func GetVeth(d types.BaseVirtualDevice) *types.VirtualEthernetCard {
	...
	default:
		log.Infof("Ignoring virtual device %s", dKind.Name())
		return nil
	}
}
