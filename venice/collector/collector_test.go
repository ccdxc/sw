...
func TestCollectorBasic(t *testing.T) {
	...
	c := NewCollector(context.Background()).WithPeriod(testPeriod)
...
func TestCollectorDual(t *testing.T) {
	...
	c := NewCollector(context.Background()).WithPeriod(testPeriod).WithSize(12)
...
func TestCollectorLineDual(t *testing.T) {
	// start two servers
	s1 := &mockdb.MockTSDB{}
	dbServerA, err := s1.Setup()
	tu.AssertOk(t, err, "failed to setup mockdb")
	...
	var wg sync.WaitGroup
	for ix := 0; ix < 3; ix++ {
		l = statssim.GetLinePoints(ss, testMeas, 6)
		wg.Add(1)
		go func(l []string) {
			defer wg.Done()
			c.WriteLines(testDB, l)
