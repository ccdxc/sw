...
func AddUserToRequest(request *http.Request, user *auth.User) error {
	return authz.PopulateMapWithUserPerms(request.Header, user, nil, true)
}
...
func AddUserPermsToRequest(request *http.Request, user *auth.User, perms []auth.Permission) error {
	return authz.PopulateMapWithUserPerms(request.Header, user, perms, true)
}
