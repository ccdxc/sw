...
func TestIsAuthorized(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			perms:     []auth.Permission{login.NewPermission("", string(apiclient.GroupAuth), "", authz.ResourceNamespaceAll, "", auth.Permission_AllActions.String())},
...
func TestAuthorizedOperations(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "all actions tenant scoped permission for one api group",
			user:      &auth.User{ObjectMeta: api.ObjectMeta{Name: "testuser", Tenant: globals.DefaultTenant}},
		...
		{
			name:      "all actions cluster scoped permission for one api group",
			user:      &auth.User{ObjectMeta: api.ObjectMeta{Name: "testuser", Tenant: globals.DefaultTenant}},
			perms:     []auth.Permission{login.NewPermission("", string(apiclient.GroupAuth), "", "", "", auth.Permission_AllActions.String())},
			tenant:    "",
			namespace: "",
