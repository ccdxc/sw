...
type MethodParams struct {
	...
	TenantDefault bool
	// AutoGen indicates that this was an autogenerated method
	AutoGen bool
}
...
func getMethodParams(m *descriptor.Method) (MethodParams, error) {
	...
	if d, err := reg.GetExtension("venice.methodAutoGen", m); err == nil {
		params.AutoGen = d.(bool)
	}
...
func genServiceManifestInternal(filenm string, file *descriptor.File) (map[string]packageDef, error) {
	...
	apiServerBacked, _ := isAPIServerServed(file)
	...
	for _, svc := range file.Services {
		...
		if apiServerBacked || len(svcdef.Messages) > 0 {
