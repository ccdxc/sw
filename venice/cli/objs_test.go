...
func TestGetKvs(t *testing.T) {
	...
	wObj.Spec.Interfaces = map[string]workload.WorkloadIntfSpec{
		"00.11.22.44.55.66": {MicroSegVlan: 23, ExternalVlan: 100},
		"00.11.33.44.55.66": {MicroSegVlan: 33, ExternalVlan: 200}}
	...
	if !reflect.DeepEqual(specKvs["micro-seg-vlan"].values, []string{"00.11.22.44.55.66" + kvSplitter + "23", "00.11.33.44.55.66" + kvSplitter + "33"}) &&
		!reflect.DeepEqual(specKvs["micro-seg-vlan"].values, []string{"00.11.33.44.55.66" + kvSplitter + "33", "00.11.22.44.55.66" + kvSplitter + "23"}) {
		t.Fatalf("unable to get specKvs %+v", specKvs)
	}
	if !reflect.DeepEqual(specKvs["external-vlan"].values, []string{"00.11.22.44.55.66" + kvSplitter + "100", "00.11.33.44.55.66" + kvSplitter + "200"}) &&
		!reflect.DeepEqual(specKvs["external-vlan"].values, []string{"00.11.33.44.55.66" + kvSplitter + "200", "00.11.22.44.55.66" + kvSplitter + "100"}) {
		t.Fatalf("unable to get specKvs %+v", specKvs)
	}
}
...
func TestGetAllKvs(t *testing.T) {
	...
	workload2 := &workload.Workload{
		...
		Spec: workload.WorkloadSpec{
			HostName: "node021",
			Interfaces: map[string]workload.WorkloadIntfSpec{
				"44:44:44:44:44:44": {ExternalVlan: 11, MicroSegVlan: 4000},
			},
		},
	}
	...
	if !specValidKvs["host-name"] || !specValidKvs["micro-seg-vlan"] || !specValidKvs["external-vlan"] {
