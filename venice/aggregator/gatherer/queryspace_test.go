...
func TestFormSubquery_Success(t *testing.T) {
	for _, tt := range []struct {
		q  string
		sq string
	}{
		{q: `SELECT time, value FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT value FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT value, host FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT * FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT time, * FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT value, * FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT max(value) FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT max(value), host FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT max(value), * FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT max(*) FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT max(/val/) FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT min(value) FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT min(value), host FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT min(value), * FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT min(*) FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT min(/val/) FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT first(value) FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT first(value), host FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT first(value), * FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT first(*) FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT first(/val/) FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT last(value) FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT last(value), host FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT last(value), * FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT last(*) FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT last(/val/) FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT count(value) FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT count(distinct(value)) FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT count(distinct value) FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT count(*) FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT count(/val/) FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT mean(value) FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT mean(*) FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT mean(/val/) FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT min(value), max(value) FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT min(*), max(*) FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT min(/val/), max(/val/) FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT first(value), last(value) FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT first(*), last(*) FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT first(/val/), last(/val/) FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT count(value) FROM cpu WHERE time >= now() - 1h GROUP BY time(10m)`, sq: `SELECT * FROM cpu WHERE time >= now() - 1h`},
		{q: `SELECT distinct value FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT distinct(value) FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT value / total FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT min(value) / total FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT max(value) / total FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT top(value, 1) FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT top(value, host, 1) FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT top(value, 1), host FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT min(top) FROM (SELECT top(value, host, 1) FROM cpu) GROUP BY region`, sq: `SELECT * FROM cpu`},
		{q: `SELECT bottom(value, 1) FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT bottom(value, host, 1) FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT bottom(value, 1), host FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT max(bottom) FROM (SELECT bottom(value, host, 1) FROM cpu) GROUP BY region`, sq: `SELECT * FROM cpu`},
		{q: `SELECT percentile(value, 75) FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT percentile(value, 75.0) FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT sample(value, 2) FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT sample(*, 2) FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT sample(/val/, 2) FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT elapsed(value) FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT elapsed(value, 10s) FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT integral(value) FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT integral(value, 10s) FROM cpu`, sq: `SELECT * FROM cpu`},
		{q: `SELECT max(value) FROM cpu WHERE time >= now() - 1m GROUP BY time(10s, 5s)`, sq: `SELECT * FROM cpu WHERE time >= now() - 1m`},
		{q: `SELECT max(value) FROM cpu WHERE time >= now() - 1m GROUP BY time(10s, '2000-01-01T00:00:05Z')`, sq: `SELECT * FROM cpu WHERE time >= now() - 1m`},
		{q: `SELECT max(value) FROM cpu WHERE time >= now() - 1m GROUP BY time(10s, now())`, sq: `SELECT * FROM cpu WHERE time >= now() - 1m`},
		{q: `SELECT max(mean) FROM (SELECT mean(value) FROM cpu GROUP BY host)`, sq: `SELECT * FROM cpu`},
		{q: `SELECT max(derivative) FROM (SELECT derivative(mean(value)) FROM cpu) WHERE time >= now() - 1m GROUP BY time(10s)`, sq: `SELECT * FROM cpu`},
		{q: `SELECT max(value) FROM (SELECT value + total FROM cpu) WHERE time >= now() - 1m GROUP BY time(10s)`, sq: `SELECT * FROM cpu`},
		{q: `SELECT value FROM cpu WHERE time >= '2000-01-01T00:00:00Z' AND time <= '2000-01-01T01:00:00Z'`, sq: `SELECT * FROM cpu WHERE time >= '2000-01-01T00:00:00Z' AND time <= '2000-01-01T01:00:00Z'`},
		{q: `SELECT value FROM (SELECT value FROM cpu) ORDER BY time DESC`, sq: `SELECT * FROM cpu`},
	} {
		t.Run(tt.q, func(t *testing.T) {
			stmt, err := influxql.ParseStatement(tt.q)
			if err != nil {
			...
			if sq, err := FormSubquery(s, opt); err != nil {
				t.Errorf("unexpected error: %s", err)
			} else if sq != tt.sq {
				t.Errorf("Expected [%s], got [%s]", tt.sq, sq)
			}
		})
	}
}

func TestFormSubquery_Failures(t *testing.T) {
	for _, tt := range []struct {
		s   string
		err string
	}{
		...
		t.Run(tt.s, func(t *testing.T) {
			...
			opt := query.CompileOptions{}
			if _, err := FormSubquery(s, opt); err == nil {
...
func TestGatherPoints(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{ss: `SHOW MEASUREMENT CARDINALITY WHERE time > now() - 1h`,
			expErr: true,
		},
		{ss: `SHOW MEASUREMENT CARDINALITY`,
			expErr: true,
		},
		{ss: `SELECT time, value FROM cpu`,
			expErr: true,
		},
	} {
		stmt, err := influxql.ParseStatement(tc.ss)
		if err != nil {
			t.Fatalf("unexpected error: %s", err)
		}

		err = qs.GatherPoints(stmt, "nodb")
		if (err != nil) != tc.expErr {
			t.Errorf("%s - Expect error %v, got %v", tc.ss, tc.expErr, !tc.expErr)
		}
	}
}
