...
// Make sure all values are of the specified type
func validateValues(values []Value, et int) error {
...
func newEntryValues(values []Value, hint int) (*entry, error) {
	...
	if validateValues(values, et) != nil {
...
func (e *entry) add(values []Value) error {
	...
	if validateValues(values, e.vtype) != nil {
		return tsdb.ErrFieldTypeConflict
	}

	// entry currently has no values, so add the new ones and we're done.
	e.mu.Lock()
	e.appendValues(values, 32)
	e.mu.Unlock()
	return nil
}

// appendValues appends values to the entry
func (e *entry) appendValues(values []Value, minAlloc int) {
	if len(e.values) != 0 {
		// simple append
	...
	if len(values) >= minAlloc {
		// replace
		e.values = values
		return
	}

	e.values = make(Values, 0, minAlloc)
	e.values = append(e.values, values...)
}
...
// Note: path is unused, kept only for consistency
...
// LastModified returns the last write time
...
// ClearSnapshot is not implemented
...
// Count returns the number of items in the cache store
...
// Split returns a split list of caches
...
// DeleteRange is not implemented
...
// Load is n/a for cache only engine
