...
func (fs Set) Has(field string) bool {
	_, exists := fs[field]
	return exists
}

// Get returns the value in the map for the provided field.
func (fs Set) Get(field string) string {
	return fs[field]
}

// AsSelector converts fields into a selectors.
func (fs Set) AsSelector() *Selector {
	return SelectorFromSet(fs)
}
