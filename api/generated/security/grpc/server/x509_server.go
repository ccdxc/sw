...
func (s *ssecurityX509Backend) regMsgsFunc(l log.Logger, scheme *runtime.Scheme) {
	...
	s.Messages = map[string]apiserver.Message{
		...
		"security.Certificate": apisrvpkg.NewMessage("security.Certificate").WithKeyGenerator(func(i interface{}, prefix string) string {
			...
			if create {
				if updateFn != nil {
					upd := &security.Certificate{}
					n, err := updateFn(upd)
					if err != nil {
						l.ErrorLog("msg", "could not create new object", "err", err)
						return nil, err
					}
					new := n.(*security.Certificate)
			...
			var n *security.Certificate
			if v, ok := i.(security.Certificate); ok {
				n = &v
			} else if v, ok := i.(*security.Certificate); ok {
				n = v
			} else {
				return nil
			}
