...
func (s *sclusterClusterBackend) regMsgsFunc(l log.Logger, scheme *runtime.Scheme) {
	...
	s.Messages = map[string]apiserver.Message{
		...
		}).WithStorageTransformer(&cluster.StorageClusterTransformer),
		...
		"cluster.DistributedServiceCardID": apisrvpkg.NewMessage("cluster.DistributedServiceCardID"),
		...
		"cluster.UpdateTLSConfigRequest": apisrvpkg.NewMessage("cluster.UpdateTLSConfigRequest").WithKeyGenerator(func(i interface{}, prefix string) string {
			...
			if create {
				if updateFn != nil {
					upd := &cluster.UpdateTLSConfigRequest{}
					n, err := updateFn(upd)
					if err != nil {
						l.ErrorLog("msg", "could not create new object", "err", err)
						return nil, err
					}
					new := n.(*cluster.UpdateTLSConfigRequest)
					new.TypeMeta = r.TypeMeta
					new.GenerationID = "1"
					new.UUID = r.UUID
					new.CreationTime = r.CreationTime
				...
				var cur cluster.UpdateTLSConfigRequest
				err = kvs.Get(ctx, key, &cur)
				if err != nil {
					l.ErrorLog("msg", "trying to update an object that does not exist", "key", key, "err", err)
					return nil, err
			...
			if create {
				...
				if updatefn != nil {
					...
					r = *robj.(*cluster.UpdateTLSConfigRequest)
					txn.AddComparator(kvstore.Compare(kvstore.WithVersion(key), "=", r.ResourceVersion))
				} else {
					var cur cluster.UpdateTLSConfigRequest
					err = kvs.Get(ctx, key, &cur)
		...
		"cluster.Version": apisrvpkg.NewMessage("cluster.Version").WithKeyGenerator(func(i interface{}, prefix string) string {
			if i == nil {
				r := cluster.Version{}
				return r.MakeKey(prefix)
			}
			...
			var n *cluster.Version
			if v, ok := i.(cluster.Version); ok {
				n = &v
			} else if v, ok := i.(*cluster.Version); ok {
				n = v
			} else {
				return nil
			}
		...
		"cluster.VersionSpec":   apisrvpkg.NewMessage("cluster.VersionSpec"),
		"cluster.VersionStatus": apisrvpkg.NewMessage("cluster.VersionStatus"),
