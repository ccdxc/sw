...
type sclusterCredentialsBackend struct {
...
func (s *sclusterCredentialsBackend) regMsgsFunc(l log.Logger, scheme *runtime.Scheme) {
	l.Infof("registering message for sclusterCredentialsBackend")
	s.Messages = map[string]apiserver.Message{

		"cluster.Credentials": apisrvpkg.NewMessage("cluster.Credentials").WithKeyGenerator(func(i interface{}, prefix string) string {
			if i == nil {
				r := cluster.Credentials{}
				return r.MakeKey(prefix)
			}
			r := i.(cluster.Credentials)
			return r.MakeKey(prefix)
			...
			var n *cluster.Credentials
			if v, ok := i.(cluster.Credentials); ok {
				n = &v
			} else if v, ok := i.(*cluster.Credentials); ok {
				n = v
			} else {
				return nil
			}
		...
		}).WithStorageTransformer(&cluster.StorageCredentialsTransformer),

		"cluster.CredentialsSpec":   apisrvpkg.NewMessage("cluster.CredentialsSpec"),
		"cluster.CredentialsStatus": apisrvpkg.NewMessage("cluster.CredentialsStatus"),
		"cluster.KeyValue":          apisrvpkg.NewMessage("cluster.KeyValue"),
...
func (s *sclusterCredentialsBackend) regSvcsFunc(ctx context.Context, logger log.Logger, grpcserver *rpckit.RPCServer, scheme *runtime.Scheme) {

}

func (s *sclusterCredentialsBackend) regWatchersFunc(ctx context.Context, logger log.Logger, grpcserver *rpckit.RPCServer, scheme *runtime.Scheme) {

}

func (s *sclusterCredentialsBackend) CompleteRegistration(ctx context.Context, logger log.Logger,
...
func (s *sclusterCredentialsBackend) Reset() {
	cleanupRegistration()
}
