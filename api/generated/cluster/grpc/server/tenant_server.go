...
func (s *sclusterTenantBackend) regMsgsFunc(l log.Logger, scheme *runtime.Scheme) {
	...
	s.Messages = map[string]apiserver.Message{
		...
		"cluster.Tenant": apisrvpkg.NewMessage("cluster.Tenant").WithKeyGenerator(func(i interface{}, prefix string) string {
			...
			var n *cluster.Tenant
			if v, ok := i.(cluster.Tenant); ok {
				n = &v
			} else if v, ok := i.(*cluster.Tenant); ok {
				n = v
			} else {
				return nil
			}
			...
			return func(oldObj runtime.Object) (runtime.Object, error) {
				...
				if ret, ok := oldObj.(*cluster.Tenant); ok {
