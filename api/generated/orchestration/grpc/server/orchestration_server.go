...
type sorchestrationOrchestrationBackend struct {
...
func (s *sorchestrationOrchestrationBackend) regMsgsFunc(l log.Logger, scheme *runtime.Scheme) {
	...
	s.Messages = map[string]apiserver.Message{
		...
		"orchestration.Orchestrator": apisrvpkg.NewMessage("orchestration.Orchestrator").WithKeyGenerator(func(i interface{}, prefix string) string {
			...
			r := i.(orchestration.Orchestrator)
			return r.MakeKey(prefix)
			...
			if create {
				if updateFn != nil {
					upd := &orchestration.Orchestrator{}
					n, err := updateFn(upd)
					if err != nil {
						l.ErrorLog("msg", "could not create new object", "err", err)
						return nil, err
					}
					new := n.(*orchestration.Orchestrator)
				...
				if updateFn != nil {
					into := &orchestration.Orchestrator{}
					...
					var cur orchestration.Orchestrator
			...
			if create {
				...
				if updatefn != nil {
					...
					r = *robj.(*orchestration.Orchestrator)
			...
			var n *orchestration.Orchestrator
			if v, ok := i.(orchestration.Orchestrator); ok {
				n = &v
			} else if v, ok := i.(*orchestration.Orchestrator); ok {
				n = v
			} else {
				return nil
			}
			return func(oldObj runtime.Object) (runtime.Object, error) {
				...
				o := oldObj.(*orchestration.Orchestrator)
			...
			return func(oldObj runtime.Object) (runtime.Object, error) {
				if oldObj == nil {
					rete := &orchestration.Orchestrator{}
...
func (s *sorchestrationOrchestrationBackend) regSvcsFunc(ctx context.Context, logger log.Logger, grpcserver *rpckit.RPCServer, scheme *runtime.Scheme) {

}

func (s *sorchestrationOrchestrationBackend) regWatchersFunc(ctx context.Context, logger log.Logger, grpcserver *rpckit.RPCServer, scheme *runtime.Scheme) {

}

func (s *sorchestrationOrchestrationBackend) CompleteRegistration(ctx context.Context, logger log.Logger,
...
func (s *sorchestrationOrchestrationBackend) Reset() {
	cleanupRegistration()
}
