...
type snetworkRouteBackend struct {
...
func (s *snetworkRouteBackend) regMsgsFunc(l log.Logger, scheme *runtime.Scheme) {
	...
	s.Messages = map[string]apiserver.Message{
		...
		"network.RDSpec":             apisrvpkg.NewMessage("network.RDSpec"),
		"network.Route":              apisrvpkg.NewMessage("network.Route"),
		"network.RouteDistinguisher": apisrvpkg.NewMessage("network.RouteDistinguisher"),
		"network.RouteTable": apisrvpkg.NewMessage("network.RouteTable").WithKeyGenerator(func(i interface{}, prefix string) string {
			if i == nil {
				r := network.RouteTable{}
				return r.MakeKey(prefix)
			}
			r := i.(network.RouteTable)
			return r.MakeKey(prefix)
			...
			var n *network.RouteTable
			if v, ok := i.(network.RouteTable); ok {
				n = &v
			} else if v, ok := i.(*network.RouteTable); ok {
				n = v
			} else {
				return nil
			}
		...
		"network.RouteTableSpec":   apisrvpkg.NewMessage("network.RouteTableSpec"),
		"network.RouteTableStatus": apisrvpkg.NewMessage("network.RouteTableStatus"),
		"network.RoutingConfig": apisrvpkg.NewMessage("network.RoutingConfig").WithKeyGenerator(func(i interface{}, prefix string) string {
			if i == nil {
				r := network.RoutingConfig{}
				return r.MakeKey(prefix)
			}
			r := i.(network.RoutingConfig)
			return r.MakeKey(prefix)
		}).WithObjectVersionWriter(func(i interface{}, version string) interface{} {
			r := i.(network.RoutingConfig)
			r.Kind = "RoutingConfig"
			r.APIVersion = version
			return r
		}).WithKvUpdater(func(ctx context.Context, kvs kvstore.Interface, i interface{}, prefix string, create bool, updateFn kvstore.UpdateFunc) (interface{}, error) {
			r := i.(network.RoutingConfig)
			...
			if create {
				if updateFn != nil {
					upd := &network.RoutingConfig{}
					n, err := updateFn(upd)
					if err != nil {
						l.ErrorLog("msg", "could not create new object", "err", err)
						return nil, err
					}
					new := n.(*network.RoutingConfig)
					new.TypeMeta = r.TypeMeta
					new.GenerationID = "1"
					new.UUID = r.UUID
					new.CreationTime = r.CreationTime
			...
			var n *network.RoutingConfig
			if v, ok := i.(network.RoutingConfig); ok {
				n = &v
			} else if v, ok := i.(*network.RoutingConfig); ok {
				n = v
			} else {
				return nil
			}
			return func(oldObj runtime.Object) (runtime.Object, error) {
				...
				o := oldObj.(*network.RoutingConfig)
				n.UUID, n.CreationTime, n.Namespace, n.GenerationID = o.UUID, o.CreationTime, o.Namespace, o.GenerationID
				ts, err := types.TimestampProto(time.Now())
				if err != nil {
					return nil, err
...
func (s *snetworkRouteBackend) regSvcsFunc(ctx context.Context, logger log.Logger, grpcserver *rpckit.RPCServer, scheme *runtime.Scheme) {

}

func (s *snetworkRouteBackend) regWatchersFunc(ctx context.Context, logger log.Logger, grpcserver *rpckit.RPCServer, scheme *runtime.Scheme) {

}

func (s *snetworkRouteBackend) CompleteRegistration(ctx context.Context, logger log.Logger,
...
func (s *snetworkRouteBackend) Reset() {
	cleanupRegistration()
}
