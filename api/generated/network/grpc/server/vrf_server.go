...
type snetworkVrfBackend struct {
...
func (s *snetworkVrfBackend) regMsgsFunc(l log.Logger, scheme *runtime.Scheme) {
	l.Infof("registering message for snetworkVrfBackend")
	s.Messages = map[string]apiserver.Message{

		"network.VirtualRouter": apisrvpkg.NewMessage("network.VirtualRouter").WithKeyGenerator(func(i interface{}, prefix string) string {
			if i == nil {
			...
			if create {
				if updateFn != nil {
					upd := &network.VirtualRouter{}
					n, err := updateFn(upd)
					if err != nil {
						l.ErrorLog("msg", "could not create new object", "err", err)
						return nil, err
					}
					new := n.(*network.VirtualRouter)
			...
			var n *network.VirtualRouter
			if v, ok := i.(network.VirtualRouter); ok {
				n = &v
			} else if v, ok := i.(*network.VirtualRouter); ok {
				n = v
			} else {
				return nil
			}
			return func(oldObj runtime.Object) (runtime.Object, error) {
				...
				o := oldObj.(*network.VirtualRouter)
		...
		"network.VirtualRouterSpec":   apisrvpkg.NewMessage("network.VirtualRouterSpec"),
		"network.VirtualRouterStatus": apisrvpkg.NewMessage("network.VirtualRouterStatus"),
...
func (s *snetworkVrfBackend) regSvcsFunc(ctx context.Context, logger log.Logger, grpcserver *rpckit.RPCServer, scheme *runtime.Scheme) {

}

func (s *snetworkVrfBackend) regWatchersFunc(ctx context.Context, logger log.Logger, grpcserver *rpckit.RPCServer, scheme *runtime.Scheme) {

}

func (s *snetworkVrfBackend) CompleteRegistration(ctx context.Context, logger log.Logger,
...
func (s *snetworkVrfBackend) Reset() {
	cleanupRegistration()
}
