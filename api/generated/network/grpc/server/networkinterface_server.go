...
type snetworkNetworkinterfaceBackend struct {
...
func (s *snetworkNetworkinterfaceBackend) regMsgsFunc(l log.Logger, scheme *runtime.Scheme) {
	...
	s.Messages = map[string]apiserver.Message{
		...
		"network.NetworkInterface": apisrvpkg.NewMessage("network.NetworkInterface").WithKeyGenerator(func(i interface{}, prefix string) string {
			...
			if create {
				if updateFn != nil {
					upd := &network.NetworkInterface{}
					n, err := updateFn(upd)
					if err != nil {
						l.ErrorLog("msg", "could not create new object", "err", err)
						return nil, err
					}
					new := n.(*network.NetworkInterface)
				...
				if updateFn != nil {
					...
					var cur network.NetworkInterface
			...
			if create {
				...
				if updatefn != nil {
					...
					r = *robj.(*network.NetworkInterface)
			...
			var n *network.NetworkInterface
			if v, ok := i.(network.NetworkInterface); ok {
				n = &v
			} else if v, ok := i.(*network.NetworkInterface); ok {
				n = v
			} else {
				return nil
			}
			return func(oldObj runtime.Object) (runtime.Object, error) {
				...
				o := oldObj.(*network.NetworkInterface)
		...
		"network.NetworkInterfaceHostStatus":   apisrvpkg.NewMessage("network.NetworkInterfaceHostStatus"),
...
func (s *snetworkNetworkinterfaceBackend) regSvcsFunc(ctx context.Context, logger log.Logger, grpcserver *rpckit.RPCServer, scheme *runtime.Scheme) {

}

func (s *snetworkNetworkinterfaceBackend) regWatchersFunc(ctx context.Context, logger log.Logger, grpcserver *rpckit.RPCServer, scheme *runtime.Scheme) {

}

func (s *snetworkNetworkinterfaceBackend) CompleteRegistration(ctx context.Context, logger log.Logger,
...
func (s *snetworkNetworkinterfaceBackend) Reset() {
	cleanupRegistration()
}
