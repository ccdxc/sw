...
func (e *sSearchV1GwService) CompleteRegistration(ctx context.Context,
	...
	if err != nil {
		logger.ErrorLog("msg", "failed to register swagger spec", "service", "search.SearchV1", "err", err)
	}
	wg.Add(1)
	go func() {
		defer func() {
		...
		for {
			...
			cancel()
			logger.ErrorLog("msg", "failed to register", "service", "search.SearchV1", "err", err)
			select {
			case <-ctx.Done():
				return
