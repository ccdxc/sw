...
func TestCrudOps(t *testing.T) {
	...
	go func() {
		...
		aopts := api.AggWatchOptions{}
		srvWatcher, err := apicl.BookstoreV1().Watch(wctx, &aopts)
		if err != nil {
			t.Fatalf("Failed to start watch (%s)\n", err)
		}
		close(waitWatch)
...
func TestRoutingConfigReferences(t *testing.T) {
	...
	prof := &cluster.DSCProfile{
		ObjectMeta: api.ObjectMeta{
			Name: cluster.DSCProfileSpec_SMARTNIC.String(),
		},
	...
	dsc := &cluster.DistributedServiceCard{
		...
		Spec: cluster.DistributedServiceCardSpec{
			...
			DSCProfile:  cluster.DSCProfileSpec_SMARTNIC.String(),
		},
