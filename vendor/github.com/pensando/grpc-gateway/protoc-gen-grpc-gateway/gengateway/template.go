	...
	for _, svc := range p.Services {
		...
		for _, meth := range svc.Methods {
			...
			for _, b := range meth.Bindings {
				...
				if err := handlerTemplate.Funcs(funcMap).Execute(w, binding{Binding: b}); err != nil {
					return "", err
				}
			}
		}
...
var (
	funcMap        = template.FuncMap{"hasPrefix": hasPrefix, "trimPrefix": trimPrefix}
...
{{if .PathParams}}
	...
	{{if hasPrefix ($param | printf "%q") "TOCTX."}}
		kvMap["{{trimPrefix $param "TOCTX."}}"] = val
	}} {{else}} {{if $param.IsNestedProto3 }}
...
func Register{{$svc.GetName}}Handler(ctx context.Context, mux *runtime.ServeMux, conn *grpc.ClientConn) error {
	...
	return Register{{$svc.GetName}}HandlerWithClient(ctx, mux, client)
}

// Register{{$svc.GetName}}HandlerClient registers the http handlers for service {{$svc.GetName}} to "mux".
// The handlers forward requests to the grpc endpoint using client provided.
func Register{{$svc.GetName}}HandlerWithClient(ctx context.Context, mux *runtime.ServeMux, client {{$svc.GetName}}Client) error {
	{{range $m := $svc.Methods}}
	{{range $b := $m.Bindings}}
